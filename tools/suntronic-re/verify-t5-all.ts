...
import { CORPUS_DIR, addCompanions, loadInstrCompanions, parseHunks, listCorpusModules } from './suntronicLib';
...
async function main(){
  ...
  let songs=0,recPlayed=0,recHitAll=0,totalHit=0,totalTry=0,contradictions=0;
  for(const name of listCorpusModules()){
    let score,h1;
    try{const data=new Uint8Array(readFileSync(join(CORPUS_DIR,name)));score=parseSunTronicV13Score(data);h1=parseHunks(data).hunks[1].data;
      const t5=score.synthInstruments.filter((s:any)=>s.synthType===5); if(!t5.length)continue;
      const uade=capture(mod,name,data); songs++;
      for(const inst of t5){const byteLen=inst.waveWordLen*2,off=inst.wave1Off;
        const arps=[...new Set(Array.from(inst.arpTable) as number[])];
        let hit=0,tri=0;
        for(const arp of arps){const st=off+2*arp;if(st<0||st+byteLen>h1.length)continue;tri++;if(uade.has(hex(h1.slice(st,st+byteLen))))hit++;}
        if(hit>0){recPlayed++;totalHit+=hit;totalTry+=tri;if(hit===tri)recHitAll++;}
      }
    }catch(e){continue;}
  }
  console.log(`songs=${songs} recordsWithHits=${recPlayed} allArpsHit=${recHitAll} windows ${totalHit}/${totalTry}`);
