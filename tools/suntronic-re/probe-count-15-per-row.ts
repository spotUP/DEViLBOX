/** probe-count-15-per-row.ts — resolve the static/dynamic contradiction. Static
 ...
 * $15 writes (once/tick, no double-write) per row. Gate every $2c write, read back $2c: the
...
async function run(name: string): Promise<void> {
  ...
  console.log(`\n${name} base0=${base0.toString(16)} — $15 writes (once/tick, no double-write) per row:`);
  let row = 0, ticks = 0, lastRR = rB(base0 + 0x2d);
  for (let s = 0; s < 200000 && row < ROWS; s++) {
    ...
    if (mod._uade_wasm_get_capture(cap)) {
      ticks++; // every $15 write == one handler tick (EFFECTS runs every tick)
      const rr = rB(base0 + 0x2d);
      if (rr !== lastRR) {
        console.log(`  row${String(row).padStart(2)} ticks=${ticks} ($2d ${lastRR}->${rr})`);
        row++; ticks = 0; lastRR = rr;
      }
    }
  }
