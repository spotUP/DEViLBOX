/** p7s-dump-bin.ts — dump a loaded-replayer memory region to a raw .bin for capstone. */
...
async function main(): Promise<void> {
  ...
  const lo = parseInt(process.argv[3] ?? '0x26e90', 16);
  const hi = parseInt(process.argv[4] ?? '0x26f00', 16);
  const outPath = process.argv[5] ?? '/tmp/calc14.bin';
  ...
  console.log(`[p7s] wrote ${len} bytes @ 0x${lo.toString(16)} -> ${outPath}`);
