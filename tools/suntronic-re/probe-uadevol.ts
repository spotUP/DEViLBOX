...
async function main(){
  ...
  const per:number[]=[],vol:number[]=[];let curV=-1;
  for(let c=0;c<N*3&&per.length<N;c++){
    ...
    for(let i=0;i<n;i++){const packed=hh[base+i*3];const ch=(packed>>>24)&0xff,reg=(packed>>>16)&0xff,val=packed&0xffff;
      if(ch!==V)continue; if(reg===REG_VOL)curV=val; else if(reg===REG_PER){per.push(val);vol.push(curV);}}
  }
  console.log(`UADE ${NAME} v${V}:`);
  for(let c=0;c<Math.min(N,per.length);c++)console.log(`  tick ${c}: per=${per[c]} vol=${vol[c]}`);
}
