/** probe-tempo-counters.ts — per-tick dump of UADE voice0 tempo counters.
 * Polls voice0 $24 (vibrato, one write/tick) as the tick clock; on each change,
 * snapshots $2c(tickInRow) $2d(rowInPos) $2e(pos) $30(speed) $31(rowsPerPos)
 * plus vibPhase/period. Reveals the true row-length pattern (native uses flat
 * speed=6; UADE doubles suggest 7,6,6,6). NOT committed. */
...
async function run(name: string): Promise<void> {
  ...
  let prev = rW(base0 + 0x24); const rows: string[] = [];
  console.log(`\n${name} base0=${base0.toString(16)} per-tick voice0 tempo counters:`);
  let tick = 0, lastDoubleTick = -1;
  for (let s = 0; s < 60000 && tick < TICKS; s++) {
    if (mod._uade_wasm_render(L, R, 1) <= 0) break;
    const cur = rW(base0 + 0x24);
    if (cur !== prev) {
      const vibDelta = ((cur << 16) >> 16) - ((prev << 16) >> 16);
      const b = base0;
      const tc = rB(b + 0x2c, 1)[0], rr = rB(b + 0x2d, 1)[0], pos = rW(b + 0x2e);
      const sp = rB(b + 0x30, 1)[0], rpp = rB(b + 0x31, 1)[0];
      const isDouble = Math.abs(vibDelta) > 12000;
      const gap = isDouble ? (lastDoubleTick < 0 ? 0 : tick - lastDoubleTick) : 0;
      if (isDouble) { rows.push(String(gap)); lastDoubleTick = tick; }
      console.log(`  t${String(tick).padStart(2)} $2c=${tc} $2d=${rr} $2e=${pos} $30(sp)=${sp} $31(rpp)=${rpp} vibΔ=${vibDelta}${isDouble ? ` DOUBLE gap=${gap}` : ''}`);
      prev = cur; tick++;
    }
  }
  console.log(`  row gaps: [${rows.join(',')}]`);
