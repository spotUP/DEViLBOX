/** probe-double-positions.ts — dump UADE voice0 $24 (vibPhase) per clean tick over a
 * long span; list the tick indices where the delta is the DOUBLE (+2×speed) instead of
 * the single +speed. Decides whether the extra vibrato advance is periodic (a vibrato-
 * internal sub-counter) or note-row driven. NOT committed. */
...
type AnyMod = any;
const SCAN_LO = 0x20000, SCAN_HI = 0x40000, STRIDE = 0x1ba, TICKS = 64;
async function run(name: string): Promise<void> {
  ...
  const readW = (b: number): number => { mod._uade_wasm_read_memory(b >>> 0, rd, 2); return s16((mod.HEAPU8[rd] << 8) | mod.HEAPU8[rd + 1]); };
  mod._uade_wasm_stop(); load();
  const vib: number[] = [];
  for (let s = 0; s < 100000 && vib.length < TICKS; s++) {
    mod._uade_wasm_arm_capture((base0 + 0x15) >>> 0, 1); mod._uade_wasm_arm_capture_pc(0, 0);
    if (mod._uade_wasm_render(L, R, 1) <= 0) break;
    if (mod._uade_wasm_get_capture(cap)) vib.push(readW(base0 + 0x24));
  }
  const doubles: number[] = [];
  for (let i = 1; i < vib.length; i++) { let d = vib[i] - vib[i - 1]; if (d < -32768) d += 65536; if (d > 32768) d -= 65536; if (Math.abs(d) > 12000) doubles.push(i); }
  const gaps: number[] = []; for (let i = 1; i < doubles.length; i++) gaps.push(doubles[i] - doubles[i - 1]);
  console.log(`${name}: double-advance ticks = [${doubles.join(',')}]`);
