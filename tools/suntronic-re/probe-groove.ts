/** probe-groove.ts — decode the global groove/fractional-tempo counter. The tick
 * handler's global block at 0x2660e reads/writes $a8a..$a8d off a6(=0x264dc). Row
 * boundaries measured 5,5,5,5,5,6 ticks with $30=6 static → a groove inserts +1 tick
 * every 6th row. Gate on voice0 $15 (one write/tick) and dump $a8a..$a8d + $2c/$2d
 * each tick to read the counter's state machine. Also dumps $a9c/$a9e (the second
 * global accumulator at 0x26644). NOT committed. */
...
type AnyMod = any;
const SCAN_LO = 0x20000, SCAN_HI = 0x40000, A6 = 0x264dc, TICKS = 24;
async function run(name: string): Promise<void> {
  ...
  console.log(`\n${name} base0=${base0.toString(16)} groove globals @a6=${A6.toString(16)} per tick:`);
  let tick = 0;
  for (let s = 0; s < 60000 && tick < TICKS; s++) {
    ...
    if (mod._uade_wasm_get_capture(cap)) {
      const a8a = rB(A6 + 0xa8a, 1)[0], a8b = s8(rB(A6 + 0xa8b, 1)[0]), a8c = s8(rB(A6 + 0xa8c, 1)[0]), a8d = s8(rB(A6 + 0xa8d, 1)[0]);
      const a9c = rW(A6 + 0xa9c), a9e = rW(A6 + 0xa9e);
      const tc = rB(base0 + 0x2c, 1)[0], rr = rB(base0 + 0x2d, 1)[0];
      console.log(`  t${String(tick).padStart(2)} $2c=${tc} $2d=${rr} | a8a=${a8a} a8b=${a8b} a8c=${a8c} a8d=${a8d} | a9c=${a9c} a9e=${a9e}`);
      tick++;
    }
  }
