...
async function main(){
  ...
  for(let c=0;c<N*3 && vol.length<N;c++){
    ...
    for(let i=0;i<n;i++){const pk=hh[base+i*3];const ch=(pk>>>24)&0xff,reg=(pk>>>16)&0xff,val=pk&0xffff;
      if(ch!==V)continue; if(reg===REG_VOL)cur=val; else if(reg===REG_PER)vol.push(cur);}
  }
  console.log(`${name} v${V} UADE Paula VOL seq:`, vol.slice(0,N).join(','));
}
