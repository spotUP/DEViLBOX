...
_fdist_in:
  ...
  {  /* SUB.W	D1,(A1) */
      uint16_t _sr = (uint16_t)(READ16(a1) - W(d1));
...
_top: ;
  ...
  /* MOVEM: no addr operand */  /* MOVEM.W	D0,D1,c_dma-c_MusicLoop(A0) */
  return;  /* RTS */
}
...
_top: ;
  ...
  {  /* ADDQ.L	#2,(A0)+ */
      uint32_t _ar = (uint32_t)(READ32_POST(a0) + 2);
      WRITE32_POST(a0, (uint32_t)_ar);
      flag_z = ((int32_t)(_ar) == 0);
      flag_n = ((int32_t)(_ar) < 0);
    }
  ...
  if (!(flag_n)) {
  /* Adjust volume */
  W(d0) = (uint16_t)(ROL32(W(d0), 7));  /* ROL.W	#7,D0 */
  {  /* ADD.W	D0,D2 */
...
_noteloop:
  ...
  {  /* SUB.W	D4,D0 */
      uint16_t _sr = (uint16_t)(W(d0) - W(d4));
      W(d0) = (uint16_t)((uint16_t)_sr);
      flag_z = ((int16_t)(_sr) == 0);
      flag_n = ((int16_t)(_sr) < 0);
    }
  ...
  /* MOVEM: no addr operand */  /* MOVEM.L	D6,A4,-8(A0) */
...
do {
  {  /* CMP.W	6(A3),D1 */
      ...
      int32_t _rhs = (int32_t)(READ16(a3 + 6));
      int32_t _cmp = _lhs - _rhs;
      flag_z = (_cmp == 0);
      flag_n = (_cmp < 0);
      flag_c = ((uint32_t)_lhs < (uint32_t)_rhs);
    }
} while (!flag_z && (flag_n==flag_v));
  {  /* MOVE.W	D0,150(A3) */
      uint16_t _mv = (uint16_t)(W(d0));
      hw_write16(a3 + 150, _mv);
      flag_z = ((int16_t)(_mv) == 0);
