/**
 * GranularFreeze.worklet.js - Granular Freeze AudioWorklet Effect Processor
 * Loads and runs the GranularFreeze WASM module as an audio effect (input -> output).
 *
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    ...
    let createModule;
    try {
      const wrappedCode = `${jsCode}; return createGranularFreezeModule;`;
      createModule = new Function(wrappedCode)();
    } catch (evalErr) {
      console.error('[GranularFreeze] Failed to evaluate JS:', evalErr);
      sharedModulePromise = null;
      throw new Error('Could not evaluate GranularFreeze module factory');
    }

    if (!createModule) {
      sharedModulePromise = null;
      throw new Error('Could not load GranularFreeze module factory');
    }
...
class GranularFreezeProcessor extends AudioWorkletProcessor {
  ...
  async initialize(data) {
    try {
      ...
      this.effect = new Module.GranularFreezeEffect();
      ...
      console.error('[GranularFreeze] initialization error:', error);
