/**
 * Maximizer.worklet.js — AudioWorklet processor for Maximizer WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createMaximizer;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createMaximizer factory');
...
class MaximizerProcessor extends AudioWorkletProcessor {
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'ceiling': M['_maximizer_set_ceiling'](h, value); break;
      case 'release': M['_maximizer_set_release'](h, value); break;
      case 'mix': M['_maximizer_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_maximizer_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_maximizer_create'](sampleRate);
      if (this.handle < 0) throw new Error('maximizer_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Maximizer] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_maximizer_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('maximizer-processor', MaximizerProcessor); processorRegistered = true; }
