 ...
 *          pointer.
 */
int ben_unpack(const struct bencode* b, const char* fmt, ...) BEN_CHECK_FORMAT(scanf, 2, 3);
 ...
 *          preceeding conversion modifiers define the type of the value.
 */
struct bencode* ben_pack(const char* fmt, ...) BEN_CHECK_FORMAT(printf, 1, 2);
...
#define ben_list_for_each(value, pos, l)                                                                              \
    for ((pos) = (size_t)0;                                                                                           \
         (pos) < (ben_list_const_cast(l))->n && ((value) = ((const struct bencode_list*)(l))->values[(pos)]) != NULL; \
         (pos)++)
...
#define ben_dict_for_each(bkey, bvalue, pos, d)                                                                        \
    for ((pos) = 0;                                                                                                    \
         (pos) < (ben_dict_const_cast(d))->n && ((bkey) = ((const struct bencode_dict*)(d))->nodes[(pos)].key) != NULL \
