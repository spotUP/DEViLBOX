...
#define die(fmt, ...)                                                 \
    do {                                                              \
        fprintf(stderr, "bencode: fatal error: " fmt, ##__VA_ARGS__); \
        abort();                                                      \
    } while (0)
#define warn(fmt, ...)                                            \
    do {                                                          \
        fprintf(stderr, "bencode: warning: " fmt, ##__VA_ARGS__); \
    } while (0)
...
static int unpack(const struct bencode* b, struct ben_decode_ctx* ctx, va_list* vl);
...
struct bencode* ben_decode3(const void* data, size_t len, size_t* off, int* error, struct bencode_type* types[128]) {
    struct ben_decode_ctx ctx = { .data = data, .len = len, .off = *off, .types = types };
    struct bencode* b = ben_ctx_decode(&ctx);
    *off = ctx.off;
    if (error != NULL) {
        assert((b != NULL) ^ (ctx.error != 0));
...
static size_t dict_find_pos(struct bencode_dict* d, const struct bencode* key, long long hash) {
    size_t pos = hash_bucket_head(hash, d);
    while (pos != -1) {
        assert(pos < d->n);
        if (d->nodes[pos].hash == hash && ben_cmp(d->nodes[pos].key, key) == 0)
...
static struct bencode* dict_pop(struct bencode_dict* d, const struct bencode* key, long long hash) {
    struct bencode* value;
...
int ben_dict_set(struct bencode* dict, struct bencode* key, struct bencode* value) {
    ...
    pos = d->n;
    d->nodes[pos] = (struct bencode_dict_node) { .hash = hash, .key = key, .value = value, .next = d->buckets[bucket] };
    d->n++;
    d->buckets[bucket] = pos;
    return 0;
}
...
static int unpack_pointer(const struct bencode* b, struct ben_decode_ctx* ctx, va_list* vl) {
    const char** str;
...
static int unpack_value(const struct bencode* b, struct ben_decode_ctx* ctx, va_list* vl) {
    long long val;
    long long* ll;
    long* l;
    int* i;
...
static int unpack_dict(const struct bencode* b, struct ben_decode_ctx* ctx, va_list* vl) {
    struct bencode* key = NULL;
...
static int unpack_list(const struct bencode* b, struct ben_decode_ctx* ctx, va_list* vl) {
    const struct bencode_list* list;
    size_t i = 0;
...
static int unpack_all(const struct bencode* b, struct ben_decode_ctx* ctx, va_list* vl) {
    if (unpack(b, ctx, vl))
        return -1;
