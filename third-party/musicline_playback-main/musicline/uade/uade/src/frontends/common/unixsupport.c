			 ...
			 const char *playerdir, const char *moduledir)
{
	...
	ptr = copy;
	// using S: drive with Windows not supported, tough luck
	if (strcasestr(ptr, "ENV:") || strcasestr(ptr, "S:") || strcasestr(ptr, "Instruments:")) {
		...
		// first try find directly with given path
		file = fopen(aname, "rb");
		if (file) {
			fclose(file);
			strlcpy(realname, aname, maxlen);
			return 0;
		}
...
#else
		const char *colon = strchr(ptr, (int) ':');
		if (colon) {
			if (colon == ptr + 1 && strchr(ptr, (int) '/') == colon + 1) {
				// absolute MS-DOS path C:/...
				snprintf(dirname, 4, "%s", ptr);
				ptr += 3;
			} else {
				// absolute Amiga path dh0:... or just :...
				snprintf(dirname, colon - ptr + 2, "%s", ptr);
				ptr += colon - ptr + 1;
			}
		} else {
			// relative Amiga path
