...
#else // DISABLE_BENCODETOOLS
#include <stddef.h>
struct bencode;
int uade_is_rmc(const char *buf, size_t size) { return 0; }
struct uade_file *uade_rmc_get_file(const struct bencode *rmc, const char *name) { return NULL; }
double uade_rmc_get_song_length(const struct bencode *rmc) { return 0; }
struct bencode *uade_rmc_decode(const void *data, size_t size) { return NULL; }
int uade_rmc_get_module(struct uade_file **module, const struct bencode *rmc) { return 1; }
#endif
