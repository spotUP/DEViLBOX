...
static void CIA_update(void)
{
    ...
    if (bovfla) {
	/* Record per-tick sample count, DMA offsets, and pair buffer snapshots before player runs */
	ml_record_tick_samples();
	ml_record_tick_dma_offsets();
	ml_record_tick_pair_bufs();
