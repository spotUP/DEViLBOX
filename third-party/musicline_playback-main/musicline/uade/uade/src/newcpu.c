...
/* Debug printf trap handler for address $f0ff14.
 * Called via JSR from the assembly printf macro.
 * Stack layout on entry:
 *   SP+0: return address (pushed by JSR)
 *   SP+4: format string pointer (pushed by pea)
 *   SP+8: arg1, SP+12: arg2, etc.
 * Supports: %d, %x, %s, %%
 * Size prefixes: l=32-bit, h=16-bit, b=8-bit
 * e.g. %lx=00f0ff14, %hx=ff14, %bx=14 */
#define PRINTF_TRAP_ADDR 0xf0ff14
#define CHPLAY_TRAP_ADDR 0xf0ff18

/* ch_Play state trap handler for address $f0ff18.
 * Called via JSR from Play8channels in the Musicline ASM player.
 * Stack layout on entry:
 *   SP+0: return address (pushed by JSR)
 *   SP+4: ch1 (longword, only low byte used)
 *   SP+8: ch2 ... SP+32: ch8 */
static void handle_chplay_trap(void)
{
    uae_u32 sp = m68k_areg(regs, 7);
    uae_u32 ret_addr = get_long(sp);
    int vals[8];
    for (int i = 0; i < 8; i++)
        vals[i] = (int)get_long(sp + 4 + i * 4);
    ml_record_chplay(vals);
    m68k_areg(regs, 7) = sp + 4;
    m68k_setpc(ret_addr);
    fill_prefetch_0();
}

static void handle_printf_trap(void)
{
    uae_u32 sp = m68k_areg(regs, 7);
    uae_u32 ret_addr = get_long(sp);
    uae_u32 fmt_ptr = get_long(sp + 4);
    int arg_offset = 8; /* first arg at SP+8 */
    char c;

    /* Pop return address from stack */
    m68k_areg(regs, 7) = sp + 4;

    /* Walk format string and print */
    while ((c = (char)get_byte(fmt_ptr++)) != '\0') {
        if (c == '\\') {
            c = (char)get_byte(fmt_ptr++);
            switch (c) {
            case 'n': fputc('\n', stderr); break;
            case 't': fputc('\t', stderr); break;
            case '\\': fputc('\\', stderr); break;
            case '\0': goto done;
            default: fputc('\\', stderr); fputc(c, stderr); break;
            }
            continue;
        }
        if (c == '%') {
            ...
            /* Size prefix: l=32-bit, h=16-bit, b=8-bit (default=32) */
            int size = 4;
            char size_ch = 0;
            if (c == 'l' || c == 'h' || c == 'b') {
                size_ch = c;
            ...
            uae_u32 val = get_long(sp + arg_offset);
            arg_offset += 4;
            switch (c) {
            case 'd':
                if (size == 1)
                    fprintf(stderr, "%d", (int)(uae_s8)val);
                else if (size == 2)
                    fprintf(stderr, "%d", (int)(uae_s16)val);
                else
                    fprintf(stderr, "%d", (int)(uae_s32)val);
                break;
            case 'u':
                if (size == 1)
                    fprintf(stderr, "%u", (unsigned)(val & 0xff));
                else if (size == 2)
                    fprintf(stderr, "%u", (unsigned)(val & 0xffff));
                else
                    fprintf(stderr, "%u", (unsigned int)val);
                break;
            case 'x':
                if (size == 1)
                    fprintf(stderr, "%02x", (unsigned)(val & 0xff));
                else if (size == 2)
                    fprintf(stderr, "%04x", (unsigned)(val & 0xffff));
                else
            ...
            case 'X':
                ...
                break;
            case 's': {
                /* Read string from Amiga memory */
                char sc;
                while ((sc = (char)get_byte(val++)) != '\0')
                    fputc(sc, stderr);
                break;
            }
            default:
                ...
                fputc('%', stderr);
                if (size_ch) fputc(size_ch, stderr);
                fputc(c, stderr);
                arg_offset -= 4; /* didn't consume an arg */
                break;
            }
        } else {
            fputc(c, stderr);
    ...
    /* Set PC to return address (the lea instruction after jsr) */
    m68k_setpc(ret_addr);
    fill_prefetch_0();
}
...
void m68k_run_1 (void)
{
  ...
  while (1) {
    ...
    {
      uaecptr pc = m68k_getpc();
      if (pc == PRINTF_TRAP_ADDR) {
        handle_printf_trap();
        continue;
      }
      if (pc == CHPLAY_TRAP_ADDR) {
        handle_chplay_trap();
        continue;
      }
    }
