...
#ifdef _WIN32
typedef struct _stat debug_stat_t;
#define debug_stat(path, buf) _stat(path, buf)
#else
typedef struct stat debug_stat_t;
#define debug_stat(path, buf) stat(path, buf)
#endif
...
void debug (void) {
  ...
  for (;;) {
	...
	*inptr = '\0';
	...
	if ((st=malloc(sizeof(debug_stat_t)))) if(!debug_stat(name,st)) fsize=st->st_size;
	free(st);
	if (!fsize)
	  return;
