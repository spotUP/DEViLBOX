...
static const int SCORE_MODULE_NAME_ADDR = 0x128;  /* Restored for backwards compat */
...
static const int MODULE_NAME_ADDR = 0x00400;  /* 1024 bytes reserved - like system UADE */

/* Execbase top for path arrays moved to after MODULE_NAME_ADDR */
...
void uadecore_reset(void)
{
 ...
 nextsong:
  ...
  /* Store full module path at MODULE_NAME_ADDR (like system UADE) */
  if (!valid_address(MODULE_NAME_ADDR, strlen(modulepath) + 1)) {
	  fprintf(stderr, "uadecore: Invalid address for modulepath.\n");
	  goto skiptonextsong;
  }
  strlcpy((char *) get_real_address(MODULE_NAME_ADDR), modulepath, 1024);
