...
/*
 * WASM override: resolve files directly from MEMFS.
 *
 * In native UADE, this function sends a REQUEST_AMIGA_FILE message to the
 * frontend over IPC, and the frontend resolves the file and sends it back.
 * In the single-threaded WASM build, this causes a deadlock: the core sends
 * the request and blocks waiting for a response, but the frontend can't
 * process the request because it's higher up in the call stack waiting for
 * the core to return.
 *
 * Fix: bypass IPC entirely. Resolve the Amiga filename to a MEMFS path
 * using uade_find_amiga_file(), then load the file directly.
 */
#include <limits.h>
extern int uade_find_amiga_file(char *realname, size_t maxlen,
				const char *aname, const char *playerdir);
extern struct uade_file *uade_file_load(const char *name);
...
struct uade_file *uade_request_amiga_file(const char *name, struct uade_ipc *ipc)
{
	...
	/* Verbose resolving/loaded logging removed — was flooding browser console */
	...
	fprintf(stderr, "[uade-wasm] uade_request_amiga_file: file not found '%s'\n", name);
	/* Return a file struct with NULL data — signals "file not found" to caller.
	 * Returning NULL would cause the caller (uade.c) to call exit(1). */
	{
