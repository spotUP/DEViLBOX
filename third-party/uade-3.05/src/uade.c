...
/* Increased stack areas for 1MB chipmem configuration.
 * Original values (0x8500/0x8F00) gave only ~5.7KB user stack and 2.5KB
 * supervisor stack — too tight for complex eagleplayers (JeroenTel, WallyBeben,
 * etc.) that make deep Amiga OS calls during init.
 * New layout (with chipmem_size=16 = 1MB = 0x100000):
 *   Score binary:    0x01000 - ~0x07000  (~24KB)
 *   User stack:      0x10000, grows down  (~37KB free below supervisor guard)
 *   Supervisor stack: 0x18000, grows down (32KB between user and super guard)
 *   Player binary:   0x20000 onwards
 *   Module binary:   above player + reloc
 */
static const int USER_STACK_ADDR   = 0x10000;
static const int SUPER_STACK_ADDR  = 0x18000;
static const int PLAYER_ADDR  = 0x20000;
...
#ifdef UADE_WASM
		/* In WASM, we can't block waiting for the frontend's next READ
		 * command because everything runs in a single thread. Set a
		 * yield flag so m68k_run_1() returns control to the shim,
		 * which returns audio data to the frontend. The frontend then
		 * sends READ + TOKEN, and the shim calls uadecore_handle_r_state()
		 * on the next cycle. */
		{
			extern volatile int uadecore_wasm_yield;
			uadecore_wasm_yield = 1;
		}
#else
