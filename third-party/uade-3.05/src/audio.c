...
#ifdef UADE_WASM
    /* After yield is set and read_size reaches 0, a few more 68k instructions
     * may execute before m68k_run_1() checks the yield flag. Discard any
     * extra samples produced during this window. */
...
#ifdef UADE_WASM
    uade_wasm_log_paula_write((uint8_t)nr, PAULA_REG_DAT, v);
#endif
    update_audio ();
...
#ifdef UADE_WASM
    uade_wasm_log_paula_write((uint8_t)nr, PAULA_REG_LCH, v);
#endif
    update_audio ();
...
#ifdef UADE_WASM
    uade_wasm_log_paula_write((uint8_t)nr, PAULA_REG_LCL, v);
#endif
    update_audio ();
...
#ifdef UADE_WASM
    uade_wasm_log_paula_write((uint8_t)nr, PAULA_REG_PER, v);
#endif
    update_audio ();
...
#ifdef UADE_WASM
    uade_wasm_log_paula_write((uint8_t)nr, PAULA_REG_LEN, v);
#endif
    update_audio ();
...
#ifdef UADE_WASM
    uade_wasm_log_paula_write((uint8_t)nr, PAULA_REG_VOL, (uint16_t)v2);
#endif
    update_audio ();
