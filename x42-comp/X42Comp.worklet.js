/**
 * X42Comp.worklet.js — AudioWorklet processor for X42Comp WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createX42Comp;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createX42Comp factory');
...
class X42CompProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  async handleMessage(event) {
    ...
    switch (type) {
      ...
      case 'parameter':
        if (this.isInitialized && this.handle >= 0) this.Module['_x42_comp_set_' + data.param](this.handle, data.value);
  ...
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._x42_comp_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._x42_comp_create(sampleRate);
      if (this.handle < 0) throw new Error('x42_comp_create failed');
      const bs = 128 * 4;
      ...
      console.error('[X42Comp] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._x42_comp_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('x42-comp-processor', X42CompProcessor); processorRegistered = true; }
