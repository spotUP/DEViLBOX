/*
 * SurgeUIBridge.cpp — WASM framebuffer bridge for Surge XT UI
 * Follows the proven OBXf/Dexed/Helm pattern: JUCE editor → software framebuffer → WASM exports.
 */
...
static SurgeSynthProcessor* g_processor = nullptr;
static juce::AudioProcessorEditor* g_editor = nullptr;
static juce::Image* g_framebuffer = nullptr;
static int g_fbWidth  = 913;
static int g_fbHeight = 569;
...
public:
    ...
    {
        EM_ASM({
            if (typeof window !== 'undefined' && window._surgeUIParamCallback)
                window._surgeUIParamCallback($0, $1);
...
static void surge_ui_init_common(float scale)
{
    ...
    g_processor = new SurgeSynthProcessor();

    // Prepare the processor with a dummy audio configuration
    g_processor->prepareToPlay(44100.0, 512);
    ...
    if (g_editor)
    {
        ...
        if (nativeW <= 0) nativeW = 913;
        if (nativeH <= 0) nativeH = 569;
    ...
    EM_ASM({ console.log("[SurgeUI WASM] Init: " + $0 + "x" + $1 + " scale=" + $2); },
           g_fbWidth, g_fbHeight, (double)g_scale);
}

EMSCRIPTEN_KEEPALIVE void surge_ui_init() { surge_ui_init_common(1.0f); }
EMSCRIPTEN_KEEPALIVE void surge_ui_init_scaled(float scale) { surge_ui_init_common(scale); }
...
EMSCRIPTEN_KEEPALIVE uint8_t* surge_ui_get_fb()
{
...
EMSCRIPTEN_KEEPALIVE int surge_ui_get_width()  { return g_fbWidth; }
EMSCRIPTEN_KEEPALIVE int surge_ui_get_height() { return g_fbHeight; }
...
EMSCRIPTEN_KEEPALIVE void surge_ui_on_mouse_down(int x, int y, int mods)
{
    if (!g_editor) return;
...
EMSCRIPTEN_KEEPALIVE void surge_ui_on_mouse_up(int x, int y, int mods)
{
    if (!g_editor) return;
    auto* peer = g_editor->getPeer();
    if (!peer) return;
...
EMSCRIPTEN_KEEPALIVE void surge_ui_on_mouse_move(int x, int y, int mods)
{
    if (!g_editor) return;
    auto* peer = g_editor->getPeer();
    if (!peer) return;
...
EMSCRIPTEN_KEEPALIVE void surge_ui_on_mouse_wheel(int x, int y, float dx, float dy)
{
    if (!g_editor) return;
    auto* peer = g_editor->getPeer();
    if (!peer) return;
...
// Parameter access
EMSCRIPTEN_KEEPALIVE void surge_ui_set_param(int idx, float val)
{
...
EMSCRIPTEN_KEEPALIVE float surge_ui_get_param(int idx)
{
...
EMSCRIPTEN_KEEPALIVE int surge_ui_get_param_count()
{
    if (!g_processor) return 0;
    return (int)g_processor->getParameters().size();
}

// Programs / presets
EMSCRIPTEN_KEEPALIVE int surge_ui_get_program() { return g_processor ? g_processor->getCurrentProgram() : 0; }
EMSCRIPTEN_KEEPALIVE void surge_ui_set_program(int p) {
    if (g_processor) {
        g_processor->setCurrentProgram(p);
        if (g_editor) g_editor->repaint();
    }
}
EMSCRIPTEN_KEEPALIVE int surge_ui_get_program_count() { return g_processor ? g_processor->getNumPrograms() : 0; }

static char g_program_name_buf[256];
EMSCRIPTEN_KEEPALIVE const char* surge_ui_get_program_name(int p) {
    if (!g_processor) return "";
...
EMSCRIPTEN_KEEPALIVE void surge_ui_shutdown()
{
