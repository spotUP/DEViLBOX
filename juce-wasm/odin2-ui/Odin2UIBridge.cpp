/*
 * Odin2UIBridge.cpp — WASM framebuffer bridge for Odin2 UI
 * Follows the proven OBXf/Dexed UI bridge pattern.
 */
...
// Odin2 base: 200×154, default scale 6 = 150% → 1200×924
// We use scale 4 (100%) = 800×616 for a reasonable WASM framebuffer
constexpr int FB_WIDTH  = 1200;
constexpr int FB_HEIGHT = 924;

static float g_scale = 1.0f;
static OdinAudioProcessor* g_processor = nullptr;
...
public:
    ...
    {
        EM_ASM({
            if (typeof window !== 'undefined' && window._odin2UIParamCallback)
                window._odin2UIParamCallback($0, $1);
...
static void odin2_ui_init_common(float scale)
{
    ...
    g_processor = new OdinAudioProcessor();
    ...
    // Emit parameter ID array to JS
    EM_ASM({ window._odin2ParamIds = []; });
    for (int i = 0; i < params.size(); i++)
    {
        if (auto* pwid = dynamic_cast<juce::AudioProcessorParameterWithID*>(params[i]))
        {
            EM_ASM({ window._odin2ParamIds.push(UTF8ToString($0)); },
                   pwid->paramID.toRawUTF8());
        }
        else
        {
            EM_ASM({ window._odin2ParamIds.push(""); });
        }
    }
    ...
    EM_ASM({ console.log("[Odin2UI WASM] Init complete: " + $0 + "x" + $1 + " (scale=" + $2 + ")"); },
...
void odin2_ui_init()
{
    odin2_ui_init_common(1.0f);
}

EMSCRIPTEN_KEEPALIVE
void odin2_ui_init_scaled(float scale)
{
    odin2_ui_init_common(scale);
}
...
EMSCRIPTEN_KEEPALIVE
int odin2_ui_get_width()  { return g_fbWidth; }

EMSCRIPTEN_KEEPALIVE
int odin2_ui_get_height() { return g_fbHeight; }
...
EMSCRIPTEN_KEEPALIVE
void odin2_ui_on_mouse_down(int x, int y, int mods)
{
    if (!g_editor) return;
...
EMSCRIPTEN_KEEPALIVE
void odin2_ui_on_mouse_up(int x, int y, int mods)
{
    if (!g_editor) return;
    auto* peer = g_editor->getPeer();
    if (!peer) return;
...
EMSCRIPTEN_KEEPALIVE
void odin2_ui_on_mouse_move(int x, int y, int mods)
{
    if (!g_editor) return;
    auto* peer = g_editor->getPeer();
    if (!peer) return;
...
EMSCRIPTEN_KEEPALIVE
void odin2_ui_on_mouse_wheel(int x, int y, float deltaX, float deltaY)
{
    if (!g_editor) return;
    auto* peer = g_editor->getPeer();
    if (!peer) return;
...
EMSCRIPTEN_KEEPALIVE
void odin2_ui_set_param(int index, float value)
{
...
EMSCRIPTEN_KEEPALIVE
float odin2_ui_get_param(int index)
{
...
EMSCRIPTEN_KEEPALIVE
int odin2_ui_get_program() { return g_processor ? g_processor->getCurrentProgram() : 0; }

EMSCRIPTEN_KEEPALIVE
void odin2_ui_set_program(int p) { if (g_processor) g_processor->setCurrentProgram(p); }

EMSCRIPTEN_KEEPALIVE
int odin2_ui_get_program_count() { return g_processor ? g_processor->getNumPrograms() : 0; }

// ── Cleanup ──

EMSCRIPTEN_KEEPALIVE
void odin2_ui_shutdown()
{
