...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try { const wrappedCode = `${jsCode}; return createAutoSat;`; createModule = new Function(wrappedCode)(); }
    catch (e) { console.error('[AutoSat] Failed:', e); sharedModulePromise = null; throw e; }
...
class AutoSatProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParameter(param, value) {
    ...
    const fn = M['_autosat_set_' + param];
    if (fn) fn(h, value);
  }
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._autosat_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = Module._autosat_create(sampleRate);
      ...
      console.error('[AutoSat] init error:', error);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._autosat_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('autosat-processor', AutoSatProcessor); processorRegistered = true; }
