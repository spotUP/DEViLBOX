/**
 * SidechainLimiter.worklet.js — AudioWorklet processor for SidechainLimiter WASM effect.
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createSidechainLimiter;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createSidechainLimiter factory');
...
class SidechainLimiterProcessor extends AudioWorkletProcessor {
  ...
  async handleMessage(event) {
    ...
    switch (type) {
      ...
      case 'parameter':
        if (this.isInitialized && this.handle >= 0) this.Module['_sidechain_limiter_set_' + data.param](this.handle, data.value);
  ...
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._sidechain_limiter_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._sidechain_limiter_create(sampleRate);
      if (this.handle < 0) throw new Error('sidechain_limiter_create failed');
      const bs = 128 * 4;
      ...
      console.error('[SidechainLimiter] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      if (hasExtSc) {
        ...
        this.Module._sidechain_limiter_process_ext(this.handle, this.inputPtrL, this.inputPtrR, this.scPtrL, this.scPtrR, this.outputPtrL, this.outputPtrR, n);
      } else {
        this.Module._sidechain_limiter_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
      }
...
if (!processorRegistered) { registerProcessor('sidechain-limiter-processor', SidechainLimiterProcessor); processorRegistered = true; }
