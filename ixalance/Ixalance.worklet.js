/**
 * Ixalance.worklet.js - AudioWorklet processor for Ixalance (IXS) WASM synth
 *
 * The WASM ixalance_render() writes interleaved stereo (LRLRLR...) into a
 * single float buffer. We deinterleave into the WebAudio output channels.
 */

class IxalanceProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
  ...
  async handleMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'loadModule':
        if (this.module && typeof this.module._ixalance_init === 'function') {
          try {
            ...
            const result = this.module._ixalance_init(wasmPtr, uint8Data.length);
            const free = this.module._free || this.module.free;
            if (free) free(wasmPtr);

            if (result !== 0) {
              this.port.postMessage({ type: 'error', message: 'ixalance_init failed with code ' + result });
              return;
            }
      ...
      case 'stop':
        if (this.module && typeof this.module._ixalance_stop === 'function') {
          this.module._ixalance_stop();
  ...
  async initModule(sampleRate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.Ixalance) {
        ...
        const wrappedCode = jsCode + '\nreturn createIxalance;';
        const factory = new Function(wrappedCode);
        const result = factory();

        if (typeof result === 'function') {
          globalThis.Ixalance = result;
        } else {
      ...
      if (typeof globalThis.Ixalance !== 'function') {
        this.port.postMessage({ type: 'error', message: 'Ixalance factory not available' });
        return;
      }
      ...
      try {
        this.module = await globalThis.Ixalance(config);
      } finally {
  ...
  process(inputs, outputs, parameters) {
    ...
    if (typeof this.module._ixalance_render === 'function') {
      this.updateBufferViews();
      if (this.interleavedBuf) {
        const rendered = this.module._ixalance_render(this.interleavedPtr, numSamples);
        if (rendered > 0) {
...
registerProcessor('ixalance-processor', IxalanceProcessor);
