...
#ifdef FURNACE_WASM_INSED
    // Expose private methods for WASM bridge
    void wasmDrawInsEdit() {
      // Force insEdit window to fill the full viewport
      ImGuiIO& io = ImGui::GetIO();
      ImGui::SetNextWindowPos(ImVec2(0, 0));
      ImGui::SetNextWindowSize(ImVec2(io.DisplaySize.x, io.DisplaySize.y));
      drawInsEdit();
    }
    void wasmSetCurIns(int ins) { curIns = ins; }
    void wasmSetInsEditOpen(bool open) { insEditOpen = open; }
    void wasmSetDpiScale(float s) { dpiScale = s; }
    void wasmSetCanvasSize(int w, int h) { canvasW = w; canvasH = h; scrW = w; scrH = h; }
#endif
};
