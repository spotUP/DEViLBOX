/**
 * Limiter.worklet.js — AudioWorklet processor for Limiter WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createLimiter;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createLimiter factory');
...
class LimiterProcessor extends AudioWorkletProcessor {
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'threshold': M._limiter_set_threshold(h, value); break;
      case 'ceiling':   M._limiter_set_ceiling(h, value);   break;
      case 'attack':    M._limiter_set_attack(h, value);    break;
      case 'release':   M._limiter_set_release(h, value);   break;
      case 'lookahead': M._limiter_set_lookahead(h, value); break;
      case 'knee':      M._limiter_set_knee(h, value);      break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._limiter_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._limiter_create(sampleRate);
      if (this.handle < 0) throw new Error('limiter_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Limiter] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._limiter_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('limiter-processor', LimiterProcessor); processorRegistered = true; }
