/**
 * DynamicsProcWASM.cpp — Full dynamics processor: expander + compressor.
 * Below lowerThresh: expand (make quieter). Above upperThresh: compress.
 * Between thresholds: pass through unchanged.
 */
#include <cmath>
#include <cstring>
#include <algorithm>
...
struct Instance {
    ...
    float lowerThresh = -40.0f; // dB (-60..0)
    float upperThresh = -12.0f; // dB (-30..0)
    ...
    float computeGainDb(float inputDb) const {
        if (inputDb < lowerThresh) {
            // Expander: below lower threshold, expand with ratio
            float diff = inputDb - lowerThresh;
            return diff * (ratio - 1.0f); // Negative gain = make quieter
        } else if (inputDb > upperThresh) {
            // Compressor: above upper threshold
            float diff = inputDb - upperThresh;
            return diff * (1.0f / ratio - 1.0f);
        }
        return 0.0f; // Between thresholds: unity
    }
...
EMSCRIPTEN_KEEPALIVE int dynamics_proc_create(int sr) {
    int s = findFree(); if (s < 0) return -1;
    instances[s].active = true; instances[s].init(static_cast<float>(sr)); return s;
}
EMSCRIPTEN_KEEPALIVE void dynamics_proc_destroy(int h) {
    if (h >= 0 && h < MAX_INSTANCES) instances[h].active = false;
}
EMSCRIPTEN_KEEPALIVE void dynamics_proc_process(int h, float* iL, float* iR, float* oL, float* oR, int n) {
...
EMSCRIPTEN_KEEPALIVE void dynamics_proc_set_lowerThresh(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) instances[h].lowerThresh = std::clamp(v, -60.0f, 0.0f);
}
EMSCRIPTEN_KEEPALIVE void dynamics_proc_set_upperThresh(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) instances[h].upperThresh = std::clamp(v, -30.0f, 0.0f);
}
EMSCRIPTEN_KEEPALIVE void dynamics_proc_set_ratio(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) instances[h].ratio = std::clamp(v, 1.0f, 20.0f);
}
EMSCRIPTEN_KEEPALIVE void dynamics_proc_set_attack(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) { instances[h].attack = std::clamp(v, 0.1f, 100.0f); instances[h].updateCoeffs(); }
}
EMSCRIPTEN_KEEPALIVE void dynamics_proc_set_release(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) { instances[h].release = std::clamp(v, 10.0f, 1000.0f); instances[h].updateCoeffs(); }
}
EMSCRIPTEN_KEEPALIVE void dynamics_proc_set_makeup(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) instances[h].makeup = std::clamp(v, 0.0f, 24.0f);
}
EMSCRIPTEN_KEEPALIVE void dynamics_proc_set_mix(int h, float v) {
