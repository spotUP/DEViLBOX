...
// DEViLBOX extension: trigger a macro on a chosen voice for editor preview.
//
// Sets up the same `cmd` struct that the sequencer's pattern parser would
// build for a "note" command, then delegates to the regular noteCmd() path.
// noteCmd() looks up the voice via channelToVoiceMap[cmd.cd & 7] and writes
// to that voice's macro pointer + note + detune state, exactly as if a
// pattern row had played the macro. The next sequencer tick processes the
// macro normally and Paula starts producing output for that voice.
//
// The voice the user picks may be one the running song is also using; in
// that case the song's next pattern step will overwrite the preview note
// after a few ticks, which is exactly what you want — the preview "lands"
// briefly and then the song continues. Pick an unused voice for a longer
// audition, or stop playback first.
void TFMXDecoder::previewMacro(int macroIdx, int note, int volume, int channel) {
    cmd.aa = (ubyte)(note & 0x3f);          // note 0..63
    cmd.bb = (ubyte)(macroIdx & 0x7f);      // macro index, mask off top bit
    cmd.cd = (ubyte)(((volume & 0x0f) << 4) // high nibble: relative volume
                    | (channel & 0x07));    // low nibble: channel index
    cmd.ee = 0;                             // no detune
    noteCmd();
}
