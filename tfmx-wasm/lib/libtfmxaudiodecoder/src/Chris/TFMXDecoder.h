...
class TFMXDecoder : public Decoder {
 public:
    ...
    /**
     * DEViLBOX extension: trigger a single macro on a free voice for editor
     * preview/audition purposes. Mirrors the work done by the noteCmd path
     * inside the regular sequencer (Macro.cpp:525 macroFunc_PlayMacro), but
     * exposed publicly so a host can audition any instrument macro outside
     * of the song's normal flow.
     *
     * Parameters:
     *   macroIdx — instrument macro index (0..127), masked to 7 bits as the
     *              sequencer would (cmd.bb & 0x7f).
     *   note     — TFMX note value 0..63 (matches noteCmd's cmd.aa range).
     *   volume   — 0..15, mapped to the high nibble of cmd.cd.
     *   channel  — 0..numVoices-1, mapped to the low nibble of cmd.cd via
     *              channelToVoiceMap (same as the sequencer's voice picker).
     */
    void previewMacro(int macroIdx, int note, int volume, int channel) override;
