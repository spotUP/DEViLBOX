...
extern "C" {
    ...
    /* DEViLBOX extension: trigger an instrument macro on a chosen voice for
     * editor preview/audition. The macro is set up exactly as if the
     * sequencer had played a "note" command at that pattern row, then
     * processed by the regular tick loop on subsequent buffer fills.
     *
     *   macroIdx: 0..127  instrument macro index
     *   note:     0..63   TFMX note value
     *   volume:   0..15   relative volume (high nibble of cmd.cd)
     *   channel:  0..7    voice index (mapped via channelToVoiceMap)
     *
     * No-op for non-TFMX (Hippel etc.) decoders.
     */
