/**
 * InStereo1Engine.ts — Singleton WASM engine wrapper for InStereo1 replayer
...
export class InStereo1Engine {
  private static instance: InStereo1Engine | null = null;
  ...
  static getInstance(): InStereo1Engine {
    if (InStereo1Engine.instance && !InStereo1Engine.instance._disposed) {
      try {
        const currentCtx = getDevilboxAudioContext();
        if (InStereo1Engine.instance.audioContext !== currentCtx) {
          InStereo1Engine.instance.dispose();
        }
      } catch { /* context not yet set */ }
    }
    if (!InStereo1Engine.instance || InStereo1Engine.instance._disposed) {
      InStereo1Engine.instance = new InStereo1Engine();
    }
    return InStereo1Engine.instance;
  }

  static hasInstance(): boolean {
    return !!InStereo1Engine.instance && !InStereo1Engine.instance._disposed;
  }

  private async initialize(): Promise<void> {
    try {
      await InStereo1Engine.ensureInitialized(this.audioContext);
      this.createNode();
    } catch (err) {
      console.error('[InStereo1Engine] Initialization failed:', err);
    }
  }
  ...
  private static async ensureInitialized(context: AudioContext): Promise<void> {
    ...
    const initPromise = (async () => {
      ...
      try { await context.audioWorklet.addModule(`${baseUrl}instereo1/InStereo1.worklet.js?v=${Date.now()}`); } catch { /* already registered */ }

      if (!this.wasmBinary || !this.jsCode) {
        const [wasmResponse, jsResponse] = await Promise.all([
          fetch(`${baseUrl}instereo1/InStereo1.wasm`),
          fetch(`${baseUrl}instereo1/InStereo1.js`),
        ]);
  ...
  private createNode(): void {
    ...
    this.workletNode = new AudioWorkletNode(ctx, 'instereo1-processor', {
    ...
    this.workletNode.port.onmessage = (event) => {
      ...
      switch (data.type) {
        case 'ready':
          console.log('[InStereo1Engine] WASM ready');
          if (this._resolveInit) { this._resolveInit(); this._resolveInit = null; }
          break;
        case 'moduleLoaded':
          useOscilloscopeStore.getState().setChipInfo(4, 0, ['Paula 0', 'Paula 1', 'Paula 2', 'Paula 3']);
          console.log('[InStereo1Engine] Module loaded, subsongs:', data.subsongCount);
          break;
        case 'oscData':
        ...
        case 'songEnd':
          ...
          break;
        case 'error':
          console.error('[InStereo1Engine]', data.message);
          break;
      }
    };
    ...
    this.workletNode.port.postMessage({
      ...
      wasmBinary: InStereo1Engine.wasmBinary, jsCode: InStereo1Engine.jsCode,
    });
  ...
  async loadTune(buffer: ArrayBuffer): Promise<void> {
    ...
    if (!this.workletNode) throw new Error('InStereo1Engine not initialized');
  ...
  /** Edit a pattern cell in the WASM replayer */  setCell(index: number, row: number, channel: number, note: number, instrument: number, effect: number, effectArg: number): void {    this.workletNode?.port.postMessage({ type: 'setCell', index, row, channel, note, instrument, effect, effectArg });  }  /** Set an instrument parameter by name */  setInstrumentParam(instrument: number, param: string, value: number): void {    this.workletNode?.port.postMessage({ type: 'setInstrumentParam', instrument, param, value });  }  dispose(): void {
    ...
    if (InStereo1Engine.instance === this) InStereo1Engine.instance = null;
  }
}
