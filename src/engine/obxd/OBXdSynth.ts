 ...
 * Generated by capturing state from the running WASM engine after
 * loading each flat preset and letting the C++ apply defaults.
 *
 * To capture new presets:
 *   1. Load synth in browser
 *   2. Call synth.getState() via MCP or console
 *   3. Add result here as { name, values }
 *
 * Empty until first capture — falls back to flat presets.
 */
