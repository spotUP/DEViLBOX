...
export class IxalanceEngine {
  ...
  private async initialize(): Promise<void> {
    try {
      await IxalanceEngine.ensureInitialized(this.audioContext);
      this.createNode();
    } catch (err) {
      console.error('[IxalanceEngine] Initialization failed:', err);
    }
  }
  ...
  private static async ensureInitialized(context: AudioContext): Promise<void> {
    ...
    const initPromise = (async () => {
      ...
      try {
        await context.audioWorklet.addModule(`${baseUrl}ixalance/Ixalance.worklet.js`);
      } catch {
      ...
      if (!this.wasmBinary || !this.jsCode) {
        const [wasmResponse, jsResponse] = await Promise.all([
          fetch(`${baseUrl}ixalance/Ixalance.wasm`),
          fetch(`${baseUrl}ixalance/Ixalance.js`),
        ]);
  ...
  private createNode(): void {
    ...
    this.workletNode.port.postMessage({
      ...
      wasmBinary: IxalanceEngine.wasmBinary,
      jsCode: IxalanceEngine.jsCode,
    });
