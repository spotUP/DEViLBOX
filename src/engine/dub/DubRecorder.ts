...
export function startDubRecorder(): () => void {
  const unsubFire = subscribeDubRouter((fireEvent) => {
    ...
    scheduleDubStoreSync(() => {
      ...
      // Write cell effect for per-channel TRIGGER moves so they're visible
      // inline in the pattern editor, not just in the automation lanes overlay.
      // Conditions:
      //   - Move has a channel (per-channel slot encoding)
      //   - Kind is 'trigger' (holds need duration which a single cell can't express)
      ...
      // Write automation step curve for ALL move types so automation lanes
      // show a visual representation of dub activity.
