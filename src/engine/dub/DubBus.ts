...
export class DubBus {
  ...
  private _swapEchoEngine(settings: DubBusSettings, deferredPreset?: Exclude<DubBusSettings['characterPreset'], 'custom'>): void {
    ...
    setTimeout(() => {
      try {
        ...
        this.sidechain.ratio.setTargetAtTime(10, now2 + WARMUP_SEC, 0.02);
        ...
        try {
          ...
          this.sidechain.ratio.setValueAtTime(10, t);
  ...
  constructor(context: AudioContext, master: AudioNode) {
    ...
    // Threshold and ratio tuned so a single effect tail barely clips the knee
    // but stacked tails (two wet moves overlapping) get clamped hard. The glue
    // compressor downstream handles the slow "lathe" texture; this one is the
    // fast guard against peak energy when effects pile up.
    this.sidechain.threshold.value = -22;
    this.sidechain.ratio.value = 10;
    this.sidechain.attack.value = 0.001;
    this.sidechain.release.value = 0.18;
    this.sidechain.knee.value = 4;
  ...
  private _warmupMute(sidechainAmount = 0.15): void {
    ...
    try {
      ...
      this.sidechain.ratio.cancelScheduledValues(now);
      this.sidechain.ratio.setValueAtTime(1, now);
      this.sidechain.ratio.setTargetAtTime(10, now + WARMUP_SEC, 0.02);
      this.glue.ratio.cancelScheduledValues(now);
      this.glue.ratio.setValueAtTime(1, now);
