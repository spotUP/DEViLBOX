...
export class SoundFxEngine {
  ...
  private async initialize(): Promise<void> {
    try {
      await SoundFxEngine.ensureInitialized(this.audioContext);
      this.createNode();
    } catch (err) {
      console.error('[SoundFxEngine] Initialization failed:', err);
    }
  }
  ...
  private static async ensureInitialized(context: AudioContext): Promise<void> {
    ...
    const initPromise = (async () => {
      ...
      try { await context.audioWorklet.addModule(`${baseUrl}soundfx/SoundFx.worklet.js?v=${Date.now()}`); } catch { /* already registered */ }

      if (!this.wasmBinary || !this.jsCode) {
        const [wasmResponse, jsResponse] = await Promise.all([
          fetch(`${baseUrl}soundfx/SoundFx.wasm`),
          fetch(`${baseUrl}soundfx/SoundFx.js`),
        ]);
  ...
  private createNode(): void {
    ...
    this.workletNode.port.postMessage({
      ...
      wasmBinary: SoundFxEngine.wasmBinary, jsCode: SoundFxEngine.jsCode,
    });
