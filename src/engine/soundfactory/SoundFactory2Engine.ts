...
export class SoundFactory2Engine {
  ...
  private async initialize(): Promise<void> {
    try {
      await SoundFactory2Engine.ensureInitialized(this.audioContext);
      this.createNode();
    } catch (err) {
      console.error('[SoundFactory2Engine] Initialization failed:', err);
    }
  }
  ...
  private static async ensureInitialized(context: AudioContext): Promise<void> {
    ...
    const initPromise = (async () => {
      ...
      try { await context.audioWorklet.addModule(`${baseUrl}soundfactory/SoundFactory2.worklet.js?v=${Date.now()}`); } catch { /* already registered */ }

      if (!this.wasmBinary || !this.jsCode) {
        const [wasmResponse, jsResponse] = await Promise.all([
          fetch(`${baseUrl}soundfactory/SoundFactory2.wasm`),
          fetch(`${baseUrl}soundfactory/SoundFactory2.js`),
        ]);
  ...
  private createNode(): void {
    ...
    this.workletNode.port.postMessage({
      ...
      wasmBinary: SoundFactory2Engine.wasmBinary, jsCode: SoundFactory2Engine.jsCode,
    });
