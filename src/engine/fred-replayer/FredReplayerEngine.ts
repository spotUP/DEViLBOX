...
export class FredReplayerEngine {
  ...
  private async initialize(): Promise<void> {
    try {
      await FredReplayerEngine.ensureInitialized(this.audioContext);
      this.createNode();
    } catch (err) {
      console.error('[FredReplayerEngine] Initialization failed:', err);
    }
  }
  ...
  private static async ensureInitialized(context: AudioContext): Promise<void> {
    ...
    const initPromise = (async () => {
      ...
      try { await context.audioWorklet.addModule(`${baseUrl}fred-replayer/FredReplayer.worklet.js?v=${Date.now()}`); } catch { /* already registered */ }

      if (!this.wasmBinary || !this.jsCode) {
        const [wasmResponse, jsResponse] = await Promise.all([
          fetch(`${baseUrl}fred-replayer/FredReplayer.wasm`),
          fetch(`${baseUrl}fred-replayer/FredReplayer.js`),
        ]);
  ...
  private createNode(): void {
    ...
    this.workletNode.port.postMessage({
      ...
      wasmBinary: FredReplayerEngine.wasmBinary, jsCode: FredReplayerEngine.jsCode,
    });
