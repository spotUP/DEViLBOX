...
export class VoodooEngine {
  ...
  private async initialize(): Promise<void> {
    try {
      await VoodooEngine.ensureInitialized(this.audioContext);
      this.createNode();
    } catch (err) {
      console.error('[VoodooEngine] Initialization failed:', err);
    }
  }
  ...
  private static async ensureInitialized(context: AudioContext): Promise<void> {
    ...
    const initPromise = (async () => {
      ...
      try { await context.audioWorklet.addModule(`${baseUrl}voodoo/Voodoo.worklet.js?v=${Date.now()}`); } catch { /* already registered */ }

      if (!this.wasmBinary || !this.jsCode) {
        const [wasmResponse, jsResponse] = await Promise.all([
          fetch(`${baseUrl}voodoo/Voodoo.wasm`),
          fetch(`${baseUrl}voodoo/Voodoo.js`),
        ]);
  ...
  private createNode(): void {
    ...
    this.workletNode = new AudioWorkletNode(ctx, 'voodoo-processor', {
    ...
    this.workletNode.port.postMessage({
      ...
      wasmBinary: VoodooEngine.wasmBinary, jsCode: VoodooEngine.jsCode,
    });
