/**
 * SoundControlEngine.ts — Singleton WASM engine wrapper for SoundControl replayer
...
export class SoundControlEngine {
  private static instance: SoundControlEngine | null = null;
  ...
  static getInstance(): SoundControlEngine {
    if (SoundControlEngine.instance && !SoundControlEngine.instance._disposed) {
      try {
        const currentCtx = getDevilboxAudioContext();
        if (SoundControlEngine.instance.audioContext !== currentCtx) {
          SoundControlEngine.instance.dispose();
        }
      } catch { /* context not yet set */ }
    }
    if (!SoundControlEngine.instance || SoundControlEngine.instance._disposed) {
      SoundControlEngine.instance = new SoundControlEngine();
    }
    return SoundControlEngine.instance;
  }

  static hasInstance(): boolean {
    return !!SoundControlEngine.instance && !SoundControlEngine.instance._disposed;
  }

  private async initialize(): Promise<void> {
    try {
      await SoundControlEngine.ensureInitialized(this.audioContext);
      this.createNode();
    } catch (err) {
      console.error('[SoundControlEngine] Initialization failed:', err);
    }
  }
  ...
  private static async ensureInitialized(context: AudioContext): Promise<void> {
    ...
    const initPromise = (async () => {
      ...
      try { await context.audioWorklet.addModule(`${baseUrl}soundcontrol/SoundControl.worklet.js?v=${Date.now()}`); } catch { /* already registered */ }

      if (!this.wasmBinary || !this.jsCode) {
        const [wasmResponse, jsResponse] = await Promise.all([
          fetch(`${baseUrl}soundcontrol/SoundControl.wasm`),
          fetch(`${baseUrl}soundcontrol/SoundControl.js`),
        ]);
  ...
  private createNode(): void {
    ...
    this.workletNode = new AudioWorkletNode(ctx, 'soundcontrol-processor', {
    ...
    this.workletNode.port.onmessage = (event) => {
      ...
      switch (data.type) {
        case 'ready':
          console.log('[SoundControlEngine] WASM ready');
          if (this._resolveInit) { this._resolveInit(); this._resolveInit = null; }
          break;
        case 'moduleLoaded':
          useOscilloscopeStore.getState().setChipInfo(4, 0, ['Paula 0', 'Paula 1', 'Paula 2', 'Paula 3']);
          console.log('[SoundControlEngine] Module loaded, subsongs:', data.subsongCount);
          break;
        case 'oscData':
        ...
        case 'songEnd':
          ...
          break;
        case 'error':
          console.error('[SoundControlEngine]', data.message);
          break;
      }
    };
    ...
    this.workletNode.port.postMessage({
      ...
      wasmBinary: SoundControlEngine.wasmBinary, jsCode: SoundControlEngine.jsCode,
    });
  ...
  async loadTune(buffer: ArrayBuffer): Promise<void> {
    ...
    if (!this.workletNode) throw new Error('SoundControlEngine not initialized');
  ...
  /** Edit a pattern cell in the WASM replayer */  setCell(index: number, row: number, channel: number, note: number, instrument: number, effect: number, effectArg: number): void {    this.workletNode?.port.postMessage({ type: 'setCell', index, row, channel, note, instrument, effect, effectArg });  }  /** Set an instrument parameter by name */  setInstrumentParam(instrument: number, param: string, value: number): void {    this.workletNode?.port.postMessage({ type: 'setInstrumentParam', instrument, param, value });  }  dispose(): void {
    ...
    if (SoundControlEngine.instance === this) SoundControlEngine.instance = null;
  }
}
