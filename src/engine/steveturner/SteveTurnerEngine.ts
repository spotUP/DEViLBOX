...
export class SteveTurnerEngine {
  ...
  private async initialize(): Promise<void> {
    try {
      await SteveTurnerEngine.ensureInitialized(this.audioContext);
      this.createNode();
    } catch (err) {
      console.error('[SteveTurnerEngine] Initialization failed:', err);
    }
  }
  ...
  private static async ensureInitialized(context: AudioContext): Promise<void> {
    ...
    const initPromise = (async () => {
      ...
      try {
        await context.audioWorklet.addModule(`${baseUrl}steveturner/SteveTurner.worklet.js`);
      } catch {
      ...
      if (!this.wasmBinary || !this.jsCode) {
        const [wasmResponse, jsResponse] = await Promise.all([
          fetch(`${baseUrl}steveturner/SteveTurner.wasm`),
          fetch(`${baseUrl}steveturner/SteveTurner.js`),
        ]);
  ...
  private createNode(): void {
    ...
    this.workletNode.port.postMessage({
      ...
      wasmBinary: SteveTurnerEngine.wasmBinary,
      jsCode: SteveTurnerEngine.jsCode,
    });
