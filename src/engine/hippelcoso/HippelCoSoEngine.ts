...
export class HippelCoSoEngine {
  ...
  private async initialize(): Promise<void> {
    try {
      await HippelCoSoEngine.ensureInitialized(this.audioContext);
      this.createNode();
    } catch (err) {
      console.error('[HippelCoSoEngine] Initialization failed:', err);
    }
  }
  ...
  private static async ensureInitialized(context: AudioContext): Promise<void> {
    ...
    const initPromise = (async () => {
      ...
      try {
        await context.audioWorklet.addModule(`${baseUrl}hippel-coso/HippelCoSo.worklet.js`);
      } catch {
      ...
      if (!this.wasmBinary || !this.jsCode) {
        const [wasmResponse, jsResponse] = await Promise.all([
          fetch(`${baseUrl}hippel-coso/HippelCoSo.wasm`),
          fetch(`${baseUrl}hippel-coso/HippelCoSo.js`),
        ]);
  ...
  private createNode(): void {
    ...
    this.workletNode.port.postMessage({
      ...
      wasmBinary: HippelCoSoEngine.wasmBinary,
      jsCode: HippelCoSoEngine.jsCode,
    });
