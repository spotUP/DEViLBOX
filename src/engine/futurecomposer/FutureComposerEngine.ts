...
export class FutureComposerEngine {
  ...
  private async initialize(): Promise<void> {
    try {
      await FutureComposerEngine.ensureInitialized(this.audioContext);
      this.createNode();
    } catch (err) {
      console.error('[FutureComposerEngine] Initialization failed:', err);
    }
  }
  ...
  private static async ensureInitialized(context: AudioContext): Promise<void> {
    ...
    const initPromise = (async () => {
      ...
      try { await context.audioWorklet.addModule(`${baseUrl}futurecomposer/FutureComposer.worklet.js?v=${Date.now()}`); } catch { /* already registered */ }

      if (!this.wasmBinary || !this.jsCode) {
        const [wasmResponse, jsResponse] = await Promise.all([
          fetch(`${baseUrl}futurecomposer/FutureComposer.wasm`),
          fetch(`${baseUrl}futurecomposer/FutureComposer.js`),
        ]);
  ...
  private createNode(): void {
    ...
    this.workletNode.port.postMessage({
      ...
      wasmBinary: FutureComposerEngine.wasmBinary, jsCode: FutureComposerEngine.jsCode,
    });
