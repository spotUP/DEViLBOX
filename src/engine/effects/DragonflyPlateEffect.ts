// src/engine/effects/DragonflyPlateEffect.ts
...
export interface DragonflyPlateOptions {
  decay?: number;
  damping?: number;
  predelay?: number;
  width?: number;
...
export class DragonflyPlateEffect extends Tone.ToneAudioNode {
  readonly name = 'DragonflyPlate';
  ...
  constructor(options: DragonflyPlateOptions = {}) {
    super();
    this._decay = options.decay ?? 0.7;
    this._damping = options.damping ?? 0.5;
    this._predelay = options.predelay ?? 10;
    this._width = options.width ?? 1;
    this._brightness = options.brightness ?? 0.7;
  ...
  private async _initWorklet(): Promise<void> {
    try {
      ...
      await DragonflyPlateEffect.ensureInitialized(rawCtx);
      this.workletNode = new AudioWorkletNode(rawCtx, 'dragonfly-plate-processor', {
      ...
      this.workletNode.port.postMessage(
        { type: 'init', wasmBinary: DragonflyPlateEffect.wasmBinary!, jsCode: DragonflyPlateEffect.jsCode! },
        [DragonflyPlateEffect.wasmBinary!.slice(0)],
      );
      ...
      console.warn('[DragonflyPlate] Worklet init failed:', err);
    }
  }
  ...
  private static async ensureInitialized(ctx: AudioContext): Promise<void> {
    ...
    const p = (async () => {
      ...
      const [wasmResp, jsResp] = await Promise.all([
        fetch(`${base}dragonfly-plate/DragonflyPlate.wasm`), fetch(`${base}dragonfly-plate/DragonflyPlate.js`),
      ]);
      this.wasmBinary = await wasmResp.arrayBuffer();
      let js = await jsResp.text();
      js = js.replace(/if\s*\(typeof exports\s*===\s*"object".*$/s, '');
      this.jsCode = js;
      await ctx.audioWorklet.addModule(`${base}dragonfly-plate/DragonflyPlate.worklet.js`);
  ...
  setPredelay(v: number): void { this._predelay = clamp(v, 0, 100); this.sendParam('predelay', this._predelay); }
