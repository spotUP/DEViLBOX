/**
 * VihdaEffect.ts — Vihda via WASM AudioWorklet.
 */
import * as Tone from 'tone';
import { getNativeAudioNode } from '@utils/audio-context';

export interface VihdaOptions {
  width?: number;
  invert?: number;
  mix?: number;
  wet?: number;
...
export class VihdaEffect extends Tone.ToneAudioNode {
  readonly name = 'Vihda';
  ...
  constructor(options: VihdaOptions = {}) {
    super();
    this._width = options.width ?? 1.0;
    this._invert = options.invert ?? 0.0;
  ...
  private static async ensureInitialized(ctx: AudioContext): Promise<void> {
    ...
    const p = (async () => {
      ...
      const [wasmResp, jsResp] = await Promise.all([
        fetch(`${base}vihda/Vihda.wasm`), fetch(`${base}vihda/Vihda.js`),
      ]);
  ...
  setWidth(v: number): void { this._width = Math.max(0.0, Math.min(2.0, v)); this.sendParam('width', this._width); }
  setInvert(v: number): void { this._invert = Math.max(0.0, Math.min(1.0, v)); this.sendParam('invert', this._invert); }
  setMix(v: number): void { this._mix = Math.max(0.0, Math.min(1.0, v)); this.sendParam('mix', this._mix); }

  get width(): number { return this._width; }
  get invert(): number { return this._invert; }
  ...
  setParam(param: string, value: number): void {
    switch (param) {
      ...
      case 'invert': this.setInvert(value); break;
