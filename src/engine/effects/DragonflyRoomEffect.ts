// src/engine/effects/DragonflyRoomEffect.ts
...
export interface DragonflyRoomOptions {
  decay?: number;
  damping?: number;
  predelay?: number;
  width?: number;
...
export class DragonflyRoomEffect extends Tone.ToneAudioNode {
  readonly name = 'DragonflyRoom';
  ...
  constructor(options: DragonflyRoomOptions = {}) {
    super();
    this._decay = options.decay ?? 0.4;
    this._damping = options.damping ?? 0.6;
    this._predelay = options.predelay ?? 5;
    this._width = options.width ?? 0.8;
    this._earlyLevel = options.earlyLevel ?? 0.7;
    this._size = options.size ?? 0.7;
  ...
  private async _initWorklet(): Promise<void> {
    try {
      ...
      await DragonflyRoomEffect.ensureInitialized(rawCtx);
      this.workletNode = new AudioWorkletNode(rawCtx, 'dragonfly-room-processor', {
      ...
      this.workletNode.port.onmessage = (ev) => {
        if (ev.data.type === 'ready') {
          ...
          try {
            ...
            console.warn('[DragonflyRoom] WASM swap failed, staying on passthrough:', swapErr);
          }
        }
      };
      this.workletNode.port.postMessage(
        { type: 'init', wasmBinary: DragonflyRoomEffect.wasmBinary!, jsCode: DragonflyRoomEffect.jsCode! },
        [DragonflyRoomEffect.wasmBinary!.slice(0)],
      );
      ...
      console.warn('[DragonflyRoom] Worklet init failed:', err);
    }
  }
  ...
  private static async ensureInitialized(ctx: AudioContext): Promise<void> {
    ...
    const p = (async () => {
      ...
      const [wasmResp, jsResp] = await Promise.all([
        fetch(`${base}dragonfly-room/DragonflyRoom.wasm`), fetch(`${base}dragonfly-room/DragonflyRoom.js`),
      ]);
      this.wasmBinary = await wasmResp.arrayBuffer();
      let js = await jsResp.text();
      js = js.replace(/if\s*\(typeof exports\s*===\s*"object".*$/s, '');
      this.jsCode = js;
      await ctx.audioWorklet.addModule(`${base}dragonfly-room/DragonflyRoom.worklet.js`);
  ...
  setDecay(v: number): void { this._decay = clamp(v, 0, 0.95); this.sendParam('decay', this._decay); }
  setDamping(v: number): void { this._damping = clamp(v, 0, 1); this.sendParam('damping', this._damping); }
  setPredelay(v: number): void { this._predelay = clamp(v, 0, 50); this.sendParam('predelay', this._predelay); }
