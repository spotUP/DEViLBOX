...
export class ActionamicsEngine {
  ...
  private async initialize(): Promise<void> {
    try {
      await ActionamicsEngine.ensureInitialized(this.audioContext);
      this.createNode();
    } catch (err) {
      console.error('[ActionamicsEngine] Initialization failed:', err);
    }
  }
  ...
  private static async ensureInitialized(context: AudioContext): Promise<void> {
    ...
    const initPromise = (async () => {
      ...
      try { await context.audioWorklet.addModule(`${baseUrl}actionamics/Actionamics.worklet.js?v=${Date.now()}`); } catch { /* already registered */ }

      if (!this.wasmBinary || !this.jsCode) {
        const [wasmResponse, jsResponse] = await Promise.all([
          fetch(`${baseUrl}actionamics/Actionamics.wasm`),
          fetch(`${baseUrl}actionamics/Actionamics.js`),
        ]);
  ...
  private createNode(): void {
    ...
    this.workletNode = new AudioWorkletNode(ctx, 'actionamics-processor', {
    ...
    this.workletNode.port.postMessage({
      ...
      wasmBinary: ActionamicsEngine.wasmBinary, jsCode: ActionamicsEngine.jsCode,
    });
