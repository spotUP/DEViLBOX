...
export class OktalyzerEngine {
  ...
  private async initialize(): Promise<void> {
    try {
      await OktalyzerEngine.ensureInitialized(this.audioContext);
      this.createNode();
    } catch (err) {
      console.error('[OktalyzerEngine] Initialization failed:', err);
    }
  }
  ...
  private static async ensureInitialized(context: AudioContext): Promise<void> {
    ...
    const initPromise = (async () => {
      ...
      try { await context.audioWorklet.addModule(`${baseUrl}oktalyzer/Oktalyzer.worklet.js?v=${Date.now()}`); } catch { /* already registered */ }

      if (!this.wasmBinary || !this.jsCode) {
        const [wasmResponse, jsResponse] = await Promise.all([
          fetch(`${baseUrl}oktalyzer/Oktalyzer.wasm`),
          fetch(`${baseUrl}oktalyzer/Oktalyzer.js`),
        ]);
  ...
  private createNode(): void {
    ...
    this.workletNode.port.postMessage({
      ...
      wasmBinary: OktalyzerEngine.wasmBinary, jsCode: OktalyzerEngine.jsCode,
    });
