...
export class DeltaMusic1Engine {
  ...
  private async initialize(): Promise<void> {
    try {
      await DeltaMusic1Engine.ensureInitialized(this.audioContext);
      this.createNode();
    } catch (err) {
      console.error('[DeltaMusic1Engine] Initialization failed:', err);
    }
  }
  ...
  private static async ensureInitialized(context: AudioContext): Promise<void> {
    ...
    const initPromise = (async () => {
      ...
      try { await context.audioWorklet.addModule(`${baseUrl}deltamusic1/DeltaMusic1.worklet.js?v=${Date.now()}`); } catch { /* already registered */ }

      if (!this.wasmBinary || !this.jsCode) {
        const [wasmResponse, jsResponse] = await Promise.all([
          fetch(`${baseUrl}deltamusic1/DeltaMusic1.wasm`),
          fetch(`${baseUrl}deltamusic1/DeltaMusic1.js`),
        ]);
  ...
  private createNode(): void {
    ...
    this.workletNode.port.postMessage({
      ...
      wasmBinary: DeltaMusic1Engine.wasmBinary, jsCode: DeltaMusic1Engine.jsCode,
    });
