...
const ResizeHandle: React.FC<ResizeHandleProps> = ({ dir, winState, camera, onResizeStart }) => {
...
export const PixiWindow: React.FC<PixiWindowProps> = ({
  id, title, camera, screenW, screenH, navBarH = 98, onFocus, children,
}) => {
  ...
  /** Scale controlled by spring animation (separate from window scale) */
  const [animScale, setAnimScale] = useState<{ x: number; y: number }>({ x: 1, y: 1 });
