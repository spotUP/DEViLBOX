/**
 * SurgeHardwareUI — Full JUCE Surge XT editor rendered from WASM framebuffer
 *
 * Loads the Surge XT UI WASM module (compiled from the original JUCE plugin
 * editor with full DSP engine), renders to a pixel buffer, and blits to an
 * HTML canvas each frame.
 *
...
interface SurgeUIModule {
  _surge_ui_init: () => void;
  _surge_ui_init_scaled: (scale: number) => void;
  _surge_ui_tick: () => void;
  _surge_ui_get_fb: () => number;
  _surge_ui_get_width: () => number;
  _surge_ui_get_height: () => number;
  _surge_ui_on_mouse_down: (x: number, y: number, mods: number) => void;
  _surge_ui_on_mouse_up: (x: number, y: number, mods: number) => void;
  _surge_ui_on_mouse_move: (x: number, y: number, mods: number) => void;
  _surge_ui_on_mouse_wheel: (x: number, y: number, deltaX: number, deltaY: number) => void;
  _surge_ui_set_param: (paramId: number, value: number) => void;
  _surge_ui_get_param: (paramId: number) => number;
  _surge_ui_get_param_count: () => number;
  _surge_ui_get_program: () => number;
  _surge_ui_set_program: (p: number) => void;
  _surge_ui_get_program_count: () => number;
  _surge_ui_get_program_name: (p: number) => number;
  _surge_ui_shutdown: () => void;
  HEAPU8: Uint8Array;
...
interface SurgeHardwareUIProps {
  parameters?: Record<string, number>;
  onParamChange?: (key: string, value: number) => void;
  instrumentId?: number;
}

export const SurgeHardwareUI: React.FC<SurgeHardwareUIProps> = ({
  onParamChange: _onParamChange,
  instrumentId,
}) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const moduleRef = useRef<SurgeUIModule | null>(null);
    ...
    const init = async () => {
      try {
        const factory = await new Promise<
          (opts: Record<string, unknown>) => Promise<SurgeUIModule>
        >((resolve, reject) => {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const existing = (window as any).createSurgeUI;
          ...
          script.onload = () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const fn = (window as any).createSurgeUI;
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (window as any)._surgeUIParamCallback = (paramIndex: number, normalizedValue: number) => {
        ...
        eventCleanups.push(() => {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          delete (window as any)._surgeUIParamCallback;
        });
...
export default SurgeHardwareUI;
