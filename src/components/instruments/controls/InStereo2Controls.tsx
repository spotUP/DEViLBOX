...
/** Waveform line graph — signed int8 samples rendered to a small canvas. */
...
/** Bar chart for table data (128 uint8 values). */
const BarChart: React.FC<{
  ...
  color: string;
  signed?: boolean;
  markers?: { pos: number; color: string; label?: string }[];
}> = ({ data, width, height, color, signed, markers }) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  ...
  useEffect(() => {
    ...
    // Draw markers (sustain point, loop points, etc.)
    if (markers) {
