...
export const WireframeSphere: React.FC<VJSceneProps> = ({ audioRef }) => {
  ...
  useFrame((state) => {
    ...
    if (audio) {
      mat.uniforms.uBass.value += (audio.bassEnergy + audio.subEnergy * 0.5 - mat.uniforms.uBass.value) * 0.2;
      mat.uniforms.uMid.value += (audio.midEnergy - mat.uniforms.uMid.value) * 0.2;
      mat.uniforms.uHigh.value += (audio.highEnergy - mat.uniforms.uHigh.value) * 0.2;
      ...
      <shaderMaterial
        ...
        fragmentShader={`
          ...
          void main() {
            ...
            vec3 col = vec3(
              0.1 + fresnel * 0.5 + uBass * 0.3,
              0.2 + fresnel * 0.3 + uMid * 0.4,
              0.8 + fresnel * 0.2 + uHigh * 0.2
            );
            
            col += vec3(0.3, 0.1, 0.5) * abs(vDisplacement) * 3.0;
            col += vec3(1.0) * uBeat * fresnel * 0.5;
