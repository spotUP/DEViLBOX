...
export const SCKnobPanel: React.FC = memo(() => {
  ...
  return (
    <div
      className="border-b border-gray-700 bg-gray-900 select-none"
    ...
    >
      ...
      {/* Param sliders row */}
      {!scCollapsed && (
        <div className="flex items-center gap-3 px-3 h-[84px] border-t border-gray-800 overflow-x-auto">
          {visibleParams.map((param) => (
            <div key={param.name} className="flex flex-col items-center gap-1 min-w-[64px]">
              ...
              />
              <span className="text-[10px] text-gray-400 truncate max-w-[64px] text-center">
                {param.name}
              </span>
