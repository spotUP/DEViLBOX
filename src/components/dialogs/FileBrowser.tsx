 ...
 * - Support for both .dbox and tracker modules (mod/xm/it/s3m)
 */
...
import { isTD3File, parseTD3File, convertTD3FileToDbox } from '@/lib/import/TD3PatternLoader';
...
// Check if file is a TD-3 pattern file (.sqs)
const isTD3Pattern = (filename: string): boolean => {
  return isTD3File(filename);
};
...
export const FileBrowser: React.FC<FileBrowserProps> = ({
  ...
  const loadFiles = useCallback(async () => {
    ...
    try {
      ...
      if (viewMode === 'library') {
        ...
        if (dirHandle) {
          const entries = await listDirectory(dirHandle, ['.dbox', '.mod', '.xm', '.it', '.s3m', '.sqs', '.seq']);
  ...
  const handleLoad = async () => {
    ...
    try {
      ...
      const isTD3 = isTD3Pattern(selectedFile.name);

      // Handle TD-3 pattern files (.sqs) - load binary and convert to dbox
      if (isTD3) {
        let buffer: ArrayBuffer;

        if (selectedFile.source === 'filesystem' && selectedFile.handle) {
          buffer = await readFileAsBuffer(selectedFile.handle as FileSystemFileHandle);
        } else {
          throw new Error('TD-3 patterns can only be loaded from filesystem');
        }

        const td3File = await parseTD3File(buffer);
        const dboxProject = convertTD3FileToDbox(td3File, selectedFile.name);
        onLoad(dboxProject, selectedFile.name.replace(/\.(sqs|seq)$/i, '.dbox'));
        onClose();
        return;
      }
  ...
  const handleBrowseFiles = async () => {
    if (isFileSystemAccessSupported()) {
      ...
      if (handles.length > 0) {
        ...
        // Check if it's a TD-3 pattern file
        if (isTD3Pattern(file.name)) {
          const buffer = await readFileAsBuffer(handles[0]);
          const td3File = await parseTD3File(buffer);
          const dboxProject = convertTD3FileToDbox(td3File, file.name);
          onLoad(dboxProject, file.name.replace(/\.(sqs|seq)$/i, '.dbox'));
          onClose();
          return;
        }

        // Check if it's a tracker module
        if (isTrackerModule(file.name) && onLoadTrackerModule) {
          const buffer = await readFileAsBuffer(handles[0]);
      ...
      input.type = 'file';
      input.accept = '.dbox,.mod,.xm,.it,.s3m,.sqs,.seq';
