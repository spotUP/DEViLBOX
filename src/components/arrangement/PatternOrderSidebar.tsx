...
export const PatternOrderSidebar: React.FC = () => {
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    switch (e.key) {
      ...
      case 'Backspace':
        e.preventDefault();
        if (selection.size > 0) {
          // Remove selected positions (highest first to preserve indices)
      ...
      case 'd':
        if (e.ctrlKey || e.metaKey) {
          e.preventDefault();
          if (selection.size > 0) {
            // Duplicate all selected (insert after last selected)
  ...
  const handleContextMenu = useCallback((e: React.MouseEvent, posIdx: number) => {
    e.preventDefault();
    // If right-clicking outside selection, select just this one
  ...
  // Effective selection for context menu (use selection if multi, otherwise just the clicked pos)
  ...
  return (
    ...
    >
      {/* Header — shows selection count when multi-selected */}
