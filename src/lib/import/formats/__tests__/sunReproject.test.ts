...
describe('applySunNoteEdit + reprojectSunGrid', () => {
  // Sign convention (proven against the display walk, decodeSunGroup):
  //   displayNote = poolNote - transpose     (pool is decoded at transpose 0)
  // so the edit inverse ADDS transpose to recover the pool note, and reproject
  // SUBTRACTS it to re-derive the display. (Both were previously inverted; the
  // two wrong signs cancelled for the edited cell at its own position, which is
  // why the old assertions looked self-consistent while every other position
  // that reused the block rendered the wrong pitch.)
  it('writes raw pitch = editedNote + transpose and invalidates sunRaw', () => {
    ...
    expect(nd.blocks[0][0].note).toBe(64);            // 52 + 12 = 64 (pool = display + transpose)
  ...
  it('editing at transpose +12 changes the raw note when the edit differs', () => {
    ...
    expect(nd.blocks[0][0].note).toBe(67);            // 55 + 12
  ...
  it('reprojectSunGrid applies pool note - transpose to each provenance cell', () => {
    ...
    expect(cell.note).toBe(28);        // 40 - 12
