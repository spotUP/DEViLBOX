...
void sa_set_param(void *ctxPtr, int handle, int paramId, float value) {
    ...
    case 11: /* Effect 0xy arpeggio arg (0=off, 0xXY=arp) — sent as integer 0-255 */
        ...
        break;
    case 12: { /* SetVibrato (SA effect 4) — arg packed as integer 0xXY.
                * Ref line 1721-1723: delay=0, speed=(hi>>3), level=-(lo<<4)+160 */
        int arg = clamp_i((int)value, 0, 255);
        p->vibratoDelayCtr = 0;
        p->ins.vibratoSpeed = (uint16_t)((arg >> 3) & 0x1E);  /* (hi>>3) */
        p->ins.vibratoLevel = (uint16_t)((-((arg & 0x0F) << 4)) + 160);
        break;
    }
    case 13: /* RestartAdsr (SA effect 2) — set ADSR position to arg */
        p->adsrPosition = (uint16_t)clamp_i((int)value, 0, SA_TABLE_SIZE - 1);
        break;
    case 14: /* SkipPortamento (SA effect 8) — zero portamento speed */
        p->ins.portamentoSpeed = 0;
        break;
    case 15: /* SetPortamento (SA effect 7) — set portamento speed directly */
        p->ins.portamentoSpeed = (uint16_t)clamp_i((int)value, 0, 255);
        break;
    case 16: /* SetSlideSpeed (SA effect 1) — signed byte as slide speed.
              * Ref line 1702: SlideSpeed reset to 0 each row, then set from effect.
              * Ref line 1709: voiceInfo.SlideSpeed = (sbyte)voiceInfo.EffectArg */
        p->slideSpeed = (int16_t)(int8_t)clamp_i((int)value, -128, 127);
        break;
    case 17: /* VolumeSlide (SA effect A) — signed byte.
              * Ref line 1755: voiceInfo.VolumeSlideSpeed = (sbyte)voiceInfo.EffectArg */
        p->volumeSlideSpeed = (int16_t)(int8_t)clamp_i((int)value, -128, 127);
        break;
    default:
        break;
    }
