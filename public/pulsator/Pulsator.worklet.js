/**
 * Pulsator.worklet.js — AudioWorklet processor for Pulsator WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createPulsator;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createPulsator factory');
...
class PulsatorProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    const fn = M['_pulsator_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._pulsator_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._pulsator_create(sampleRate);
      if (this.handle < 0) throw new Error('pulsator_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Pulsator] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._pulsator_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('pulsator-processor', PulsatorProcessor); processorRegistered = true; }
