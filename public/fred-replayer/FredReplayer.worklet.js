/**
 * FredReplayer.worklet.js — AudioWorklet processor for FredReplayer WASM replayer
 * Whole-song replayer with per-channel output for oscilloscope.
 */
class FredReplayerProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
    this.handle = 0;
  ...
  async handleMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'loadModule': {
        ...
        try {
          if (this.handle) { this.module._fred_destroy(this.handle); this.handle = 0; }
          ...
          this.handle = this.module._fred_create(wasmPtr, uint8Data.length, sampleRate);
          this.module._free(wasmPtr);
          if (!this.handle) { this.port.postMessage({ type: 'error', message: 'fred_create failed (unsupported format)' }); return; }
          const subsongCount = this.module._fred_subsong_count(this.handle);
          if (typeof data.subsong === 'number' && data.subsong > 0) this.module._fred_select_subsong(this.handle, data.subsong);
          this.playing = false;
      ...
      case 'stop':
        this.playing = false;
        if (this.handle) this.module._fred_select_subsong(this.handle, 0);
  ...
  async initWasm(rate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.createFredReplayer) {
        const wrappedCode = jsCode + '\nreturn createFredReplayer;';
        const factory = new Function(wrappedCode);
        const result = factory();
        if (typeof result === 'function') globalThis.createFredReplayer = result;
        else { this.port.postMessage({ type: 'error', message: 'Failed to load JS module' }); return; }
      }
      if (typeof globalThis.createFredReplayer !== 'function') { this.port.postMessage({ type: 'error', message: 'createFredReplayer factory not available' }); return; }
      ...
      try { this.module = await globalThis.createFredReplayer(config); } finally { WebAssembly.instantiate = origInstantiate; }
  ...
  cleanup() {
    if (this.module && this.handle) { this.module._fred_destroy(this.handle); this.handle = 0; }
    if (this.module) {
  ...
  process(inputs, outputs) {
    ...
    // Render per-channel
    const rendered = this.module._fred_render_multi(
    ...
    if (this.module._fred_has_ended(this.handle)) this.port.postMessage({ type: 'songEnd' });
    return true;
  }
}
registerProcessor('fred-replayer-processor', FredReplayerProcessor);
