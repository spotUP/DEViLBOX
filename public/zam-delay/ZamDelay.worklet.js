...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try {
      const wrappedCode = `${jsCode}; return createZamDelay;`;
      createModule = new Function(wrappedCode)();
    } catch (e) { console.error('[ZamDelay] Failed:', e); sharedModulePromise = null; throw e; }
...
class ZamDelayProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParameter(param, value) {
    ...
    const fn = M['_zam_delay_set_' + param];
    if (fn) fn(h, value);
  }
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._zam_delay_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = Module._zam_delay_create(sampleRate);
      ...
      console.error('[ZamDelay] init error:', error);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._zam_delay_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('zam-delay-processor', ZamDelayProcessor); processorRegistered = true; }
