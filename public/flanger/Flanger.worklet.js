/**
 * Flanger.worklet.js — AudioWorklet processor for Flanger WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createFlanger;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createFlanger factory');
...
class FlangerProcessor extends AudioWorkletProcessor {
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'rate':     M._flanger_set_rate(h, value);     break;
      case 'depth':    M._flanger_set_depth(h, value);    break;
      case 'delay':    M._flanger_set_delay(h, value);    break;
      case 'feedback': M._flanger_set_feedback(h, value); break;
      case 'stereo':   M._flanger_set_stereo(h, value);   break;
      case 'mix':      M._flanger_set_mix(h, value);      break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._flanger_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._flanger_create(sampleRate);
      if (this.handle < 0) throw new Error('flanger_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Flanger] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._flanger_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('flanger-processor', FlangerProcessor); processorRegistered = true; }
