/**
 * Panda.worklet.js — AudioWorklet processor for Panda WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createPanda;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createPanda factory');
...
class PandaProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'threshold': M['_panda_set_threshold'](h, value); break;
      case 'factor': M['_panda_set_factor'](h, value); break;
      case 'release': M['_panda_set_release'](h, value); break;
      case 'mix': M['_panda_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_panda_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_panda_create'](sampleRate);
      if (this.handle < 0) throw new Error('panda_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Panda] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_panda_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('panda-processor', PandaProcessor); processorRegistered = true; }
