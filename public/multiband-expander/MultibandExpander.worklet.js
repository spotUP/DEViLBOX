/**
 * MultibandExpander.worklet.js — AudioWorklet processor for MultibandExpander WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createMultibandExpander;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createMultibandExpander factory');
...
class MultibandExpanderProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'lowCross': M['_multiband_expander_set_lowCross'](h, value); break;
      case 'highCross': M['_multiband_expander_set_highCross'](h, value); break;
      case 'lowThresh': M['_multiband_expander_set_lowThresh'](h, value); break;
      case 'midThresh': M['_multiband_expander_set_midThresh'](h, value); break;
      case 'highThresh': M['_multiband_expander_set_highThresh'](h, value); break;
      case 'ratio': M['_multiband_expander_set_ratio'](h, value); break;
      case 'attack': M['_multiband_expander_set_attack'](h, value); break;
      case 'release': M['_multiband_expander_set_release'](h, value); break;
      case 'range': M['_multiband_expander_set_range'](h, value); break;
      case 'mix': M['_multiband_expander_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_multiband_expander_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_multiband_expander_create'](sampleRate);
      if (this.handle < 0) throw new Error('multiband_expander_create failed');
      const bs = 128 * 4;
      ...
      console.error('[MultibandExpander] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_multiband_expander_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('multiband-expander-processor', MultibandExpanderProcessor); processorRegistered = true; }
