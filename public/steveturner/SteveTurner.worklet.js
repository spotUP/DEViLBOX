/**
 * SteveTurner.worklet.js - AudioWorklet processor for Steve Turner WASM replayer
 */

class SteveTurnerProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
  ...
  async initModule(sampleRate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.SteveTurner) {
        ...
        const wrappedCode = jsCode + '\nreturn createSteveTurner;';
        const factory = new Function(wrappedCode);
        const result = factory();

        if (typeof result === 'function') {
          globalThis.SteveTurner = result;
        } else {
      ...
      if (typeof globalThis.SteveTurner !== 'function') {
        this.port.postMessage({ type: 'error', message: 'SteveTurner factory not available' });
        return;
      }
      ...
      try {
        this.module = await globalThis.SteveTurner(config);
      } finally {
...
registerProcessor('steveturner-processor', SteveTurnerProcessor);
