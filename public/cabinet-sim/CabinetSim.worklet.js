/**
 * CabinetSim.worklet.js — AudioWorklet processor for CabinetSim WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createCabinetSim;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createCabinetSim factory');
...
class CabinetSimProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    const fn = M['_cabinet_sim_set_' + param];
    if (fn) fn(h, value);
  }
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._cabinet_sim_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._cabinet_sim_create(sampleRate);
      if (this.handle < 0) throw new Error('cabinet_sim_create failed');
      const bs = 128 * 4;
      ...
      console.error('[CabinetSim] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._cabinet_sim_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
      outL.set(this.outputBufferL.subarray(0, n)); outR.set(this.outputBufferR.subarray(0, n));
    } catch (err) { outL.set(inL); outR.set(inR); }
    return true;
  }
}
if (!processorRegistered) { registerProcessor('cabinet-sim-processor', CabinetSimProcessor); processorRegistered = true; }
