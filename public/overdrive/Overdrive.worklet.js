/**
 * Overdrive.worklet.js — AudioWorklet processor for Overdrive WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createOverdrive;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createOverdrive factory');
...
class OverdriveProcessor extends AudioWorkletProcessor {
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'drive': M._overdrive_set_drive(h, value); break;
      case 'tone':  M._overdrive_set_tone(h, value);  break;
      case 'mix':   M._overdrive_set_mix(h, value);   break;
      case 'level': M._overdrive_set_level(h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._overdrive_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._overdrive_create(sampleRate);
      if (this.handle < 0) throw new Error('overdrive_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Overdrive] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._overdrive_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('overdrive-processor', OverdriveProcessor); processorRegistered = true; }
