/**
 * BeatBreather.worklet.js — AudioWorklet processor for BeatBreather WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createBeatBreather;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createBeatBreather factory');
...
class BeatBreatherProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'transientBoost': M['_beat_breather_set_transientBoost'](h, value); break;
      case 'sustainBoost': M['_beat_breather_set_sustainBoost'](h, value); break;
      case 'sensitivity': M['_beat_breather_set_sensitivity'](h, value); break;
      case 'attack': M['_beat_breather_set_attack'](h, value); break;
      case 'release': M['_beat_breather_set_release'](h, value); break;
      case 'mix': M['_beat_breather_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_beat_breather_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_beat_breather_create'](sampleRate);
      if (this.handle < 0) throw new Error('beat_breather_create failed');
      const bs = 128 * 4;
      ...
      console.error('[BeatBreather] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_beat_breather_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('beat-breather-processor', BeatBreatherProcessor); processorRegistered = true; }
