/**
 * GEQ31.worklet.js — AudioWorklet processor for 31-band graphic EQ WASM effect.
 * Custom applyParam: parses 'band_0' through 'band_30' → geq31_set_band(h, idx, gain).
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createGEQ31;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createGEQ31 factory');
...
class GEQ31Processor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    if (param === 'mix') {
      M._geq31_set_mix(h, value);
      return;
    }
    // Parse 'band_0' through 'band_30'
    if (param.startsWith('band_')) {
      const idx = parseInt(param.substring(5));
      if (!isNaN(idx) && idx >= 0 && idx < 31) {
        M._geq31_set_band(h, idx, value);
      }
    }
  }
  ...
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._geq31_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._geq31_create(sampleRate);
      if (this.handle < 0) throw new Error('geq31_create failed');
      const bs = 128 * 4;
      ...
      console.error('[GEQ31] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._geq31_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('geq31-processor', GEQ31Processor); processorRegistered = true; }
