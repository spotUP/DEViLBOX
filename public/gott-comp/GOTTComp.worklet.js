/**
 * GOTTComp.worklet.js — AudioWorklet processor for GOTTComp WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createGOTTComp;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createGOTTComp factory');
...
class GOTTCompProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'lowCross': M['_gott_comp_set_lowCross'](h, value); break;
      case 'highCross': M['_gott_comp_set_highCross'](h, value); break;
      case 'lowThresh': M['_gott_comp_set_lowThresh'](h, value); break;
      case 'midThresh': M['_gott_comp_set_midThresh'](h, value); break;
      case 'highThresh': M['_gott_comp_set_highThresh'](h, value); break;
      case 'lowRatio': M['_gott_comp_set_lowRatio'](h, value); break;
      case 'midRatio': M['_gott_comp_set_midRatio'](h, value); break;
      case 'highRatio': M['_gott_comp_set_highRatio'](h, value); break;
      case 'attack': M['_gott_comp_set_attack'](h, value); break;
      case 'release': M['_gott_comp_set_release'](h, value); break;
      case 'mix': M['_gott_comp_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_gott_comp_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_gott_comp_create'](sampleRate);
      if (this.handle < 0) throw new Error('gott_comp_create failed');
      const bs = 128 * 4;
      ...
      console.error('[GOTTComp] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_gott_comp_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('gott-comp-processor', GOTTCompProcessor); processorRegistered = true; }
