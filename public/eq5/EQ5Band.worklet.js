/**
 * EQ5Band.worklet.js — AudioWorklet processor for 5-band EQ WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createEQ5Band;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createEQ5Band factory');
...
class EQ5BandProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    const fn = M['_eq5_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._eq5_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._eq5_create(sampleRate);
      if (this.handle < 0) throw new Error('eq5_create failed');
      const bs = 128 * 4;
      ...
      console.error('[EQ5Band] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._eq5_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('eq5-processor', EQ5BandProcessor); processorRegistered = true; }
