/**
 * PMD AudioWorklet — pmdmini playback in Web Audio.
...
class PMDWorklet extends AudioWorkletProcessor {
  constructor() {
    super();
    this._module = null;
    this._playing = false;
  ...
  async _initWASM(wasmUrl) {
    try {
      const wasmModule = await import(wasmUrl || '/pmd/PMD.js');
      const factory = wasmModule.default || wasmModule.createPMD || wasmModule;
      ...
      this.port.postMessage({ type: 'error', message: 'PMD WASM init failed: ' + err.message });
    }
  }
  ...
  _loadFile(arrayBuffer) {
    ...
    const result = this._module._pmd_bridge_open(ptr, u8.length);
    this._module._free(ptr);

    if (result < 0) {
      this.port.postMessage({ type: 'error', message: 'Failed to open PMD' });
      return;
    }

    const title = this._module.UTF8ToString(this._module._pmd_bridge_get_title());
    const duration = this._module._pmd_bridge_get_length();
    this.port.postMessage({ type: 'loaded', title, duration, channels: 11 });
  }
  ...
  _renderToRing() {
    ...
    this._module._pmd_bridge_calc(this._i16Ptr >> 1, RENDER_FRAMES);
  ...
  process(_inputs, outputs) {
    ...
    while (this._ringAvail < blockSize * 2) {
      this._renderToRing();
      if (this._module._pmd_bridge_track_ended()) {
    ...
    if (this._posCounter >= sampleRate / 4) {
      this._posCounter = 0;
      this.port.postMessage({ type: 'position', msec: this._module._pmd_bridge_tell() });
    }
    return true;
  }
}

registerProcessor('pmd-worklet', PMDWorklet);
