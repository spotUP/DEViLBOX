/**
 * Vihda.worklet.js — AudioWorklet processor for Vihda WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createVihda;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createVihda factory');
...
class VihdaProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'width': M['_vihda_set_width'](h, value); break;
      case 'invert': M['_vihda_set_invert'](h, value); break;
      case 'mix': M['_vihda_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_vihda_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_vihda_create'](sampleRate);
      if (this.handle < 0) throw new Error('vihda_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Vihda] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_vihda_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('vihda-processor', VihdaProcessor); processorRegistered = true; }
