/**
 * InStereo2.worklet.js — AudioWorklet processor for InStereo2 WASM replayer
 * Whole-song replayer with per-channel output for oscilloscope.
 */
class InStereo2Processor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
    this.handle = 0;
  ...
  async handleMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'loadModule': {
        ...
        try {
          if (this.handle) { this.module._is2_destroy(this.handle); this.handle = 0; }
          ...
          this.handle = this.module._is2_create(wasmPtr, uint8Data.length, sampleRate);
          this.module._free(wasmPtr);
          if (!this.handle) { this.port.postMessage({ type: 'error', message: 'is2_create failed (unsupported format)' }); return; }
          const subsongCount = this.module._is2_subsong_count(this.handle);
          if (typeof data.subsong === 'number' && data.subsong > 0) this.module._is2_select_subsong(this.handle, data.subsong);
          this.playing = false;
      ...
      case 'stop':
        this.playing = false;
        if (this.handle) this.module._is2_select_subsong(this.handle, 0);
        this.port.postMessage({ type: 'stopped' });
        break;
      case 'pause': this.playing = !this.playing; break;
      case 'setSubsong': if (this.handle) this.module._is2_select_subsong(this.handle, data.subsong); break;
      case 'setChannelMask': if (this.handle) this.module._is2_set_channel_mask(this.handle, data.mask); break;
      case 'setCell': {        if (this.handle && this.module._is2_set_cell) {          this.module._is2_set_cell(this.handle, data.index, data.row, data.channel, data.note, data.instrument, data.effect, data.effectArg);        }        break;      }      case 'setInstrumentParam': {        if (this.handle && this.module._is2_set_instrument_param) {          var pLen = this.module.lengthBytesUTF8(data.param) + 1;          var pPtr = this.module._malloc(pLen);          this.module.stringToUTF8(data.param, pPtr, pLen);          this.module._is2_set_instrument_param(this.handle, data.instrument, pPtr, data.value);          this.module._free(pPtr);        }        break;      }      case 'dispose': this.cleanup(); break;
    }
  }
  ...
  async initWasm(rate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.createInStereo2) {
        const wrappedCode = jsCode + '\nreturn createInStereo2;';
        const factory = new Function(wrappedCode);
        const result = factory();
        if (typeof result === 'function') globalThis.createInStereo2 = result;
        else { this.port.postMessage({ type: 'error', message: 'Failed to load JS module' }); return; }
      }
      if (typeof globalThis.createInStereo2 !== 'function') { this.port.postMessage({ type: 'error', message: 'createInStereo2 factory not available' }); return; }
      ...
      try { this.module = await globalThis.createInStereo2(config); } finally { WebAssembly.instantiate = origInstantiate; }
  ...
  cleanup() {
    if (this.module && this.handle) { this.module._is2_destroy(this.handle); this.handle = 0; }
    if (this.module) {
  ...
  process(inputs, outputs) {
    ...
    // Render per-channel
    const rendered = this.module._is2_render_multi(
    ...
    if (this.module._is2_has_ended(this.handle)) this.port.postMessage({ type: 'songEnd' });
    return true;
  }
}
registerProcessor('instereo2-processor', InStereo2Processor);
