/**
 * JunoChorus.worklet.js — AudioWorklet processor for JunoChorus WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createJunoChorus;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createJunoChorus factory');
...
class JunoChorusProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    const fn = M['_juno_chorus_set_' + param];
    if (fn) fn(h, value);
  }
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._juno_chorus_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._juno_chorus_create(sampleRate);
      if (this.handle < 0) throw new Error('juno_chorus_create failed');
      const bs = 128 * 4;
      ...
      console.error('[JunoChorus] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._juno_chorus_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
      outL.set(this.outputBufferL.subarray(0, n)); outR.set(this.outputBufferR.subarray(0, n));
    } catch (err) { outL.set(inL); outR.set(inR); }
    return true;
  }
}
if (!processorRegistered) { registerProcessor('juno-chorus-processor', JunoChorusProcessor); processorRegistered = true; }
