/**
 * Gmc.worklet.js — AudioWorklet processor for Gmc WASM replayer
 * Whole-song replayer with per-channel output for oscilloscope.
 */
class GmcProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
    this.handle = 0;
  ...
  async handleMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'loadModule': {
        ...
        try {
          if (this.handle) { this.module._gmc_destroy(this.handle); this.handle = 0; }
          ...
          this.handle = this.module._gmc_create(wasmPtr, uint8Data.length, sampleRate);
          this.module._free(wasmPtr);
          if (!this.handle) { this.port.postMessage({ type: 'error', message: 'gmc_create failed (unsupported format)' }); return; }
          const subsongCount = this.module._gmc_subsong_count(this.handle);
          if (typeof data.subsong === 'number' && data.subsong > 0) this.module._gmc_select_subsong(this.handle, data.subsong);
          this.playing = false;
      ...
      case 'stop':
        this.playing = false;
        if (this.handle) this.module._gmc_select_subsong(this.handle, 0);
  ...
  async initWasm(rate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.createGmc) {
        const wrappedCode = jsCode + '\nreturn createGmc;';
        const factory = new Function(wrappedCode);
        const result = factory();
        if (typeof result === 'function') globalThis.createGmc = result;
        else { this.port.postMessage({ type: 'error', message: 'Failed to load JS module' }); return; }
      }
      if (typeof globalThis.createGmc !== 'function') { this.port.postMessage({ type: 'error', message: 'createGmc factory not available' }); return; }
      ...
      try { this.module = await globalThis.createGmc(config); } finally { WebAssembly.instantiate = origInstantiate; }
  ...
  cleanup() {
    if (this.module && this.handle) { this.module._gmc_destroy(this.handle); this.handle = 0; }
    if (this.module) {
  ...
  process(inputs, outputs) {
    ...
    // Render per-channel
    const rendered = this.module._gmc_render_multi(
    ...
    if (this.module._gmc_has_ended(this.handle)) this.port.postMessage({ type: 'songEnd' });
    return true;
  }
}
registerProcessor('gmc-processor', GmcProcessor);
