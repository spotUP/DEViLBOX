/**
 * ArtOfNoise.worklet.js - AudioWorklet processor for Art of Noise WASM engine
 *
...
class ArtOfNoiseProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
      ...
      case 'loadModule':
        if (this.module && typeof this.module._player_load === 'function') {
          try {
            ...
            const ok = this.module._player_load(wasmPtr, uint8Data.length);
            const free = this.module._free || this.module.free;
            if (free) free(wasmPtr);
  ...
  async initModule(sampleRate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.ArtOfNoise) {
        ...
        const wrappedCode = jsCode + '\nreturn createArtOfNoise;';
        const factory = new Function(wrappedCode);
        const result = factory();

        if (typeof result === 'function') {
          globalThis.ArtOfNoise = result;
        } else {
      ...
      if (typeof globalThis.ArtOfNoise !== 'function') {
        this.port.postMessage({ type: 'error', message: 'ArtOfNoise factory not available' });
        return;
      }
      ...
      try {
        this.module = await globalThis.ArtOfNoise(config);
      } finally {
      ...
      if (typeof this.module._player_init === 'function') {
        this.module._player_init();
      }
  ...
  process(inputs, outputs, parameters) {
    ...
    // _player_get_channel_levels returns a pointer to float[16] (8ch × L+R)
    if (++this.processCount >= 8 && typeof this.module._player_get_channel_levels === 'function') {
      this.processCount = 0;
      const levelsPtr = this.module._player_get_channel_levels();
      if (levelsPtr) {
        const heapF32 = this.module.HEAPF32 || (this.module.wasmMemory && new Float32Array(this.module.wasmMemory.buffer));
        if (heapF32) {
          const off = levelsPtr >> 2;
          // Send up to 8 channels × 2 (L+R) = 16 floats
          const levels = [];
          for (let i = 0; i < 16; i++) levels.push(heapF32[off + i]);
...
registerProcessor('artofnoise-processor', ArtOfNoiseProcessor);
