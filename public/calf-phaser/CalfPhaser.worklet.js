/**
 * CalfPhaser.worklet.js — AudioWorklet processor for CalfPhaser WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createCalfPhaser;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createCalfPhaser factory');
...
class CalfPhaserProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'rate': M['_calf_phaser_set_rate'](h, value); break;
      case 'depth': M['_calf_phaser_set_depth'](h, value); break;
      case 'stages': M['_calf_phaser_set_stages'](h, value); break;
      case 'feedback': M['_calf_phaser_set_feedback'](h, value); break;
      case 'stereoPhase': M['_calf_phaser_set_stereoPhase'](h, value); break;
      case 'mix': M['_calf_phaser_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_calf_phaser_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_calf_phaser_create'](sampleRate);
      if (this.handle < 0) throw new Error('calf_phaser_create failed');
      const bs = 128 * 4;
      ...
      console.error('[CalfPhaser] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_calf_phaser_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('calf-phaser-processor', CalfPhaserProcessor); processorRegistered = true; }
