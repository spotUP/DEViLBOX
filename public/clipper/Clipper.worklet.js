/**
 * Clipper.worklet.js — AudioWorklet processor for Clipper WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createClipper;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createClipper factory');
...
class ClipperProcessor extends AudioWorkletProcessor {
  ...
  async handleMessage(event) {
    ...
    switch (type) {
      ...
      case 'parameter':
        if (this.isInitialized && this.handle >= 0) this.Module['_clipper_set_' + data.param](this.handle, data.value);
  ...
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._clipper_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._clipper_create(sampleRate);
      if (this.handle < 0) throw new Error('clipper_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Clipper] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._clipper_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('clipper-processor', ClipperProcessor); processorRegistered = true; }
