/**
 * DattorroPlate.worklet.js - DattorroPlate Plate Reverb AudioWorklet Effect Processor
 * Loads and runs the DattorroPlate WASM module as an audio effect (input -> output).
 *
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    ...
    let createModule;
    try {
      const wrappedCode = `${jsCode}; return createDattorroPlateModule;`;
      createModule = new Function(wrappedCode)();
    } catch (evalErr) {
      console.error('Failed to evaluate DattorroPlate JS:', evalErr);
      sharedModulePromise = null;
      throw new Error('Could not evaluate DattorroPlate module factory');
    }

    if (!createModule) {
      sharedModulePromise = null;
      throw new Error('Could not load DattorroPlate module factory');
    }
...
class DattorroPlateProcessor extends AudioWorkletProcessor {
  ...
  async initialize(data) {
    try {
      ...
      this.effect = new Module.DattorroPlateEffect();
      ...
      console.error('DattorroPlate initialization error:', error);
...
if (!processorRegistered) {
  registerProcessor('dattorro-plate-processor', DattorroPlateProcessor);
