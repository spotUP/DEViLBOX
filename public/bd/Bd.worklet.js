/**
 * Bd.worklet.js - AudioWorklet processor for Ben Daglish WASM synth
 *
...
class BdProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
  ...
  async handleMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'getInstrumentCount': {
        if (!this.module || typeof this.module._bd_get_instrument_count !== 'function') break;
        var count = this.module._bd_get_instrument_count();
  ...
  async initModule(sampleRate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (typeof globalThis.Bd !== 'function') {
        this.port.postMessage({ type: 'error', message: 'Bd factory not available' });
        return;
      }
      ...
      try {
        this.module = await globalThis.Bd(config);
      } finally {
...
registerProcessor('bd-processor', BdProcessor);
