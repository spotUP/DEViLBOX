/**
 * Cpsycle.worklet.js - AudioWorklet processor for Cpsycle (Psycle tracker) WASM engine
 *
 * The WASM _cpsycle_render() writes interleaved stereo (LRLRLR...) into a
 * single float buffer. We deinterleave into the WebAudio output channels.
 */

class CpsycleProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
  ...
  async handleMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'loadModule':
        if (this.module && typeof this.module._cpsycle_init === 'function') {
          try {
            ...
            const result = this.module._cpsycle_init(wasmPtr, uint8Data.length);
            const free = this.module._free || this.module.free;
            if (free) free(wasmPtr);

            if (result !== 0) {
              this.port.postMessage({ type: 'error', message: 'cpsycle_init failed with code ' + result });
              return;
            }
      ...
      case 'stop':
        if (this.module && typeof this.module._cpsycle_stop === 'function') {
          this.module._cpsycle_stop();
  ...
  async initModule(sampleRate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.Cpsycle) {
        ...
        const wrappedCode = jsCode + '\nreturn createCpsycle;';
        const factory = new Function(wrappedCode);
        const result = factory();

        if (typeof result === 'function') {
          globalThis.Cpsycle = result;
        } else {
      ...
      if (typeof globalThis.Cpsycle !== 'function') {
        this.port.postMessage({ type: 'error', message: 'Cpsycle factory not available' });
        return;
      }
      ...
      try {
        this.module = await globalThis.Cpsycle(config);
      } finally {
  ...
  cleanup() {
    if (this.module && typeof this.module._cpsycle_stop === 'function') {
      try { this.module._cpsycle_stop(); } catch (_) { /* ignore */ }
    }
    const free = this.module?._free || this.module?.free;
  ...
  process(inputs, outputs, parameters) {
    ...
    if (typeof this.module._cpsycle_render === 'function') {
      this.updateBufferViews();
      if (this.interleavedBuf) {
        const rendered = this.module._cpsycle_render(this.interleavedPtr, numSamples);
        if (rendered > 0) {
...
registerProcessor('cpsycle-processor', CpsycleProcessor);
