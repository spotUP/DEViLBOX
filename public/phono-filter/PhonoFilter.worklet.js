/**
 * PhonoFilter.worklet.js — AudioWorklet processor for PhonoFilter WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createPhonoFilter;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createPhonoFilter factory');
...
class PhonoFilterProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    const fn = M['_phono_filter_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._phono_filter_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._phono_filter_create(sampleRate);
      if (this.handle < 0) throw new Error('phono_filter_create failed');
      const bs = 128 * 4;
      ...
      console.error('[PhonoFilter] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._phono_filter_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('phono-filter-processor', PhonoFilterProcessor); processorRegistered = true; }
