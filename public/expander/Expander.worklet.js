/**
 * Expander.worklet.js — AudioWorklet processor for Expander WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try {
      const wrappedCode = `${jsCode}; return createExpander;`;
      createModule = new Function(wrappedCode)();
    } catch (e) {
      console.error('[Expander] Failed to evaluate JS:', e);
    ...
    if (!createModule) {
      sharedModulePromise = null;
      throw new Error('Could not load Expander module factory');
    }
...
class ExpanderProcessor extends AudioWorkletProcessor {
  ...
  applyParameter(param, value) {
    const M = this.Module;
    const h = this.handle;
    const fn = M['_expander_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._expander_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = Module._expander_create(sampleRate);
      if (this.handle < 0) throw new Error('expander_create returned ' + this.handle);
      ...
      console.error('[Expander] init error:', error);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._expander_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
