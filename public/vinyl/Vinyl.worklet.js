/**
 * Vinyl.worklet.js — AudioWorklet processor for Vinyl WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createVinyl;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createVinyl factory');
...
class VinylProcessor extends AudioWorkletProcessor {
  ...
  applyParam(param, value) {
    ...
    const fn = M['_vinyl_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._vinyl_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._vinyl_create(sampleRate);
      if (this.handle < 0) throw new Error('vinyl_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Vinyl] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._vinyl_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('vinyl-processor', VinylProcessor); processorRegistered = true; }
