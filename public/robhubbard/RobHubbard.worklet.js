/**
 * RobHubbard.worklet.js - AudioWorklet processor for Rob Hubbard WASM synth
 *
 * Architecture:
 *   - Multiple simultaneous instrument players (handles 0..MAX_PLAYERS-1)
 *   - Each player is a standalone rh_create_player() instance
 *   - Messages from main thread: init, createPlayer, destroyPlayer,
 *     loadInstrument, noteOn, noteOff, setParam
 *   - Audio rendering: rh_render() per-player, mixed into output
 *
 * Follows the HippelCoSo worklet pattern exactly, adapted for the rh_ prefix.
 */

class RobHubbardProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.wasm = null;
    this.ctx = null;
  ...
  async handleMessage(data) {
    switch (data.type) {
      ...
      case 'createPlayer': {
        ...
        const handle = this.wasm._rh_create_player(this.ctx);
        if (handle >= 0) {
          ...
          this.port.postMessage({ type: 'error', message: 'rh_create_player failed (max players reached)' });
        }
        break;
      }
      ...
      case 'destroyPlayer': {
        ...
        this.wasm._rh_destroy_player(this.ctx, h);
        break;
      }
      ...
      case 'loadInstrument': {
        ...
        heap.set(insData);
        const result = this.wasm._rh_load_instrument(this.ctx, data.handle, ptr, insData.length);
        this.wasm._free(ptr);
        if (result !== 0) {
          this.port.postMessage({ type: 'error', message: `rh_load_instrument failed: ${result}` });
        }
        break;
      }
      ...
      case 'noteOn':
        if (this.wasm && this.ctx) {
          this.wasm._rh_note_on(this.ctx, data.handle, data.note, data.velocity || 100);
        }
        break;

      case 'noteOff':
        if (this.wasm && this.ctx) {
          this.wasm._rh_note_off(this.ctx, data.handle);
        }
        break;

      case 'setParam':
        if (this.wasm && this.ctx) {
          this.wasm._rh_set_param(this.ctx, data.handle, data.paramId, data.value);
        }
        break;

      case 'setInstrumentParam': {
        if (this.wasm && this.ctx) {
          var paramMap = { volume: [0, 64], sampleVolume: [0, 64], portaSpeed: [1, 127], divider: [2, 255] };
          var entry = paramMap[data.param];
          if (entry) {
            var normalized = data.value / entry[1];
            for (var h in this.players) {
              this.wasm._rh_set_param(this.ctx, parseInt(h), entry[0], normalized);
            }
          }
        }
        break;
      ...
      case 'dispose':
        if (this.wasm && this.ctx) {
          ...
          for (const h of Object.keys(this.players)) {
            ...
            this.wasm._rh_destroy_player(this.ctx, hi);
          }
          this.players = {};
          this.wasm._rh_dispose(this.ctx);
          this.ctx = null;
        }
        this.initialized = false;
        break;
  ...
  async initWasm(sr, wasmBinary, jsCode) {
    try {
      ...
      if (jsCode && !globalThis.createRobHubbard) {
        const wrappedCode = jsCode + '\nreturn createRobHubbard;';
        const factory = new Function(wrappedCode);
        const result = factory();
        if (typeof result === 'function') {
          globalThis.createRobHubbard = result;
        }
      }

      if (!globalThis.createRobHubbard) {
        throw new Error('createRobHubbard factory not available');
      }

      // Instantiate WASM module
      this.wasm = await globalThis.createRobHubbard({
        wasmBinary: wasmBinary,
      });

      // Create the global synthesis context
      this.ctx = this.wasm._rh_init(Math.floor(sr));
      if (!this.ctx) {
        throw new Error('rh_init returned null');
      }

      this.initialized = true;
      this.port.postMessage({ type: 'ready' });
    } catch (err) {
      console.error('[RobHubbard Worklet] Init failed:', err);
  ...
  process(_inputs, outputs, _parameters) {
    ...
    for (const h of Object.keys(this.players)) {
      ...
      this.wasm._rh_render(this.ctx, hi, ptrs.outPtrL, ptrs.outPtrR, numSamples);
...
registerProcessor('rob-hubbard-processor', RobHubbardProcessor);
