/**
 * Qsf.worklet.js - AudioWorklet processor for QSF (Capcom QSound) WASM player
 *
 * The WASM qsf_wasm_render() writes interleaved stereo float (LRLRLR...) into
 * a buffer. We deinterleave into the WebAudio output channels.
 */

class QsfProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
  ...
  async handleMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'loadModule':
        if (this.module && typeof this.module._qsf_wasm_load === 'function') {
          try {
            ...
            // Detect WASM memory growth
            let heapU8 = this.module.HEAPU8 || (this.module.wasmMemory && new Uint8Array(this.module.wasmMemory.buffer));
            if (!heapU8) {
            ...
            // Re-check after malloc in case of heap growth
            if (this.module.wasmMemory) {
              heapU8 = new Uint8Array(this.module.wasmMemory.buffer);
            }

            heapU8.set(uint8Data, wasmPtr);
            const result = this.module._qsf_wasm_load(wasmPtr, uint8Data.length);
            const free = this.module._free || this.module.free;
            if (free) free(wasmPtr);

            if (result === 0) {
              // Re-allocate interleaved buffer in case of heap growth
              this.updateBufferViews();
              this.port.postMessage({ type: 'moduleLoaded' });
            } else {
              this.port.postMessage({ type: 'error', message: 'qsf_wasm_load failed with code ' + result });
            }
          } catch (error) {
      ...
      case 'play':
        /* Loading automatically starts playback in the WASM bridge */
        break;

      case 'stop':
        if (this.module && typeof this.module._qsf_wasm_stop === 'function') {
          this.module._qsf_wasm_stop();
  ...
  async initModule(sampleRate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (typeof globalThis.Qsf !== 'function') {
        this.port.postMessage({ type: 'error', message: 'Qsf factory not available' });
        return;
      }
      ...
      try {
        this.module = await globalThis.Qsf(config);
      } finally {
      ...
      // Initialize QSF engine with worklet sample rate
      if (typeof this.module._qsf_wasm_init === 'function') {
        this.module._qsf_wasm_init(sampleRate);
      }
  ...
  updateBufferViews() {
    ...
    const heapF32 = this.module.HEAPF32 || (mem && new Float32Array(mem.buffer));
    if (!heapF32) return;
  ...
  cleanup() {
    if (this.module) {
      if (typeof this.module._qsf_wasm_dispose === 'function') {
        this.module._qsf_wasm_dispose();
      }
      const free = this.module._free || this.module.free;
  ...
  process(inputs, outputs, parameters) {
    ...
    if (typeof this.module._qsf_wasm_render === 'function') {
      this.updateBufferViews();
      if (this.interleavedBuf) {
        const rendered = this.module._qsf_wasm_render(this.interleavedPtr, numSamples);
        if (rendered > 0) {
...
registerProcessor('qsf-processor', QsfProcessor);
