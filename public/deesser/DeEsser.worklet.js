/**
 * DeEsser.worklet.js — AudioWorklet processor for DeEsser WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createDeEsser;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createDeEsser factory');
...
class DeEsserProcessor extends AudioWorkletProcessor {
  ...
  applyParam(param, value) {
    ...
    const fn = M['_deesser_set_' + param];
    if (fn) fn(h, value);
  }
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._deesser_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._deesser_create(sampleRate);
      if (this.handle < 0) throw new Error('deesser_create failed');
      const bs = 128 * 4;
      ...
      console.error('[DeEsser] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._deesser_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
      outL.set(this.outputBufferL.subarray(0, n)); outR.set(this.outputBufferR.subarray(0, n));
    } catch (err) { outL.set(inL); outR.set(inR); }
    return true;
  }
}
if (!processorRegistered) { registerProcessor('deesser-processor', DeEsserProcessor); processorRegistered = true; }
