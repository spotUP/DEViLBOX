/**
 * SwedishChainsaw.worklet.js — AudioWorklet processor for Swedish Chainsaw HM-2 distortion
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try {
      const wrappedCode = `${jsCode}; return createSwedishChainsawModule;`;
      createModule = new Function(wrappedCode)();
    } catch (evalErr) {
      console.error('Failed to evaluate SwedishChainsaw JS:', evalErr);
      sharedModulePromise = null;
      throw new Error('Could not evaluate SwedishChainsaw module factory');
    }

    if (!createModule) {
      sharedModulePromise = null;
      throw new Error('Could not load SwedishChainsaw module factory');
    }
...
class SwedishChainsawProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  async initialize(data) {
    try {
      ...
      this.effect = new Module.SwedishChainsaw();
      ...
      console.error('SwedishChainsaw initialization error:', error);
...
if (!processorRegistered) {
  registerProcessor('swedishchainsaw-processor', SwedishChainsawProcessor);
