/**
 * ActivisionPro.worklet.js — AudioWorklet processor for ActivisionPro WASM replayer
 * Whole-song replayer with per-channel output for oscilloscope.
 */
class ActivisionProProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
    this.handle = 0;
  ...
  async handleMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'loadModule': {
        ...
        try {
          if (this.handle) { this.module._avp_destroy(this.handle); this.handle = 0; }
          ...
          this.handle = this.module._avp_create(wasmPtr, uint8Data.length, sampleRate);
          this.module._free(wasmPtr);
          if (!this.handle) { this.port.postMessage({ type: 'error', message: 'avp_create failed (unsupported format)' }); return; }
          const subsongCount = this.module._avp_subsong_count(this.handle);
          if (typeof data.subsong === 'number' && data.subsong > 0) this.module._avp_select_subsong(this.handle, data.subsong);
          this.playing = false;
      ...
      case 'stop':
        this.playing = false;
        if (this.handle) this.module._avp_select_subsong(this.handle, 0);
        this.port.postMessage({ type: 'stopped' });
        break;
      case 'pause': this.playing = !this.playing; break;
      case 'setSubsong': if (this.handle) this.module._avp_select_subsong(this.handle, data.subsong); break;
      case 'setChannelMask': if (this.handle) this.module._avp_set_channel_mask(this.handle, data.mask); break;
      case 'setCell': {        if (this.handle && this.module._avp_set_cell) {          this.module._avp_set_cell(this.handle, data.index, data.row, data.channel, data.note, data.instrument, data.effect, data.effectArg);        }        break;      }      case 'setInstrumentParam': {        if (this.handle && this.module._avp_set_instrument_param) {          var pLen = this.module.lengthBytesUTF8(data.param) + 1;          var pPtr = this.module._malloc(pLen);          this.module.stringToUTF8(data.param, pPtr, pLen);          this.module._avp_set_instrument_param(this.handle, data.instrument, pPtr, data.value);          this.module._free(pPtr);        }        break;      }            case 'getInstrumentParam': {
        if (!this.handle || !this.module) break;
        const m = this.module;
        const fn = m._avp_get_instrument_param;
        if (!fn) break;
  ...
  async initWasm(rate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.createActivisionPro) {
        const wrappedCode = jsCode + '\nreturn createActivisionPro;';
        const factory = new Function(wrappedCode);
        const result = factory();
        if (typeof result === 'function') globalThis.createActivisionPro = result;
        else { this.port.postMessage({ type: 'error', message: 'Failed to load JS module' }); return; }
      }
      if (typeof globalThis.createActivisionPro !== 'function') { this.port.postMessage({ type: 'error', message: 'createActivisionPro factory not available' }); return; }
      ...
      try { this.module = await globalThis.createActivisionPro(config); } finally { WebAssembly.instantiate = origInstantiate; }
  ...
  cleanup() {
    if (this.module && this.handle) { this.module._avp_destroy(this.handle); this.handle = 0; }
    if (this.module) {
  ...
  process(inputs, outputs) {
    ...
    // Render per-channel
    const rendered = this.module._avp_render_multi(
    ...
    if (this.module._avp_has_ended(this.handle)) this.port.postMessage({ type: 'songEnd' });
    return true;
  }
}
registerProcessor('activisionpro-processor', ActivisionProProcessor);
