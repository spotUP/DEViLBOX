/**
 * MultibandEnhancer.worklet.js — AudioWorklet processor for MultibandEnhancer WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createMultibandEnhancer;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createMultibandEnhancer factory');
...
class MultibandEnhancerProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    const fn = M['_multiband_enhancer_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._multiband_enhancer_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._multiband_enhancer_create(sampleRate);
      if (this.handle < 0) throw new Error('multiband_enhancer_create failed');
      const bs = 128 * 4;
      ...
      console.error('[MultibandEnhancer] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._multiband_enhancer_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('multiband-enhancer-processor', MultibandEnhancerProcessor); processorRegistered = true; }
