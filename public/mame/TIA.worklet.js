 ...
 * Atari 2600 Television Interface Adaptor Sound for DEViLBOX
 *
 * 4-voice polyphonic synthesizer using polynomial counter-based synthesis
 * with 16 audio control modes (AUDC), 3 polynomial counters (4/5/9-bit),
 * and pure tone generation with 5-bit frequency divider.
 *
 * The distinctive "Atari sound" - buzzy, crunchy, and lo-fi.
 */
...
class TIAProcessor extends AudioWorkletProcessor {
  ...
  async initSynth(sampleRate) {
    try {
      ...
      const moduleFactory = await import(`${BASE_URL}mame/TIA.js`);
      this.module = await moduleFactory.default();

      this.synth = new this.module.TIASynth();
      ...
      console.error('TIA init error:', error);
