 ...
 * Atari 2600 TIA sound (two channels, polynomial noise/tone AUDC modes)
 * for DEViLBOX. Based on the MAME emulator (Ron Fries' TIASound core).
 */
...
class TIAProcessor extends AudioWorkletProcessor {
  ...
  async initSynth(data) {
    try {
      ...
      this.module = await globalThis.initMAMEWasmModule(data.wasmBinary, data.jsCode, 'createTIAModule');
