...
/**
 * VLM5030 AudioWorklet Processor
 * Sanyo VLM5030 LPC Speech Synthesizer (Konami arcade) for DEViLBOX
 *
 * 4-voice polyphonic 10-pole LPC lattice filter speech synth.
 * Based on MAME emulator by Tatsuyuki Satoh.
 */

class VLM5030Processor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.synth = null;
    this.module = null;
  ...
  async initSynth(data) {
    try {
      ...
      this.module = await globalThis.initMAMEWasmModule(data.wasmBinary, data.jsCode, 'createVLM5030Module');

      this.synth = new this.module.VLM5030Synth();
      ...
      console.error('VLM5030 init error:', error);
...
registerProcessor('vlm5030-processor', VLM5030Processor);
