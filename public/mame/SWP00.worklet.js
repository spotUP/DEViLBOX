...
/**
 * Yamaha SWP00 AWM2 AudioWorklet Processor
 *
...
class SWP00Processor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.synth = null;
    this.module = null;
      ...
      case 'loadSample':
        if (this.synth && this.module && data.data) {
          if (!this.module.wasmMemory) {
            console.error('[SWP00 Worklet] WASM memory not available');
            break;
          }
  ...
  async initSynth(data) {
    try {
      this.cleanup();
      this.module = await globalThis.initMAMEWasmModule(data.wasmBinary, data.jsCode, 'createSWP00Module');
      this.synth = new this.module.SWP00Synth();
      ...
      console.error('[SWP00 Worklet] init error:', error);
...
registerProcessor('swp00-processor', SWP00Processor);
