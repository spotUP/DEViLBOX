...
class SCSPProcessor extends AudioWorkletProcessor {
    ...
    switch (data.type) {
      ...
      case 'loadSample':
        if (this.synth && this.module) {
          // Copy sample data to WASM memory
      ...
      case 'configureSlot':
        if (this.synth) {
          this.synth.configureSlot(data.slot, data.sampleAddr, data.loopStart, data.loopEnd, data.loop, data.is8bit);
        }
        break;
      case 'controlChange':
        if (this.synth) {
  ...
  async initSynth(sampleRate) {
    try {
      ...
      console.error('SCSP init error:', error);
...
registerProcessor('scsp-processor', SCSPProcessor);
