/**
 * Kuiza.worklet.js — AudioWorklet processor for Kuiza WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createKuiza;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createKuiza factory');
...
class KuizaProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'low': M['_kuiza_set_low'](h, value); break;
      case 'lowMid': M['_kuiza_set_lowMid'](h, value); break;
      case 'highMid': M['_kuiza_set_highMid'](h, value); break;
      case 'high': M['_kuiza_set_high'](h, value); break;
      case 'gain': M['_kuiza_set_gain'](h, value); break;
      case 'mix': M['_kuiza_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_kuiza_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_kuiza_create'](sampleRate);
      if (this.handle < 0) throw new Error('kuiza_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Kuiza] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_kuiza_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('kuiza-processor', KuizaProcessor); processorRegistered = true; }
