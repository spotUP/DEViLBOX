...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try { const wrappedCode = `${jsCode}; return createDistortionShaper;`; createModule = new Function(wrappedCode)(); }
    catch (e) { console.error('[DistortionShaper] Failed:', e); sharedModulePromise = null; throw e; }
...
class DistortionShaperProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParameter(param, value) {
    ...
    const fn = M['_distortion_shaper_set_' + param];
    if (fn) fn(h, value);
  }
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._distortion_shaper_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = Module._distortion_shaper_create(sampleRate);
      ...
      console.error('[DistortionShaper] init error:', error);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._distortion_shaper_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('distortion-shaper-processor', DistortionShaperProcessor); processorRegistered = true; }
