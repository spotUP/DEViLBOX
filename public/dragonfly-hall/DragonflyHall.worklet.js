/**
 * DragonflyHall.worklet.js — AudioWorklet processor for DragonflyHall WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createDragonflyHall;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createDragonflyHall factory');
...
class DragonflyHallProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    const fn = M['_dragonfly_hall_set_' + param];
    if (fn) fn(h, value);
  }
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._dragonfly_hall_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._dragonfly_hall_create(sampleRate);
      if (this.handle < 0) throw new Error('dragonfly_hall_create failed');
      const bs = 128 * 4;
      ...
      console.error('[DragonflyHall] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._dragonfly_hall_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
      outL.set(this.outputBufferL.subarray(0, n)); outR.set(this.outputBufferR.subarray(0, n));
    } catch (err) { outL.set(inL); outR.set(inR); }
    return true;
  }
}
if (!processorRegistered) { registerProcessor('dragonfly-hall-processor', DragonflyHallProcessor); processorRegistered = true; }
