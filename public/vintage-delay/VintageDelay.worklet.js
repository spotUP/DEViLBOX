...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try {
      const wrappedCode = `${jsCode}; return createVintageDelay;`;
      createModule = new Function(wrappedCode)();
    } catch (e) { console.error('[VintageDelay] Failed:', e); sharedModulePromise = null; throw e; }
...
class VintageDelayProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParameter(param, value) {
    ...
    const fn = M['_vintage_delay_set_' + param];
    if (fn) fn(h, value);
  }
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._vintage_delay_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = Module._vintage_delay_create(sampleRate);
      ...
      console.error('[VintageDelay] init error:', error);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._vintage_delay_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('vintage-delay-processor', VintageDelayProcessor); processorRegistered = true; }
