...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createSidechainGate;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createSidechainGate factory');
...
class SidechainGateProcessor extends AudioWorkletProcessor {
  ...
  async handleMessage(event) {
    ...
    switch (type) {
      ...
      case 'parameter':
        if (this.isInitialized && this.handle >= 0) this.Module['_sidechain_gate_set_' + data.param](this.handle, data.value);
  ...
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._sidechain_gate_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._sidechain_gate_create(sampleRate);
      if (this.handle < 0) throw new Error('sidechain_gate_create failed');
      const bs = 128 * 4;
      ...
      console.error('[SidechainGate] init error:', e);
...
if (!processorRegistered) { registerProcessor('sidechain-gate-processor', SidechainGateProcessor); processorRegistered = true; }
