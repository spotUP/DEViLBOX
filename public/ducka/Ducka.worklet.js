/**
 * Ducka.worklet.js — AudioWorklet processor for Ducka WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createDucka;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createDucka factory');
...
class DuckaProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    const fn = M['_ducka_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._ducka_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._ducka_create(sampleRate);
      if (this.handle < 0) throw new Error('ducka_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Ducka] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._ducka_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('ducka-processor', DuckaProcessor); processorRegistered = true; }
