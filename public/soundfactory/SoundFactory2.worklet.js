/**
 * SoundFactory2.worklet.js — AudioWorklet processor for SoundFactory2 WASM replayer
 * Whole-song replayer with per-channel output for oscilloscope.
 */
class SoundFactory2Processor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
    this.handle = 0;
  ...
  async handleMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'loadModule': {
        ...
        try {
          if (this.handle) { this.module._sf_destroy(this.handle); this.handle = 0; }
          ...
          this.handle = this.module._sf_create(wasmPtr, uint8Data.length, sampleRate);
          this.module._free(wasmPtr);
          if (!this.handle) { this.port.postMessage({ type: 'error', message: 'sf_create failed (unsupported format)' }); return; }
          const subsongCount = this.module._sf_subsong_count(this.handle);
          if (typeof data.subsong === 'number' && data.subsong > 0) this.module._sf_select_subsong(this.handle, data.subsong);
          this.playing = false;
      ...
      case 'stop':
        this.playing = false;
        if (this.handle) this.module._sf_select_subsong(this.handle, 0);
  ...
  cleanup() {
    if (this.module && this.handle) { this.module._sf_destroy(this.handle); this.handle = 0; }
    if (this.module) {
  ...
  process(inputs, outputs) {
    ...
    // Render per-channel
    const rendered = this.module._sf_render_multi(
    ...
    if (this.module._sf_has_ended(this.handle)) this.port.postMessage({ type: 'songEnd' });
    return true;
  }
}
registerProcessor('soundfactory-processor', SoundFactory2Processor);
