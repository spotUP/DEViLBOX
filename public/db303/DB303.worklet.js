    ...
    switch (data.type) {
      ...
      case 'noteOn':
        if (this.synth) {
          ...
          // DB303 internally uses a noteList to decide trigger vs slide.
          ...
          if (!data.slide && this.currentNote >= 0) {
            console.log('[DB303] TRIGGER: noteOff(' + this.currentNote + ') then noteOn(' + data.note + ') vel=' + data.velocity);
            this.synth.noteOff(this.currentNote);
          } else if (data.slide && this.currentNote >= 0) {
            console.log('[DB303] SLIDE: noteOn(' + data.note + ') over held note ' + this.currentNote + ' vel=' + data.velocity);
          } else {
            console.log('[DB303] FIRST NOTE: noteOn(' + data.note + ') vel=' + data.velocity);
          }
      ...
      case 'noteOff':
        if (this.synth) {
          ...
          if (data.note > 0 && data.note === this.currentNote) {
            console.log('[DB303] RELEASE: noteOff(' + data.note + ')');
      ...
      case 'gateOff':
        if (this.synth && this.currentNote >= 0) {
          ...
          console.log('[DB303] GATE OFF: noteOff(' + this.currentNote + ')');
          this.synth.noteOff(this.currentNote);
          this.currentNote = -1;
        }
        break;
      case 'allNotesOff':
        if (this.synth) {
          console.log('[DB303] ALL NOTES OFF');
