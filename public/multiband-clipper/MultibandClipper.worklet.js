/**
 * MultibandClipper.worklet.js — AudioWorklet processor for MultibandClipper WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createMultibandClipper;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createMultibandClipper factory');
...
class MultibandClipperProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'lowCross': M['_multiband_clipper_set_lowCross'](h, value); break;
      case 'highCross': M['_multiband_clipper_set_highCross'](h, value); break;
      case 'lowCeil': M['_multiband_clipper_set_lowCeil'](h, value); break;
      case 'midCeil': M['_multiband_clipper_set_midCeil'](h, value); break;
      case 'highCeil': M['_multiband_clipper_set_highCeil'](h, value); break;
      case 'softness': M['_multiband_clipper_set_softness'](h, value); break;
      case 'mix': M['_multiband_clipper_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_multiband_clipper_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_multiband_clipper_create'](sampleRate);
      if (this.handle < 0) throw new Error('multiband_clipper_create failed');
      const bs = 128 * 4;
      ...
      console.error('[MultibandClipper] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_multiband_clipper_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('multiband-clipper-processor', MultibandClipperProcessor); processorRegistered = true; }
