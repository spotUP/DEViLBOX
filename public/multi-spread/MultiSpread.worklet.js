/**
 * MultiSpread.worklet.js — AudioWorklet processor for MultiSpread WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createMultiSpread;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createMultiSpread factory');
...
class MultiSpreadProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    const fn = M['_multi_spread_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._multi_spread_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._multi_spread_create(sampleRate);
      if (this.handle < 0) throw new Error('multi_spread_create failed');
      const bs = 128 * 4;
      ...
      console.error('[MultiSpread] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._multi_spread_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('multi-spread-processor', MultiSpreadProcessor); processorRegistered = true; }
