/**
 * RingMod.worklet.js — AudioWorklet processor for Ring Modulator WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createRingMod;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createRingMod factory');
...
class RingModProcessor extends AudioWorkletProcessor {
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'frequency': M._ring_mod_set_frequency(h, value); break;
      case 'mix':       M._ring_mod_set_mix(h, value);       break;
      case 'waveform':  M._ring_mod_set_waveform(h, value);  break;
      case 'lfoRate':   M._ring_mod_set_lfo_rate(h, value);  break;
      case 'lfoDepth':  M._ring_mod_set_lfo_depth(h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._ring_mod_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._ring_mod_create(sampleRate);
      if (this.handle < 0) throw new Error('ring_mod_create failed');
      const bs = 128 * 4;
      ...
      console.error('[RingMod] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._ring_mod_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('ring-mod-processor', RingModProcessor); processorRegistered = true; }
