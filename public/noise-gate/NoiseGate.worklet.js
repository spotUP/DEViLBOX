/**
 * NoiseGate.worklet.js — AudioWorklet processor for NoiseGate WASM effect.
 *
 * Loads the Emscripten-compiled NoiseGate WASM module and processes
 * stereo audio through it. Uses plain C API (not Embind).
 *
 * WASM binary and JS code are received via postMessage from the main thread.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try {
      const wrappedCode = `${jsCode}; return createNoiseGate;`;
      createModule = new Function(wrappedCode)();
    } catch (e) {
      console.error('[NoiseGate] Failed to evaluate JS:', e);
    ...
    if (!createModule) {
      sharedModulePromise = null;
      throw new Error('Could not load NoiseGate module factory');
    }

    // Capture WASM memory for buffer views
...
class NoiseGateProcessor extends AudioWorkletProcessor {
    ...
    switch (type) {
      ...
      case 'parameter':
        if (this.isInitialized && this.handle >= 0) {
          this.applyParameter(data.param, data.value);
        } else {
  ...
  applyParameter(param, value) {
    ...
    const h = this.handle;
    switch (param) {
      case 'threshold': M._noise_gate_set_threshold(h, value); break;
      case 'attack':    M._noise_gate_set_attack(h, value);    break;
      case 'hold':      M._noise_gate_set_hold(h, value);      break;
      case 'release':   M._noise_gate_set_release(h, value);   break;
      case 'range':     M._noise_gate_set_range(h, value);     break;
      case 'hpf':       M._noise_gate_set_hpf(h, value);       break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._noise_gate_destroy(this.handle);
    ...
    this.handle = -1;
    this.inputPtrL = this.inputPtrR = 0;
    this.outputPtrL = this.outputPtrR = 0;
    this.inputBufferL = this.inputBufferR = null;
    this.outputBufferL = this.outputBufferR = null;
    this.isInitialized = false;
  }
  ...
  async initialize(data) {
    try {
      ...
      this.handle = Module._noise_gate_create(sampleRate);
      if (this.handle < 0) throw new Error('noise_gate_create returned ' + this.handle);

      // Allocate stereo I/O buffers
      const bufBytes = 128 * 4;
      this.inputPtrL  = Module._malloc(bufBytes);
      this.inputPtrR  = Module._malloc(bufBytes);
      this.outputPtrL = Module._malloc(bufBytes);
      this.outputPtrR = Module._malloc(bufBytes);

      const wasmMem = Module.wasmMemory;
      const heapBuf = Module.HEAPF32
        ? Module.HEAPF32.buffer
        : (wasmMem ? wasmMem.buffer : null);
      if (!heapBuf) throw new Error('Cannot access WASM memory');
      ...
      console.error('[NoiseGate] init error:', error);
  ...
  process(inputs, outputs) {
    ...
    try {
      // Recreate views if WASM memory resized
      const curBuf = this._wasmMemory
      ...
      this.inputBufferL.set(inputL.subarray(0, n));
      this.inputBufferR.set(inputR.subarray(0, n));

      this.Module._noise_gate_process(
        this.handle,
      ...
      outputL.set(this.outputBufferL.subarray(0, n));
      outputR.set(this.outputBufferR.subarray(0, n));
