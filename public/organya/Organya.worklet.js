/**
 * Organya.worklet.js - AudioWorklet processor for Organya WASM player
 *
 * Supports Cave Story music (.org) files.
 * The WASM organya_render() writes interleaved stereo (LRLRLR...) into a
 * single float buffer. We deinterleave into the WebAudio output channels.
 */

class OrganyaProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
  ...
  async handleMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'loadModule':
        if (this.module && typeof this.module._organya_init === 'function') {
          try {
            ...
            const result = this.module._organya_init(wasmPtr, uint8Data.length);
            const free = this.module._free || this.module.free;
            if (free) free(wasmPtr);

            if (result !== 0) {
              this.port.postMessage({ type: 'error', message: 'organya_init failed: ' + result });
              return;
            }

            // Get metadata
            const meta = {};
            if (typeof this.module._organya_get_tempo === 'function') {
              meta.tempo = this.module._organya_get_tempo();
            }
            if (typeof this.module._organya_get_num_channels === 'function') {
              meta.channels = this.module._organya_get_num_channels();
            }
      ...
      case 'stop':
        if (this.module && typeof this.module._organya_stop === 'function') {
          this.module._organya_stop();
  ...
  async initModule(sampleRate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.Organya) {
        ...
        const wrappedCode = jsCode + '\nreturn createOrganya;';
        const factory = new Function(wrappedCode);
        const result = factory();

        if (typeof result === 'function') {
          globalThis.Organya = result;
        } else {
      ...
      if (typeof globalThis.Organya !== 'function') {
        this.port.postMessage({ type: 'error', message: 'Organya factory not available' });
        return;
      }
      ...
      try {
        this.module = await globalThis.Organya(config);
      } finally {
      ...
      // Set sample rate
      if (typeof this.module._organya_set_sample_rate === 'function') {
        this.module._organya_set_sample_rate(sampleRate);
      }
  ...
  cleanup() {
    if (this.module && typeof this.module._organya_stop === 'function') {
      try { this.module._organya_stop(); } catch (e) {}
    }
    const free = this.module?._free || this.module?.free;
  ...
  process(inputs, outputs, parameters) {
    ...
    if (typeof this.module._organya_render === 'function') {
      this.updateBufferViews();
      if (this.interleavedBuf) {
        const rendered = this.module._organya_render(this.interleavedPtr, numSamples);
        if (rendered > 0) {
...
registerProcessor('organya-processor', OrganyaProcessor);
