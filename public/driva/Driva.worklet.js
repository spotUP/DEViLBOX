/**
 * Driva.worklet.js — AudioWorklet processor for Driva WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createDriva;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createDriva factory');
...
class DrivaProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'amount': M['_driva_set_amount'](h, value); break;
      case 'tone': M['_driva_set_tone'](h, value); break;
      case 'mix': M['_driva_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_driva_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_driva_create'](sampleRate);
      if (this.handle < 0) throw new Error('driva_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Driva] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_driva_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('driva-processor', DrivaProcessor); processorRegistered = true; }
