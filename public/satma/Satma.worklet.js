...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try { const wrappedCode = `${jsCode}; return createSatma;`; createModule = new Function(wrappedCode)(); }
    catch (e) { console.error('[Satma] Failed:', e); sharedModulePromise = null; throw e; }
...
class SatmaProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParameter(param, value) {
    ...
    const fn = M['_satma_set_' + param];
    if (fn) fn(h, value);
  }
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._satma_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = Module._satma_create(sampleRate);
      ...
      console.error('[Satma] init error:', error);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._satma_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('satma-processor', SatmaProcessor); processorRegistered = true; }
