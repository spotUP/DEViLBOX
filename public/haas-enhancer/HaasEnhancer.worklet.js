/**
 * HaasEnhancer.worklet.js — AudioWorklet processor for HaasEnhancer WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createHaasEnhancer;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createHaasEnhancer factory');
...
class HaasEnhancerProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    const fn = M['_haas_enhancer_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._haas_enhancer_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._haas_enhancer_create(sampleRate);
      if (this.handle < 0) throw new Error('haas_enhancer_create failed');
      const bs = 128 * 4;
      ...
      console.error('[HaasEnhancer] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._haas_enhancer_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('haas-enhancer-processor', HaasEnhancerProcessor); processorRegistered = true; }
