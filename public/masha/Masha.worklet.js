/**
 * Masha.worklet.js — AudioWorklet processor for Masha WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createMasha;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createMasha factory');
...
class MashaProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    const fn = M['_masha_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._masha_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._masha_create(sampleRate);
      if (this.handle < 0) throw new Error('masha_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Masha] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._masha_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('masha-processor', MashaProcessor); processorRegistered = true; }
