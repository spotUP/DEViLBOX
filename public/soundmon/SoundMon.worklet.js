/**
 * SoundMon.worklet.js — AudioWorklet processor for SoundMon WASM replayer
 * Whole-song replayer with per-channel output for oscilloscope.
 */
class SoundMonProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
    this.handle = 0;
  ...
  async handleMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'loadModule': {
        ...
        try {
          if (this.handle) { this.module._sm_destroy(this.handle); this.handle = 0; }
          ...
          this.handle = this.module._sm_create(wasmPtr, uint8Data.length, sampleRate);
          this.module._free(wasmPtr);
          if (!this.handle) { this.port.postMessage({ type: 'error', message: 'sm_create failed (unsupported format)' }); return; }
          const subsongCount = this.module._sm_subsong_count(this.handle);
          if (typeof data.subsong === 'number' && data.subsong > 0) this.module._sm_select_subsong(this.handle, data.subsong);
          this.playing = false;
      ...
      case 'stop':
        this.playing = false;
        if (this.handle) this.module._sm_select_subsong(this.handle, 0);
        this.port.postMessage({ type: 'stopped' });
        break;
      case 'pause': this.playing = !this.playing; break;
      case 'setSubsong': if (this.handle) this.module._sm_select_subsong(this.handle, data.subsong); break;
      case 'setChannelMask': if (this.handle) this.module._sm_set_channel_mask(this.handle, data.mask); break;
      case 'setCell': {        if (this.handle && this.module._sm_set_cell) {          this.module._sm_set_cell(this.handle, data.index, data.row, data.channel, data.note, data.instrument, data.effect, data.effectArg);        }        break;      }      case 'setInstrumentParam': {        if (this.handle && this.module._sm_set_instrument_param) {          var pLen = this.module.lengthBytesUTF8(data.param) + 1;          var pPtr = this.module._malloc(pLen);          this.module.stringToUTF8(data.param, pPtr, pLen);          this.module._sm_set_instrument_param(this.handle, data.instrument, pPtr, data.value);          this.module._free(pPtr);        }        break;      }      case 'dispose': this.cleanup(); break;
    }
  }
  ...
  async initWasm(rate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.createSoundMon) {
        const wrappedCode = jsCode + '\nreturn createSoundMon;';
        const factory = new Function(wrappedCode);
        const result = factory();
        if (typeof result === 'function') globalThis.createSoundMon = result;
        else { this.port.postMessage({ type: 'error', message: 'Failed to load JS module' }); return; }
      }
      if (typeof globalThis.createSoundMon !== 'function') { this.port.postMessage({ type: 'error', message: 'createSoundMon factory not available' }); return; }
      ...
      try { this.module = await globalThis.createSoundMon(config); } finally { WebAssembly.instantiate = origInstantiate; }
  ...
  cleanup() {
    if (this.module && this.handle) { this.module._sm_destroy(this.handle); this.handle = 0; }
    if (this.module) {
  ...
  process(inputs, outputs) {
    ...
    // Render per-channel
    const rendered = this.module._sm_render_multi(
    ...
    if (this.module._sm_has_ended(this.handle)) this.port.postMessage({ type: 'songEnd' });
    return true;
  }
}
registerProcessor('soundmon-processor', SoundMonProcessor);
