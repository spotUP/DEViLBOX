/**
 * ResonanceTamer.worklet.js — AudioWorklet host for the master-bus resonance
 * suppressor. Receives blocks of 128 samples from Web Audio, hands them to
 * the WASM effect which runs an internal STFT (2048 / hop 1024) pipeline.
 *
 * Message contract:
 ...
 *
 * Shares a single WASM module across processors (pattern matches the
 * DattorroPlate / SoundTouch worklets already in public/).
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try {
      const wrappedCode = `${jsCode}; return createResonanceTamerModule;`;
      createModule = new Function(wrappedCode)();
    } catch (evalErr) {
      console.error('Failed to evaluate ResonanceTamer JS:', evalErr);
      sharedModulePromise = null;
      throw new Error('Could not evaluate ResonanceTamer module factory');
    }
    if (!createModule) {
      sharedModulePromise = null;
      throw new Error('Could not load ResonanceTamer module factory');
    }
...
class ResonanceTamerProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.effect = null;
  ...
  async initialize(data) {
    try {
      ...
      this.effect = new Module.ResonanceTamerEffect();
      ...
      console.error('ResonanceTamer initialization error:', error);
  ...
  process(inputs, outputs) {
    ...
    try {
      // Recreate views if WASM memory was resized (grew).
...
if (!processorRegistered) {
  registerProcessor('resonance-tamer-processor', ResonanceTamerProcessor);
