/**
 * FredEditorReplayer.worklet.js - AudioWorklet processor for Fred Editor WASM replayer
 */

class FredEditorReplayerProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
  ...
  async initModule(sampleRate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.FredEditorReplayer) {
        ...
        const wrappedCode = jsCode + '\nreturn createFredEditorReplayer;';
        const factory = new Function(wrappedCode);
        const result = factory();

        if (typeof result === 'function') {
          globalThis.FredEditorReplayer = result;
        } else {
      ...
      if (typeof globalThis.FredEditorReplayer !== 'function') {
        this.port.postMessage({ type: 'error', message: 'FredEditorReplayer factory not available' });
        return;
      }
      ...
      try {
        this.module = await globalThis.FredEditorReplayer(config);
      } finally {
...
registerProcessor('fred-editor-replayer-processor', FredEditorReplayerProcessor);
