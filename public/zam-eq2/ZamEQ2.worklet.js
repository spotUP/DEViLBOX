/**
 * ZamEQ2.worklet.js — AudioWorklet processor for ZamEQ2 WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createZamEQ2;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createZamEQ2 factory');
...
class ZamEQ2Processor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    const fn = M['_zam_eq2_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._zam_eq2_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._zam_eq2_create(sampleRate);
      if (this.handle < 0) throw new Error('zam_eq2_create failed');
      const bs = 128 * 4;
      ...
      console.error('[ZamEQ2] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._zam_eq2_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('zam-eq2-processor', ZamEQ2Processor); processorRegistered = true; }
