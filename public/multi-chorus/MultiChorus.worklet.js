/**
 * MultiChorus.worklet.js — AudioWorklet processor for MultiChorus WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createMultiChorus;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createMultiChorus factory');
...
class MultiChorusProcessor extends AudioWorkletProcessor {
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'rate': M['_multi_chorus_set_rate'](h, value); break;
      case 'depth': M['_multi_chorus_set_depth'](h, value); break;
      case 'voices': M['_multi_chorus_set_voices'](h, value); break;
      case 'stereoPhase': M['_multi_chorus_set_stereoPhase'](h, value); break;
      case 'mix': M['_multi_chorus_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_multi_chorus_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_multi_chorus_create'](sampleRate);
      if (this.handle < 0) throw new Error('multi_chorus_create failed');
      const bs = 128 * 4;
      ...
      console.error('[MultiChorus] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_multi_chorus_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('multi-chorus-processor', MultiChorusProcessor); processorRegistered = true; }
