/**
 * Cinter4.worklet.js - AudioWorklet processor for Cinter4 WASM synth
 *
...
class Cinter4Processor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
  ...
  async handleMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'loadRaw':
        // Raw (non-Cinter) sample PCM for songs with raw instruments. MUST be
        // loaded BEFORE loadModule — CinterMakeInstruments reads it at synth time.
        if (this.module && typeof this.module._player_load_raw === 'function') {
          try {
            const raw = new Uint8Array(data.rawData);
            const malloc = this.module._malloc || this.module.malloc;
            const free = this.module._free || this.module.free;
            if (malloc && raw.length > 0) {
              const ptr = malloc(raw.length);
              const heapU8 = this.module.HEAPU8 || (this.module.wasmMemory && new Uint8Array(this.module.wasmMemory.buffer));
              if (ptr && heapU8) {
                heapU8.set(raw, ptr);
                this.module._player_load_raw(ptr, raw.length, data.offset || 0);
              }
              if (free && ptr) free(ptr);
            }
            this.port.postMessage({ type: 'rawLoaded' });
          } catch (error) {
  ...
  async initModule(sampleRate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.Cinter4) {
        ...
        const wrappedCode = jsCode + '\nreturn createCinter4;';
        const factory = new Function(wrappedCode);
        const result = factory();

        if (typeof result === 'function') {
          globalThis.Cinter4 = result;
        } else {
      ...
      if (typeof globalThis.Cinter4 !== 'function') {
        this.port.postMessage({ type: 'error', message: 'Cinter4 factory not available' });
        return;
      }
      ...
      try {
        this.module = await globalThis.Cinter4(config);
      } finally {
      ...
      } else if (typeof this.module._player_init === 'function') {
        this.module._player_init(sampleRate);
      }
...
registerProcessor('cinter4-processor', Cinter4Processor);
