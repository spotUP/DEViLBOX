/**
 * SidMon1Replayer.worklet.js - AudioWorklet processor for SidMon 1.0 WASM song replayer
 * Follows the SteveTurner.worklet.js pattern exactly.
 */

class SidMon1ReplayerProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
  ...
  async handleMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'loadModule':
        if (this.module && typeof this.module._player_load === 'function') {
          try {
            const uint8Data = new Uint8Array(data.moduleData);
            const malloc = this.module._malloc || this.module.malloc;
            if (!malloc) { this.port.postMessage({ type: 'error', message: 'malloc not available' }); return; }
      ...
      case 'pause':
        if (this.module && typeof this.module._player_pause === 'function') {
          this.module._player_pause();
        }
        break;

      case 'resume':
        if (this.module && typeof this.module._player_resume === 'function') {
          this.module._player_resume();
        }
        break;
      ...
      case 'getInstrumentParam':
        if (this.module && typeof this.module._player_get_instrument_param === 'function') {
          const value = this.module._player_get_instrument_param(data.inst, data.paramId);
      ...
      case 'setInstrumentParam':
        if (this.module && typeof this.module._player_set_instrument_param === 'function') {
          this.module._player_set_instrument_param(data.inst, data.paramId, data.value);
        }
        break;

      case 'getNumInstruments':
        if (this.module && typeof this.module._player_get_num_instruments === 'function') {
          const count = this.module._player_get_num_instruments();
          this.port.postMessage({ type: 'numInstruments', count, requestId: data.requestId });
        }
        break;
  ...
  async initModule(sampleRate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.SidMon1Replayer) {
        ...
        const wrappedCode = jsCode + '\nreturn createSidMon1Replayer;';
        const factory = new Function(wrappedCode);
        const result = factory();

        if (typeof result === 'function') {
          globalThis.SidMon1Replayer = result;
        } else {
      ...
      if (typeof globalThis.SidMon1Replayer !== 'function') {
        this.port.postMessage({ type: 'error', message: 'SidMon1Replayer factory not available' });
        return;
      }
      ...
      try {
        this.module = await globalThis.SidMon1Replayer(config);
      } finally {
...
registerProcessor('sidmon1-replayer-processor', SidMon1ReplayerProcessor);
