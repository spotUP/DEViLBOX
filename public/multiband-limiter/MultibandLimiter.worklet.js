/**
 * MultibandLimiter.worklet.js — AudioWorklet processor for MultibandLimiter WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createMultibandLimiter;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createMultibandLimiter factory');
...
class MultibandLimiterProcessor extends AudioWorkletProcessor {
  ...
  async handleMessage(event) {
    ...
    switch (type) {
      ...
      case 'parameter':
        if (this.isInitialized && this.handle >= 0) this.Module['_multiband_limiter_set_' + data.param](this.handle, data.value);
  ...
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._multiband_limiter_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._multiband_limiter_create(sampleRate);
      if (this.handle < 0) throw new Error('multiband_limiter_create failed');
      const bs = 128 * 4;
      ...
      console.error('[MultibandLimiter] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._multiband_limiter_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('multiband-limiter-processor', MultibandLimiterProcessor); processorRegistered = true; }
