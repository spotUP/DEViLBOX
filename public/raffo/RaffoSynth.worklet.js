/**
 * RaffoSynth AudioWorklet Processor
 * Minimoog-style Monophonic Synthesizer for DEViLBOX
 * 4 oscillators, IIR filter, dual ADSR, glide
 */

class RaffoSynthProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.engine = null;
    this.module = null;
  ...
  async handleMessage(data) {
    switch (data.type) {
      ...
      case 'noteOn':
        if (this.engine) {
          this.module._raffo_note_on(this.engine, data.note, data.velocity);
        }
        break;
      case 'noteOff':
        if (this.engine) {
          this.module._raffo_note_off(this.engine, data.note);
        }
        break;
      case 'setParam':
        if (this.engine) {
          this.module._raffo_set_param(this.engine, data.index, data.value);
        }
        break;
      case 'pitchBend':
        if (this.engine) {
          this.module._raffo_pitch_bend(this.engine, data.value);
        }
        break;
      case 'allNotesOff':
        if (this.engine) {
          this.module._raffo_all_notes_off(this.engine);
        }
        break;
      case 'loadPatch':
        if (this.engine && data.values) {
          const numParams = this.module._raffo_get_num_params(this.engine);
          const count = Math.min(data.values.length, numParams);
          for (let i = 0; i < count; i++) {
            this.module._raffo_set_param(this.engine, i, data.values[i]);
          }
      ...
      case 'getState':
        if (this.engine) {
          ...
          for (let i = 0; i < numParams; i++) {
            state[i] = this.module._raffo_get_param(this.engine, i);
          }
      ...
      case 'getInfo':
        if (this.engine) {
          this.port.postMessage({
            type: 'info',
            numParams: this.module._raffo_get_num_params(this.engine)
          });
        }
        break;
      case 'dispose':
  ...
  async initEngine(sr, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.RaffoSynthFactory) {
        ...
        const wrappedCode = jsCode + '\nreturn createRaffoSynth;';
        const factory = new Function(wrappedCode);
        const result = factory();

        if (typeof result === 'function') {
          globalThis.RaffoSynthFactory = result;
        } else {
      ...
      if (typeof globalThis.RaffoSynthFactory !== 'function') {
        this.port.postMessage({ type: 'error', message: 'RaffoSynth factory not available' });
        return;
      }
      ...
      try {
        this.module = await globalThis.RaffoSynthFactory(config);
      } finally {
      ...
      this.engine = this.module._raffo_create(sr || sampleRate);
      ...
      this.initializing = false;
      console.error('[RaffoSynth Worklet] Init error:', error);
  ...
  cleanup() {
    if (this.module && this.engine) {
      this.module._raffo_destroy(this.engine);
      this.engine = null;
    }
    if (this.module) {
      if (this.outputPtrL) {
  ...
  process(inputs, outputs, parameters) {
    ...
    this.module._raffo_process(this.engine, this.outputPtrL, this.outputPtrR, numSamples);
...
registerProcessor('raffo-processor', RaffoSynthProcessor);
