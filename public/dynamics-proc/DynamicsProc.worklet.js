/**
 * DynamicsProc.worklet.js — AudioWorklet processor for DynamicsProc WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createDynamicsProc;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createDynamicsProc factory');
...
class DynamicsProcProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  async handleMessage(event) {
    ...
    switch (type) {
      ...
      case 'parameter':
        if (this.isInitialized && this.handle >= 0) this.Module['_dynamics_proc_set_' + data.param](this.handle, data.value);
  ...
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._dynamics_proc_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._dynamics_proc_create(sampleRate);
      if (this.handle < 0) throw new Error('dynamics_proc_create failed');
      const bs = 128 * 4;
      ...
      console.error('[DynamicsProc] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._dynamics_proc_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('dynamics-proc-processor', DynamicsProcProcessor); processorRegistered = true; }
