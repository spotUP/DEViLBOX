/**
 * Della.worklet.js — AudioWorklet processor for Della WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createDella;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createDella factory');
...
class DellaProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'time': M['_della_set_time'](h, value); break;
      case 'feedback': M['_della_set_feedback'](h, value); break;
      case 'volume': M['_della_set_volume'](h, value); break;
      case 'mix': M['_della_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_della_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_della_create'](sampleRate);
      if (this.handle < 0) throw new Error('della_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Della] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_della_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('della-processor', DellaProcessor); processorRegistered = true; }
