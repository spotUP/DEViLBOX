/**
 * PumaTracker.worklet.js - AudioWorklet processor for PumaTracker WASM synth
 *
...
class PumaTrackerProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.module = null;
  ...
  async initModule(sampleRate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.Pumatracker) {
        ...
        const wrappedCode = jsCode + '\nreturn createPumatracker;';
        const factory = new Function(wrappedCode);
        const result = factory();

        if (typeof result === 'function') {
          globalThis.Pumatracker = result;
        } else {
      ...
      if (typeof globalThis.Pumatracker !== 'function') {
        this.port.postMessage({ type: 'error', message: 'Pumatracker factory not available' });
        return;
      }
      ...
      try {
        this.module = await globalThis.Pumatracker(config);
      } finally {
...
registerProcessor('pumatracker-processor', PumaTrackerProcessor);
