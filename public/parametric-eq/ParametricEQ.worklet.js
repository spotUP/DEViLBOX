/**
 * ParametricEQ.worklet.js — AudioWorklet processor for ParametricEQ WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createParametricEQ;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createParametricEQ factory');
...
class ParametricEQProcessor extends AudioWorkletProcessor {
  ...
  applyParam(param, value) {
    ...
    const fn = M['_parametric_eq_set_' + param];
    if (fn) fn(h, value);
  }
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._parametric_eq_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._parametric_eq_create(sampleRate);
      if (this.handle < 0) throw new Error('parametric_eq_create failed');
      const bs = 128 * 4;
      ...
      console.error('[ParametricEQ] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._parametric_eq_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
      outL.set(this.outputBufferL.subarray(0, n)); outR.set(this.outputBufferR.subarray(0, n));
    } catch (err) { outL.set(inL); outR.set(inR); }
    return true;
  }
}
if (!processorRegistered) { registerProcessor('parametric-eq-processor', ParametricEQProcessor); processorRegistered = true; }
