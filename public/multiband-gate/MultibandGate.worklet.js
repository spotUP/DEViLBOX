/**
 * MultibandGate.worklet.js — AudioWorklet processor for MultibandGate WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createMultibandGate;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createMultibandGate factory');
...
class MultibandGateProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  async handleMessage(event) {
    ...
    switch (type) {
      ...
      case 'parameter':
        if (this.isInitialized && this.handle >= 0) this.Module['_multiband_gate_set_' + data.param](this.handle, data.value);
  ...
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._multiband_gate_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._multiband_gate_create(sampleRate);
      if (this.handle < 0) throw new Error('multiband_gate_create failed');
      const bs = 128 * 4;
      ...
      console.error('[MultibandGate] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._multiband_gate_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('multiband-gate-processor', MultibandGateProcessor); processorRegistered = true; }
