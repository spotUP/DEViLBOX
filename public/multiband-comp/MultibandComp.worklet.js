/**
 * MultibandComp.worklet.js — AudioWorklet processor for MultibandComp WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try {
      const wrappedCode = `${jsCode}; return createMultibandComp;`;
      createModule = new Function(wrappedCode)();
    } catch (e) {
      console.error('[MultibandComp] Failed to evaluate JS:', e);
    ...
    if (!createModule) {
      sharedModulePromise = null;
      throw new Error('Could not load MultibandComp module factory');
    }
...
class MultibandCompProcessor extends AudioWorkletProcessor {
  ...
  applyParameter(param, value) {
    const M = this.Module;
    const h = this.handle;
    const fn = M['_multiband_comp_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._multiband_comp_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = Module._multiband_comp_create(sampleRate);
      if (this.handle < 0) throw new Error('multiband_comp_create returned ' + this.handle);
      ...
      console.error('[MultibandComp] init error:', error);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._multiband_comp_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
