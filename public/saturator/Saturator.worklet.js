...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try { const wrappedCode = `${jsCode}; return createSaturator;`; createModule = new Function(wrappedCode)(); }
    catch (e) { console.error('[Saturator] Failed:', e); sharedModulePromise = null; throw e; }
...
class SaturatorProcessor extends AudioWorkletProcessor {
  ...
  applyParameter(param, value) {
    ...
    const fn = M['_saturator_set_' + param];
    if (fn) fn(h, value);
  }
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._saturator_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = Module._saturator_create(sampleRate);
      ...
      console.error('[Saturator] init error:', error);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._saturator_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('saturator-processor', SaturatorProcessor); processorRegistered = true; }
