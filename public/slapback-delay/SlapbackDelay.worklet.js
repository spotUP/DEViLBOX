...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try {
      const wrappedCode = `${jsCode}; return createSlapbackDelay;`;
      createModule = new Function(wrappedCode)();
    } catch (e) { console.error('[SlapbackDelay] Failed:', e); sharedModulePromise = null; throw e; }
...
class SlapbackDelayProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParameter(param, value) {
    ...
    const fn = M['_slapback_delay_set_' + param];
    if (fn) fn(h, value);
  }
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._slapback_delay_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = Module._slapback_delay_create(sampleRate);
      ...
      console.error('[SlapbackDelay] init error:', error);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._slapback_delay_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('slapback-delay-processor', SlapbackDelayProcessor); processorRegistered = true; }
