/**
 * SoundFx.worklet.js — AudioWorklet processor for SoundFx WASM replayer
...
class SoundFxProcessor extends AudioWorkletProcessor {
  ...
  async handleMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'loadModule': {
        ...
        try {
          if (this.handle) { this.module._sfx_destroy(this.handle); this.handle = 0; }
          ...
          this.handle = this.module._sfx_create(wasmPtr, uint8Data.length, sampleRate);
          this.module._free(wasmPtr);
          if (!this.handle) { this.port.postMessage({ type: 'error', message: 'sfx_create failed (unsupported format)' }); return; }
          const subsongCount = this.module._sfx_subsong_count(this.handle);
          if (typeof data.subsong === 'number' && data.subsong > 0) this.module._sfx_select_subsong(this.handle, data.subsong);
          this.playing = false;
      ...
      case 'stop':
        this.playing = false;
        if (this.handle) this.module._sfx_select_subsong(this.handle, 0);
        this.port.postMessage({ type: 'stopped' });
        break;
      case 'pause': this.playing = !this.playing; break;
      case 'setSubsong': if (this.handle) this.module._sfx_select_subsong(this.handle, data.subsong); break;
      case 'setChannelMask': if (this.handle) this.module._sfx_set_channel_mask(this.handle, data.mask); break;
      case 'setCell': {        if (this.handle && this.module._sfx_set_cell) {          this.module._sfx_set_cell(this.handle, data.index, data.row, data.channel, data.note, data.instrument, data.effect, data.effectArg);        }        break;      }      case 'setInstrumentParam': {        if (this.handle && this.module._sfx_set_instrument_param) {          var pLen = this.module.lengthBytesUTF8(data.param) + 1;          var pPtr = this.module._malloc(pLen);          this.module.stringToUTF8(data.param, pPtr, pLen);          this.module._sfx_set_instrument_param(this.handle, data.instrument, pPtr, data.value);          this.module._free(pPtr);        }        break;      }            case 'getInstrumentParam': {
        if (!this.handle || !this.module) break;
        const m = this.module;
        const fn = m._sfx_get_instrument_param;
        if (!fn) break;
  ...
  async initWasm(rate, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.createSoundFx) {
        const wrappedCode = jsCode + '\nreturn createSoundFx;';
        const factory = new Function(wrappedCode);
        const result = factory();
        if (typeof result === 'function') globalThis.createSoundFx = result;
        else { this.port.postMessage({ type: 'error', message: 'Failed to load JS module' }); return; }
      }
      if (typeof globalThis.createSoundFx !== 'function') { this.port.postMessage({ type: 'error', message: 'createSoundFx factory not available' }); return; }
      ...
      try { this.module = await globalThis.createSoundFx(config); } finally { WebAssembly.instantiate = origInstantiate; }
  ...
  cleanup() {
    if (this.module && this.handle) { this.module._sfx_destroy(this.handle); this.handle = 0; }
    if (this.module) {
  ...
  process(inputs, outputs) {
    ...
    // Render per-channel
    const rendered = this.module._sfx_render_multi(
    ...
    if (this.module._sfx_has_ended(this.handle)) this.port.postMessage({ type: 'songEnd' });
    return true;
  }
}
registerProcessor('soundfx-processor', SoundFxProcessor);
