/**
 * Roomy.worklet.js — AudioWorklet processor for Roomy WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createRoomy;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createRoomy factory');
...
class RoomyProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'time': M['_roomy_set_time'](h, value); break;
      case 'damping': M['_roomy_set_damping'](h, value); break;
      case 'mix': M['_roomy_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_roomy_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_roomy_create'](sampleRate);
      if (this.handle < 0) throw new Error('roomy_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Roomy] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_roomy_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('roomy-processor', RoomyProcessor); processorRegistered = true; }
