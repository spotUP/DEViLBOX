/**
 * TransientDesigner.worklet.js — AudioWorklet processor for TransientDesigner WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try {
      const wrappedCode = `${jsCode}; return createTransientDesigner;`;
      createModule = new Function(wrappedCode)();
    } catch (e) {
      console.error('[TransientDesigner] Failed to evaluate JS:', e);
    ...
    if (!createModule) {
      sharedModulePromise = null;
      throw new Error('Could not load TransientDesigner module factory');
    }
...
class TransientDesignerProcessor extends AudioWorkletProcessor {
  ...
  applyParameter(param, value) {
    const M = this.Module;
    const h = this.handle;
    const fn = M['_transient_designer_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._transient_designer_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = Module._transient_designer_create(sampleRate);
      if (this.handle < 0) throw new Error('transient_designer_create returned ' + this.handle);
      ...
      console.error('[TransientDesigner] init error:', error);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._transient_designer_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
