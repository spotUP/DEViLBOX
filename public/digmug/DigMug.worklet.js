...
class DigMugProcessor extends AudioWorkletProcessor {
  ...
  async handleMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'setCell': {        if (this.handle && this.module._dm_set_cell) {          this.module._dm_set_cell(this.handle, data.index, data.row, data.channel, data.note, data.instrument, data.effect, data.effectArg);        }        break;      }      case 'setInstrumentParam': {        if (this.handle && this.module._dm_set_instrument_param) {          var pLen = this.module.lengthBytesUTF8(data.param) + 1;          var pPtr = this.module._malloc(pLen);          this.module.stringToUTF8(data.param, pPtr, pLen);          this.module._dm_set_instrument_param(this.handle, data.instrument, pPtr, data.value);          this.module._free(pPtr);        }        break;      }      case 'dispose': this.cleanup(); break;
    }
  }
  ...
  process(inputs, outputs) {
    ...
    // Render per-channel
    const rendered = this.module._dm_render_multi(
