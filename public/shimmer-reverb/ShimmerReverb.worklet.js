/**
 * ShimmerReverb.worklet.js - Shimmer Reverb AudioWorklet Effect Processor
 * Loads and runs the ShimmerReverb WASM module as an audio effect (input -> output).
 *
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    ...
    let createModule;
    try {
      const wrappedCode = `${jsCode}; return createShimmerReverbModule;`;
      createModule = new Function(wrappedCode)();
    } catch (evalErr) {
      console.error('[ShimmerReverb] Failed to evaluate JS:', evalErr);
      sharedModulePromise = null;
      throw new Error('Could not evaluate ShimmerReverb module factory');
    }

    if (!createModule) {
      sharedModulePromise = null;
      throw new Error('Could not load ShimmerReverb module factory');
    }
...
class ShimmerReverbProcessor extends AudioWorkletProcessor {
  ...
  async initialize(data) {
    try {
      ...
      this.effect = new Module.ShimmerReverbEffect();
      ...
      console.error('[ShimmerReverb] initialization error:', error);
...
if (!processorRegistered) {
  registerProcessor('shimmer-reverb-processor', ShimmerReverbProcessor);
