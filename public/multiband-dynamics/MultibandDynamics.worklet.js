/**
 * MultibandDynamics.worklet.js — AudioWorklet processor for MultibandDynamics WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createMultibandDynamics;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createMultibandDynamics factory');
...
class MultibandDynamicsProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'lowCross': M['_multiband_dynamics_set_lowCross'](h, value); break;
      case 'highCross': M['_multiband_dynamics_set_highCross'](h, value); break;
      case 'lowExpThresh': M['_multiband_dynamics_set_lowExpThresh'](h, value); break;
      case 'midExpThresh': M['_multiband_dynamics_set_midExpThresh'](h, value); break;
      case 'highExpThresh': M['_multiband_dynamics_set_highExpThresh'](h, value); break;
      case 'lowCompThresh': M['_multiband_dynamics_set_lowCompThresh'](h, value); break;
      case 'midCompThresh': M['_multiband_dynamics_set_midCompThresh'](h, value); break;
      case 'highCompThresh': M['_multiband_dynamics_set_highCompThresh'](h, value); break;
      case 'ratio': M['_multiband_dynamics_set_ratio'](h, value); break;
      case 'attack': M['_multiband_dynamics_set_attack'](h, value); break;
      case 'release': M['_multiband_dynamics_set_release'](h, value); break;
      case 'mix': M['_multiband_dynamics_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_multiband_dynamics_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_multiband_dynamics_create'](sampleRate);
      if (this.handle < 0) throw new Error('multiband_dynamics_create failed');
      const bs = 128 * 4;
      ...
      console.error('[MultibandDynamics] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_multiband_dynamics_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('multiband-dynamics-processor', MultibandDynamicsProcessor); processorRegistered = true; }
