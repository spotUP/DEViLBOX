/**
 * EQ12Band.worklet.js — AudioWorklet processor for 12-band EQ WASM effect.
 * Custom applyParam: parses 'freq_3', 'gain_7', 'q_11' → indexed setter calls.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createEQ12Band;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createEQ12Band factory');
...
class EQ12BandProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    if (param === 'mix') {
      M._eq12_set_mix(h, value);
      return;
    }
    // Parse indexed params: 'freq_3', 'gain_7', 'q_11'
    const idx = param.lastIndexOf('_');
    if (idx >= 0) {
      const name = param.substring(0, idx);
      const band = parseInt(param.substring(idx + 1));
      if (!isNaN(band)) {
        const fn = M['_eq12_set_' + name];
        if (fn) fn(h, band, value);
      }
    }
  }
  ...
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._eq12_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._eq12_create(sampleRate);
      if (this.handle < 0) throw new Error('eq12_create failed');
      const bs = 128 * 4;
      ...
      console.error('[EQ12Band] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._eq12_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('eq12-processor', EQ12BandProcessor); processorRegistered = true; }
