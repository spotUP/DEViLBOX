/**
 * EarlyReflections.worklet.js — AudioWorklet processor for EarlyReflections WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createEarlyReflections;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createEarlyReflections factory');
...
class EarlyReflectionsProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    const fn = M['_early_reflections_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._early_reflections_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._early_reflections_create(sampleRate);
      if (this.handle < 0) throw new Error('early_reflections_create failed');
      const bs = 128 * 4;
      ...
      console.error('[EarlyReflections] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._early_reflections_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('early-reflections-processor', EarlyReflectionsProcessor); processorRegistered = true; }
