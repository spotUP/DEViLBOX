/**
 * TubeAmp.worklet.js — AudioWorklet processor for TubeAmp WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createTubeAmp;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createTubeAmp factory');
...
class TubeAmpProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    const fn = M['_tube_amp_set_' + param];
    if (fn) fn(h, value);
  }
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._tube_amp_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._tube_amp_create(sampleRate);
      if (this.handle < 0) throw new Error('tube_amp_create failed');
      const bs = 128 * 4;
      ...
      console.error('[TubeAmp] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._tube_amp_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
      outL.set(this.outputBufferL.subarray(0, n)); outR.set(this.outputBufferR.subarray(0, n));
    } catch (err) { outL.set(inL); outR.set(inR); }
    return true;
  }
}
if (!processorRegistered) { registerProcessor('tube-amp-processor', TubeAmpProcessor); processorRegistered = true; }
