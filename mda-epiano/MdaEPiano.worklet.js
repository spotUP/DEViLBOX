/**
 * MDA ePiano AudioWorklet Processor
 * Electric Piano Synthesizer for DEViLBOX
 *
 * AudioWorklets don't support dynamic import().
 * The WASM module JS is passed as a string and executed via Function constructor.
 */

class MdaEPianoProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.engine = null;
    this.module = null;
  ...
  async handleMessage(data) {
    switch (data.type) {
      ...
      case 'noteOn':
        if (this.engine) {
          this.module._epiano_note_on(this.engine, data.note, data.velocity);
        }
        break;
      case 'noteOff':
        if (this.engine) {
          this.module._epiano_note_off(this.engine, data.note);
        }
        break;
      case 'setParam':
        if (this.engine) {
          this.module._epiano_set_param(this.engine, data.index, data.value);
        }
        break;
      case 'setProgram':
        if (this.engine) {
          this.module._epiano_set_program(this.engine, data.program);
        }
        break;
      case 'sustain':
        if (this.engine) {
          this.module._epiano_sustain(this.engine, data.value);
        }
        break;
      case 'allNotesOff':
        if (this.engine) {
          this.module._epiano_all_notes_off(this.engine);
        }
        break;
      case 'getInfo':
        if (this.engine) {
          this.port.postMessage({
            type: 'info',
            activeVoices: this.module._epiano_get_active_voices(this.engine),
            numParams: this.module._epiano_get_num_params(this.engine),
            numPrograms: this.module._epiano_get_num_programs(this.engine)
          });
        }
        break;
      case 'dispose':
  ...
  async initEngine(sr, wasmBinary, jsCode) {
    ...
    try {
      ...
      if (jsCode && !globalThis.MdaEPianoFactory) {
        ...
        const wrappedCode = jsCode + '\nreturn createMdaEPiano;';
        const factory = new Function(wrappedCode);
        const result = factory();

        if (typeof result === 'function') {
          globalThis.MdaEPianoFactory = result;
        } else {
      ...
      if (typeof globalThis.MdaEPianoFactory !== 'function') {
        this.port.postMessage({ type: 'error', message: 'MdaEPiano factory not available' });
        return;
      }
      ...
      try {
        this.module = await globalThis.MdaEPianoFactory(config);
      } finally {
      ...
      this.engine = this.module._epiano_create(sr || sampleRate);
      ...
      this.initializing = false;
      console.error('[MdaEPiano Worklet] Init error:', error);
  ...
  cleanup() {
    if (this.module && this.engine) {
      this.module._epiano_destroy(this.engine);
      this.engine = null;
    }
    if (this.module) {
      if (this.outputPtrL) {
  ...
  process(inputs, outputs, parameters) {
    ...
    // Generate audio
    this.module._epiano_process(this.engine, this.outputPtrL, this.outputPtrR, numSamples);
...
registerProcessor('mda-epiano-processor', MdaEPianoProcessor);
