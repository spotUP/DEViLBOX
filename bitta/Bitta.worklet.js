/**
 * Bitta.worklet.js — AudioWorklet processor for Bitta WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createBitta;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createBitta factory');
...
class BittaProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'crush': M['_bitta_set_crush'](h, value); break;
      case 'mix': M['_bitta_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_bitta_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_bitta_create'](sampleRate);
      if (this.handle < 0) throw new Error('bitta_create failed');
      const bs = 128 * 4;
      ...
      console.error('[Bitta] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_bitta_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('bitta-processor', BittaProcessor); processorRegistered = true; }
