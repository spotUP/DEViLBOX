/*
  ...
  DSP algorithm is identical to the original mdaJX10.
*/

#include "mdaJX10_standalone.h"
...
mdaJX10::mdaJX10(float sampleRate)
{
  int32_t i = 0;
  Fs = sampleRate;
  curProgram = 0;
...
float mdaJX10::getParameter(int32_t index)
{
  if(index >= 0 && index < NPARAMS)
...
void mdaJX10::process(float* outL, float* outR, int32_t sampleFrames)
{
        ...
        for(v = 0; v < NVOICES; v++)
        {
          ...
          { // Sinc-Loop Oscillator
            ...
            else
            {
              ...
              if(x*x > .1f) x = V->sin0 / x; else x = V->lev;
            }
...
void mdaJX10::noteOn(int32_t note, int32_t velocity)
{
  ...
  if(velocity > 0) // note on
  {
    ...
    if(mode & 2)
    {
      if((mode & 1) || held) tmp = note - lastnote;
    }
    voice[v].period = p * (float)pow(1.059463094359, (double)tmp - glidedisp);
    if(voice[v].period < 3.0f) voice[v].period = 3.0f;
