...
bool DivEngine::load(unsigned char* f, size_t slen, const char* nameHint) {
  ...
  try {
    ...
    // PATCH: Check for zlib header (0x78) — if present, use raw deflate mode
    // to skip header/checksum validation (DefleMask files have bad checksums)
    bool hasZlibHeader=(slen>=2 && f[0]==0x78);
    int skipBytes=0;

    if (hasZlibHeader) {
      // Skip the 2-byte zlib header; use raw deflate (wbits=-15)
      zl.avail_in=slen-2;
      zl.next_in=(Bytef*)(f+2);
      skipBytes=2;
    } else {
      zl.avail_in=slen;
      zl.next_in=(Bytef*)f;
    ...
    int nextErr;
    if (hasZlibHeader) {
      nextErr=inflateInit2(&zl,-15); // raw deflate, no header/checksum
    } else {
    ...
    while (true) {
      ...
      if (nextErr!=Z_OK && nextErr!=Z_STREAM_END) {
        // PATCH: On Z_DATA_ERROR (bad checksum), keep data we already have
        // DefleMask files have bad zlib checksums but valid data
        ...
        if (!blocks.empty()) {
          logD("zlib error %d but keeping %d blocks of decompressed data",nextErr,(int)blocks.size());
          inflateEnd(&zl);
          break; // use whatever data we have
        }
        if (zl.msg==NULL) {
    ...
    if (nextErr!=Z_OK) {
      // PATCH: Don't discard data on inflateEnd failure — data is likely fine,
      // only the checksum verification failed (common in DefleMask files)
      logD("zlib end error %d — continuing with decompressed data",nextErr);
    }
