/**
 * BassEnhancer.worklet.js — AudioWorklet processor for BassEnhancer WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try {
      const wrappedCode = `${jsCode}; return createBassEnhancer;`;
      createModule = new Function(wrappedCode)();
    } catch (e) {
      console.error('[BassEnhancer] Failed to evaluate JS:', e);
    ...
    if (!createModule) {
      sharedModulePromise = null;
      throw new Error('Could not load BassEnhancer module factory');
    }
...
class BassEnhancerProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.handle = -1;
  ...
  applyParameter(param, value) {
    const M = this.Module;
    const h = this.handle;
    const fn = M['_bass_enhancer_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._bass_enhancer_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = Module._bass_enhancer_create(sampleRate);
      if (this.handle < 0) throw new Error('bass_enhancer_create returned ' + this.handle);
      ...
      console.error('[BassEnhancer] init error:', error);
  ...
  process(inputs, outputs) {
    ...
    if (!this.isInitialized || this.handle < 0) {
      outputL.set(inputL); outputR.set(inputR);
      return true;
    }
    try {
      ...
      this.Module._bass_enhancer_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) {
  registerProcessor('bass-enhancer-processor', BassEnhancerProcessor);
