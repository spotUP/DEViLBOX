/**
 * MonoComp.worklet.js — AudioWorklet processor for MonoComp WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createMonoComp;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createMonoComp factory');
...
class MonoCompProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  async handleMessage(event) {
    ...
    switch (type) {
      ...
      case 'parameter':
        if (this.isInitialized && this.handle >= 0) this.Module['_mono_comp_set_' + data.param](this.handle, data.value);
  ...
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._mono_comp_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._mono_comp_create(sampleRate);
      if (this.handle < 0) throw new Error('mono_comp_create failed');
      const bs = 128 * 4;
      ...
      console.error('[MonoComp] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._mono_comp_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('mono-comp-processor', MonoCompProcessor); processorRegistered = true; }
