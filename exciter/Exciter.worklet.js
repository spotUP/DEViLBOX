...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try { const wrappedCode = `${jsCode}; return createExciter;`; createModule = new Function(wrappedCode)(); }
    catch (e) { console.error('[Exciter] Failed:', e); sharedModulePromise = null; throw e; }
...
class ExciterProcessor extends AudioWorkletProcessor {
  ...
  applyParameter(param, value) {
    ...
    const fn = M['_exciter_set_' + param];
    if (fn) fn(h, value);
  }
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._exciter_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = Module._exciter_create(sampleRate);
      ...
      console.error('[Exciter] init error:', error);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._exciter_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('exciter-processor', ExciterProcessor); processorRegistered = true; }
