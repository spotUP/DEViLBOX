...
/* Open MDX from memory buffer (for WASM - no filesystem access) */
MDX_DATA *mdx_open_mdx_mem( unsigned char *data, int data_len ) {
  ...
  /* Copy the data so mdx owns it (will be freed by mdx_close_mdx) */
  buf = (unsigned char *)malloc(data_len + 16);
  if (!buf) { free(mdx); return NULL; }
  memcpy(buf, data, data_len);
  memset(buf + data_len, 0, 16);
  mdx->length = data_len;
  mdx->data = buf;
  ...
  while(1) {
    ...
    mdx->data_title[i++]=buf[ptr++];
    if ( i>=MDX_MAX_TITLE_LENGTH ) i--;
    if ( ptr > mdx->length ) { free(buf); free(mdx); return NULL; }
  }
  mdx->data_title[i++]=0;
  ...
  while(1) {
    ...
    if ( strcasecmp( ".pdx", (char *)(buf+ptr-1) )==0 ) j=1;
    if ( i>= MDX_MAX_PDX_FILENAME_LENGTH ) i--;
    if ( ptr > mdx->length ) goto mem_error_end;
  }
  if ( mdx->haspdx==FLAG_TRUE && j==0 ) {
    mdx->pdx_name[i+0] = '.';
    mdx->pdx_name[i+1] = 'p';
  ...
  if ( mdx->voice_data_offset > mdx->length ) goto mem_error_end;

  /* get MML data offset */
  mdx->mml_data_offset[0] =
    (unsigned int)buf[ptr+2+0] * 256 +
    (unsigned int)buf[ptr+2+1] + mdx->base_pointer;
  if ( mdx->mml_data_offset[0] > mdx->length ) goto mem_error_end;
  ...
  for ( i=0 ; i<mdx->tracks ; i++ ) {
    mdx->mml_data_offset[i] =
      (unsigned int)buf[ptr+i*2+2+0] * 256 +
      (unsigned int)buf[ptr+i*2+2+1] + mdx->base_pointer;
    if ( mdx->mml_data_offset[i] > mdx->length ) goto mem_error_end;
  }
