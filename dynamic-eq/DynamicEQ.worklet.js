/**
 * DynamicEQ.worklet.js — AudioWorklet processor for DynamicEQ WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createDynamicEQ;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createDynamicEQ factory');
...
class DynamicEQProcessor extends AudioWorkletProcessor {
  ...
  applyParam(param, value) {
    ...
    const fn = M['_dynamic_eq_set_' + param];
    if (fn) fn(h, value);
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._dynamic_eq_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M._dynamic_eq_create(sampleRate);
      if (this.handle < 0) throw new Error('dynamic_eq_create failed');
      const bs = 128 * 4;
      ...
      console.error('[DynamicEQ] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._dynamic_eq_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('dynamic-eq-processor', DynamicEQProcessor); processorRegistered = true; }
