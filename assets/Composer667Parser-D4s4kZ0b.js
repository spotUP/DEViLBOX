...
function readString669(v, off, len) {
  const bytes = [];
  for (let i = 0; i < len; i++) {
...
const EFF_NONE = 0;
function isComposer667Format(bytes) {
...
function parseComposer667File(bytes, filename) {
  try {
...
function _parse669(bytes, filename) {
  ...
  function buildPattern(patIdx, orderPos, speed, breakRow) {
    ...
    for (let row = 0; row < ROWS_PER_PATTERN; row++) {
      for (let ch = 0; ch < NUM_CHANNELS; ch++) {
        ...
        const noteInstr = cellBase < buffer.byteLength ? u8dv(v, cellBase) : 255;
        const instrVol = cellBase + 1 < buffer.byteLength ? u8dv(v, cellBase + 1) : 0;
        const effParam = cellBase + 2 < buffer.byteLength ? u8dv(v, cellBase + 2) : 255;
        let note = 0;
        let instrument = 0;
        let volume = 0;
        if (noteInstr < 254) {
        ...
        if (channelEffect[ch] !== 255) {
          ...
          eff = mapped.eff;
          if (command !== 6) channelEffect[ch] = 255;
        }
  ...
  const instruments = sampleHeaders.map((hdr, i) => {
    ...
    if (hdr.length === 0 || hdr.length >= 67108864) {
      return { id, name, type: "sample", synthType: "Sampler", effects: [], volume: -60, pan: 0 };
    }
  ...
  const uadePatternLayout = {
    formatId: "composer667",
    patternDataFileOffset: HEADER_SIZE + numSamples * SAMPLE_HDR_SIZE,
