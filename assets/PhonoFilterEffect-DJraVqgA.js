...
class PhonoFilterEffect extends ToneAudioNode {
  name = "PhonoFilter";
  input;
  output;
  dryGain;
  ...
  static async ensureInitialized(ctx) {
    ...
    const p = (async () => {
      ...
      const [wasmResp, jsResp] = await Promise.all([
        fetch(`${base}phono-filter/PhonoFilter.wasm`),
        fetch(`${base}phono-filter/PhonoFilter.js`)
      ]);
  ...
  setMode(v) {
    this._mode = Math.round(clamp(v, 0, 3));
    this.sendParam("mode", this._mode);
  }
  setMix(v) {
