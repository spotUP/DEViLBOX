...
class DellaEffect extends ToneAudioNode {
  name = "Della";
  _input;
  _output;
  dryGain;
  ...
  static async ensureInitialized(ctx) {
    ...
    const p = (async () => {
      ...
      const [wasmResp, jsResp] = await Promise.all([
        fetch(`${base}della/Della.wasm`),
        fetch(`${base}della/Della.js`)
      ]);
  ...
  setTime(v) {
    this._time = Math.max(50, Math.min(2e3, v));
    this.sendParam("time", this._time);
  }
  setFeedback(v) {
