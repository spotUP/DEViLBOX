/**
 * YMOPQ AudioWorklet Processor
 * Yamaha YM3806 (OPQ) 4-Operator FM Synthesizer for DEViLBOX
 *
 * 8-channel FM synthesis with 32 operators, 8 algorithms,
 * LFO, reverb, stereo output. Based on ymfm by Aaron Giles.
 */

class YMOPQProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.synth = null;
    this.module = null;
  ...
  async handleMessage(data) {
    switch (data.type) {
      ...
      // Convenience setters
      case 'setAlgorithm':
        if (this.synth) this.synth.setAlgorithm(data.value);
        break;
      case 'setFeedback':
        if (this.synth) this.synth.setFeedback(data.value);
        break;
      case 'setLFORate':
        if (this.synth) this.synth.setLFORate(data.value);
        break;
      case 'setVolume':
  ...
  async initSynth(data) {
    try {
      ...
      this.module = await globalThis.initMAMEWasmModule(data.wasmBinary, data.jsCode, 'createYMOPQModule');

      this.synth = new this.module.YMOPQSynth();
      ...
      console.error('YMOPQ init error:', error);
...
registerProcessor('ymopq-processor', YMOPQProcessor);
