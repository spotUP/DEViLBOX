...
/**
 * HC55516 AudioWorklet Processor
 * Harris HC55516 CVSD Speech Codec (Williams/Bally arcade) for DEViLBOX
 *
 * 4-voice polyphonic CVSD delta modulation speech synth.
 * Syllabic + integration filters for characteristic lo-fi voice.
 * Based on MAME emulator by Aaron Giles, Jonathan Gevaryahu.
 */

class HC55516Processor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.synth = null;
    this.module = null;
  ...
  async initSynth(data) {
    try {
      ...
      this.module = await globalThis.initMAMEWasmModule(data.wasmBinary, data.jsCode, 'createHC55516Module');

      this.synth = new this.module.HC55516Synth();
      ...
      console.error('HC55516 init error:', error);
...
registerProcessor('hc55516-processor', HC55516Processor);
