/**
 * ICS2115 WaveFront AudioWorklet Processor
 * 32-voice wavetable synthesizer for DEViLBOX
 *
 * Features:
 * - 32 independent voices
 * - 16-bit, 8-bit, and u-law sample formats
 * - Volume and oscillator envelopes
...
class ICS2115Processor extends AudioWorkletProcessor {
  ...
  async handleMessage(data) {
    switch (data.type) {
      ...
      case 'writeRegister':
        if (this.synth) {
          this.synth.writeRegister(data.voice, data.reg, data.data);
        }
        break;
      case 'loadROM':
  ...
  async initSynth(data) {
    try {
      ...
      this.module = await globalThis.initMAMEWasmModule(data.wasmBinary, data.jsCode, 'createICS2115Module');
      ...
      console.error('ICS2115 init error:', error);
...
registerProcessor('ics2115-processor', ICS2115Processor);
