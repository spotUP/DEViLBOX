/* Calf Monosynth - Standalone DSP implementation for WASM
 * Based on Calf Studio Gear by Krzysztof Foltman
 * Original: Copyright (C) 2001-2007 Krzysztof Foltman (LGPL v2+)
 *
 * Adapted from src/monosynth.cpp - removes all plugin framework dependencies.
 */
#include "calf_mono_standalone.h"

using namespace dsp;
using namespace calf_plugins;
using namespace std;

float monosynth_audio_module::silence[4097] = {0};
waveform_family<MONOSYNTH_WAVE_BITS> *monosynth_audio_module::waves = nullptr;
...
, inertia_pressure(64)
{
    // Initialize parameter storage with defaults
    for (int i = 0; i < param_count; i++) {
        param_values[i] = monosynth_param_defaults[i].def;
...
void monosynth_audio_module::precalculate_waves()
{
...
void monosynth_audio_module::calculate_buffer_oscs(float lfo1)
{
    ...
    if (unison > 0)
    {
        float freq_val = fabs(*params[par_o2unisonfrq] / muls[7]);
        if (moddest[moddest_o2unisondetune] != 0)
            freq_val *= pow(2.0, moddest[moddest_o2unisondetune]);
        unison_osc.set_freq(freq_val, srate);
    ...
    for (uint32_t i = 0; i < step_size; i++)
    {
        ...
        buffer[i] = dsp::lerp(r * osc1val, osc2val, cur_xfade);
        osc1.advance();
        osc2.advance();
        shift1 += shift_delta1;
        shift2 += shift_delta2;
...
void monosynth_audio_module::calculate_buffer_stereo()
{
    ...
    for (uint32_t i = 0; i < step_size; i++)
    {
        float w1 = buffer[i] * fgain;
        buffer[i] = fgain * filter.process(w1);
        buffer2[i] = fgain * filter2.process(w1);
        fgain += fgain_delta;
    }
}
...
{
    force_fadeout = false;
    fadeout_obj.reset_soft();
    fadeout2_obj.reset_soft();
    porta_time = 0.f;
    start_freq = freq;
    ...
    if (!running)
    {
        ...
        {
        case 1: osc2.phase = 0x80000000; break;
        case 2: osc2.phase = 0x40000000; break;
        case 3: osc1.phase = osc2.phase = 0x40000000; break;
        case 4: osc1.phase = 0x40000000; osc2.phase = 0xC0000000; break;
        case 5: osc1.phase = rand() << 16; osc2.phase = rand() << 16; break;
        default: break;
        }
        running = true;
    }
...
void monosynth_audio_module::set_sample_rate(uint32_t sr) {
    ...
    fgain = 0.f;
    fgain_delta = 0.f;
    inertia_cutoff.ramp.set_length(crate / 30);
    inertia_pitchbend.ramp.set_length(crate / 30);
...
void monosynth_audio_module::calculate_step()
{
    ...
    lfo_clock += odcr;
    if (fabs(*params[par_lfopitch]) > dsp::small_value<float>())
...
{
    if (fadeout_obj.undoing)
    {
        fadeout_obj.process(buffer, step_size);
        if (is_stereo_filter())
            fadeout2_obj.process(buffer2, step_size);
    }
    else
    {
        ...
        if (do_fadeout || fadeout_obj.undoing || fadeout2_obj.undoing)
        {
            fadeout_obj.process(buffer, step_size);
            if (is_stereo_filter())
                fadeout2_obj.process(buffer2, step_size);
...
{
    if (*params[par_midi] && channel != (int)*params[par_midi]) return;
    queue_note_on = note;
    queue_note_on_and_off = false;
    last_key = note;
    queue_vel = vel / 127.f;
...
void monosynth_audio_module::pitch_bend(int channel, int value)
{
...
void monosynth_audio_module::all_notes_off()
{
    force_fadeout = true;
    gate = false;
    queue_note_on = -1;
