...
emcc \
    ...
    -s EXPORTED_FUNCTIONS='["_buzz_get_info","_buzz_create_machine","_buzz_delete_machine","_buzz_init","_buzz_tick","_buzz_work","_buzz_stop","_buzz_set_parameter","_buzz_get_global_vals","_buzz_set_num_tracks","_buzz_get_track_vals","_buzz_set_sample_rate","_buzz_set_bpm","_malloc","_free"]' \
...
emcc \
    "${COMMON_FLAGS[@]}" \
    -I "$BUZZ_BASE/common/mdk" -DUSE_MDK \
...
emcc \
    ...
    "$BUZZ_BASE/Makk/M4/waves/waves.cpp" \
    "$BUZZ_BASE/common/dsplib/dsplib.cpp" \
    "$WRAPPER_SRC/BuzzmachineWrapper.cpp" \
...
# 43. Build Oomek Aggressor Devil Fish (303-style synth with Devil Fish mods, requires MDK)
echo ""
echo "Building Oomek Aggressor Devil Fish (with MDK)..."
emcc \
    "${COMMON_FLAGS[@]}" \
    -I "$BUZZ_BASE/common/mdk" -DUSE_MDK \
    -s EXPORT_NAME="OomekAggressorDF" \
    -s EXPORTED_FUNCTIONS='["_buzz_get_info","_buzz_create_machine","_buzz_delete_machine","_buzz_init","_buzz_tick","_buzz_work","_buzz_stop","_buzz_set_parameter","_buzz_get_global_vals","_buzz_set_num_tracks","_buzz_get_track_vals","_buzz_set_sample_rate","_buzz_set_bpm","_malloc","_free"]' \
    "$BUZZ_BASE/Oomek/Aggressor/303_devilfish.cpp" \
    "$BUZZ_BASE/common/mdk/mdkimp.cpp" \
    "$BUZZ_BASE/common/dsplib/dsplib.cpp" \
    "$WRAPPER_SRC/BuzzmachineWrapper.cpp" \
    -o "$OUTPUT_DIR/Oomek_Aggressor_DF.js"

if [ -f "$OUTPUT_DIR/Oomek_Aggressor_DF.wasm" ]; then
    echo "✓ Oomek Aggressor Devil Fish built successfully"
else
    echo "❌ Failed to build Oomek Aggressor Devil Fish (continuing...)"
fi
