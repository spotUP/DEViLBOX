/**
 * DragonflyHallWASM.cpp — Hall reverb for DEViLBOX
 *
 * Simulates a large hall with long tail and spacious early reflections.
 * Uses longer delay lines and more diffusion stages than the plate reverb.
 *
...
public:
    ...
    void init(float sampleRate, float sizeMultiplier) {
        delete[] buf;
        size = static_cast<int>(0.1f * sampleRate);
        buf = new float[size]();
        pos = 0;
        float scale = sampleRate / 48000.0f * sizeMultiplier;
        int baseTaps[] = { 210, 430, 680, 1050, 1480, 1930 };
        float baseGains[] = { 0.75f, 0.62f, 0.50f, 0.40f, 0.32f, 0.25f };
...
// ─── Hall Reverb Instance ───────────────────────────────────────────────────

struct HallReverbInstance {
    bool active = false;
    float sampleRate = 48000.0f;

    // Parameters
    float decay = 0.8f;        // 0-0.99
    float damping = 0.3f;      // 0-1
    float predelayMs = 20.0f;  // 0-200 ms
    float width = 1.0f;        // 0-1 stereo width
    float earlyLevel = 0.5f;   // 0-1
    float size = 1.5f;         // 0.5-3.0 delay length multiplier
    ...
    // Diffusion (6 allpass filters for dense, spacious sound)
    Allpass diffAP[6];
    ...
    void init(float sr) {
        ...
        // Predelay (max 200ms)
        predelaySize = static_cast<int>(0.2f * sr);
        delete[] predelayBuf;
        ...
        // Early reflections
        earlyL.init(sr, size);
        earlyR.init(sr, size * 1.08f);

        // 6 diffusion allpasses (longer than plate for more diffusion)
        int diffSizes[] = { 284, 214, 758, 554, 410, 340 };
        float diffFbs[] = { 0.70f, 0.70f, 0.65f, 0.65f, 0.60f, 0.60f };
        for (int i = 0; i < 6; i++) {
            diffAP[i].init(static_cast<int>(diffSizes[i] * scale * size), diffFbs[i]);
        }

        // Parallel comb filters (2-3x plate sizes for longer tail)
        int combSizesL[] = { 2892, 3078, 3306, 3510 };
        int combSizesR[] = { 2940, 3132, 3366, 3576 };
        for (int i = 0; i < 4; i++) {
        ...
        // Tank allpasses
        tankAPL.init(static_cast<int>(1112 * scale * size), 0.5f);
        tankAPR.init(static_cast<int>(882 * scale * size), 0.5f);

        lpfStateL = lpfStateR = 0;
    }

    ~HallReverbInstance() { delete[] predelayBuf; }
    ...
    void process(const float* inL, const float* inR, float* outL, float* outR, int n) {
        ...
        float lpfCoeff = 0.7f - damping * 0.5f;
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int d = 0; d < 6; d++) diff = diffAP[d].process(diff);
...
static HallReverbInstance instances[MAX_INSTANCES];
static int findFree() { for (int i = 0; i < MAX_INSTANCES; i++) if (!instances[i].active) return i; return -1; }

extern "C" {

EMSCRIPTEN_KEEPALIVE int dragonfly_hall_create(int sr) {
    int s = findFree(); if (s < 0) return -1;
    instances[s].active = true; instances[s].init(static_cast<float>(sr)); return s;
}
EMSCRIPTEN_KEEPALIVE void dragonfly_hall_destroy(int h) { if (h >= 0 && h < MAX_INSTANCES) instances[h].active = false; }
EMSCRIPTEN_KEEPALIVE void dragonfly_hall_process(int h, float* iL, float* iR, float* oL, float* oR, int n) {
...
EMSCRIPTEN_KEEPALIVE void dragonfly_hall_set_decay(int h, float v) { if (h >= 0 && h < MAX_INSTANCES && instances[h].active) { instances[h].decay = std::clamp(v, 0.0f, 0.99f); instances[h].updateParams(); } }
EMSCRIPTEN_KEEPALIVE void dragonfly_hall_set_damping(int h, float v) { if (h >= 0 && h < MAX_INSTANCES && instances[h].active) { instances[h].damping = std::clamp(v, 0.0f, 1.0f); instances[h].updateParams(); } }
EMSCRIPTEN_KEEPALIVE void dragonfly_hall_set_predelay(int h, float v) { if (h >= 0 && h < MAX_INSTANCES && instances[h].active) instances[h].predelayMs = std::clamp(v, 0.0f, 200.0f); }
EMSCRIPTEN_KEEPALIVE void dragonfly_hall_set_width(int h, float v) { if (h >= 0 && h < MAX_INSTANCES && instances[h].active) instances[h].width = std::clamp(v, 0.0f, 1.0f); }
EMSCRIPTEN_KEEPALIVE void dragonfly_hall_set_early_level(int h, float v) { if (h >= 0 && h < MAX_INSTANCES && instances[h].active) instances[h].earlyLevel = std::clamp(v, 0.0f, 1.0f); }
EMSCRIPTEN_KEEPALIVE void dragonfly_hall_set_size(int h, float v) { if (h >= 0 && h < MAX_INSTANCES && instances[h].active) instances[h].size = std::clamp(v, 0.5f, 3.0f); }
