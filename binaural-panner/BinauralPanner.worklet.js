/**
 * BinauralPanner.worklet.js — AudioWorklet processor for BinauralPanner WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createBinauralPanner;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createBinauralPanner factory');
...
class BinauralPannerProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'azimuth': M['_binaural_panner_set_azimuth'](h, value); break;
      case 'elevation': M['_binaural_panner_set_elevation'](h, value); break;
      case 'distance': M['_binaural_panner_set_distance'](h, value); break;
      case 'mix': M['_binaural_panner_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_binaural_panner_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_binaural_panner_create'](sampleRate);
      if (this.handle < 0) throw new Error('binaural_panner_create failed');
      const bs = 128 * 4;
      ...
      console.error('[BinauralPanner] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_binaural_panner_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('binaural-panner-processor', BinauralPannerProcessor); processorRegistered = true; }
