...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    let createModule;
    try {
      const wrappedCode = `${jsCode}; return createArtisticDelay;`;
      createModule = new Function(wrappedCode)();
    } catch (e) { console.error('[ArtisticDelay] Failed:', e); sharedModulePromise = null; throw e; }
...
class ArtisticDelayProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
  ...
  applyParameter(param, value) {
    ...
    const fn = M['_artistic_delay_set_' + param];
    if (fn) fn(h, value);
  }
  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module._artistic_delay_destroy(this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = Module._artistic_delay_create(sampleRate);
      ...
      console.error('[ArtisticDelay] init error:', error);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module._artistic_delay_process(this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('artistic-delay-processor', ArtisticDelayProcessor); processorRegistered = true; }
