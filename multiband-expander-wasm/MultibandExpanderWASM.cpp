/**
 * MultibandExpanderWASM.cpp — MultibandExpander effect for DEViLBOX
...
struct MultibandExpanderInstance {
    bool active = false;
    ...
    float expandGain(float envDb, float thresh) {
        if (envDb >= thresh) return 1.0f;
        float below = thresh - envDb;
        float gainDb = -below * (ratio - 1.0f);
        gainDb = std::max(gainDb, range);
    ...
    void process(const float* inL, const float* inR, float* outL, float* outR, int n) {
        for (int i = 0; i < n; i++) {
            ...
            float aH=std::max(std::fabs(highL),std::fabs(highR));
            ...
            float gL=expandGain(20.0f*std::log10(envLow+1e-30f),lowThresh);
            float gM=expandGain(20.0f*std::log10(envMid+1e-30f),midThresh);
            float gH=expandGain(20.0f*std::log10(envHigh+1e-30f),highThresh);
...
static MultibandExpanderInstance instances[MAX_INSTANCES];
...
EMSCRIPTEN_KEEPALIVE int multiband_expander_create(int sampleRate) {
...
EMSCRIPTEN_KEEPALIVE void multiband_expander_destroy(int h) {
    if (h >= 0 && h < MAX_INSTANCES) instances[h].active = false;
}

EMSCRIPTEN_KEEPALIVE void multiband_expander_process(int h, float* inL, float* inR, float* outL, float* outR, int n) {
...
EMSCRIPTEN_KEEPALIVE void multiband_expander_set_lowCross(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) {
...
EMSCRIPTEN_KEEPALIVE void multiband_expander_set_highCross(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) {
...
EMSCRIPTEN_KEEPALIVE void multiband_expander_set_lowThresh(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) {
        instances[h].lowThresh = std::clamp(v, -60.0f, 0.0f);
    }
}

EMSCRIPTEN_KEEPALIVE void multiband_expander_set_midThresh(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) {
        instances[h].midThresh = std::clamp(v, -60.0f, 0.0f);
    }
}

EMSCRIPTEN_KEEPALIVE void multiband_expander_set_highThresh(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) {
        instances[h].highThresh = std::clamp(v, -60.0f, 0.0f);
    }
}

EMSCRIPTEN_KEEPALIVE void multiband_expander_set_ratio(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) {
        instances[h].ratio = std::clamp(v, 1.0f, 10.0f);
    }
}

EMSCRIPTEN_KEEPALIVE void multiband_expander_set_attack(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) {
...
EMSCRIPTEN_KEEPALIVE void multiband_expander_set_release(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) {
...
EMSCRIPTEN_KEEPALIVE void multiband_expander_set_range(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) {
        instances[h].range = std::clamp(v, -90.0f, 0.0f);
    }
}

EMSCRIPTEN_KEEPALIVE void multiband_expander_set_mix(int h, float v) {
    if (h >= 0 && h < MAX_INSTANCES && instances[h].active) {
