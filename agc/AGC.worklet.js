/**
 * AGC.worklet.js — AudioWorklet processor for AGC WASM effect.
 */
...
async function getOrCreateModule(wasmBinary, jsCode) {
  ...
  sharedModulePromise = (async () => {
    const wrappedCode = `${jsCode}; return createAGC;`;
    const createModule = new Function(wrappedCode)();
    if (!createModule) throw new Error('No createAGC factory');
...
class AGCProcessor extends AudioWorkletProcessor {
  ...
  applyParam(param, value) {
    ...
    switch (param) {
      case 'target': M['_agc_set_target'](h, value); break;
      case 'speed': M['_agc_set_speed'](h, value); break;
      case 'maxGain': M['_agc_set_maxGain'](h, value); break;
      case 'mix': M['_agc_set_mix'](h, value); break;
    }
  }

  cleanup() {
    if (this.Module) {
      if (this.handle >= 0) this.Module['_agc_destroy'](this.handle);
  ...
  async initialize(data) {
    try {
      ...
      this.handle = M['_agc_create'](sampleRate);
      if (this.handle < 0) throw new Error('agc_create failed');
      const bs = 128 * 4;
      ...
      console.error('[AGC] init error:', e);
  ...
  process(inputs, outputs) {
    ...
    try {
      ...
      this.Module['_agc_process'](this.handle, this.inputPtrL, this.inputPtrR, this.outputPtrL, this.outputPtrR, n);
...
if (!processorRegistered) { registerProcessor('agc-processor', AGCProcessor); processorRegistered = true; }
